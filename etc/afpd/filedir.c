			...
			char *rbuf, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	LOG(log_debug, logtype_afpd,
	    ...
	    ntohl(curdir->d_did), cfrombstr(curdir->d_fullpath),
	    s_path->u_name);
	...
	if (S_ISDIR(st->st_mode)) {
		...
		*(rbuf + 2 * sizeof(u_int16_t)) = (char) FILDIRBIT_ISDIR;
		...
		if (fbitmap
		    && AFP_OK != (ret =
				  getfilparams(vol, fbitmap, s_path,
					       curdir,
					       rbuf +
					       3 * sizeof(u_int16_t),
					       &buflen))) {
...
int check_name(const struct vol *vol, char *name)
{
	...
	if (!vol->vfs->vfs_validupath(vol, name)) {
		LOG(log_error, logtype_afpd,
		    "check_name: illegal name: '%s'", name);
		return AFPERR_EXIST;
	}
...
static int moveandrename(const struct vol *vol,
			 struct dir *sdir,
			 int sdir_fd,
			 char *oldname, char *newname, int isdir)
{
	...
	if (sdir_fd != -1) {
		if (fchdir(cwd_fd) != 0) {
			LOG(log_error, logtype_afpd, "moveandrename: %s",
			    strerror(errno));
			rc = AFPERR_MISC;
			goto exit;
		}
	...
	if (NULL ==
	    (upath =
	     mtoupath(vol, newname, curdir->d_did, utf8_encoding()))) {
		rc = AFPERR_PARAM;
		goto exit;
	}
	path.u_name = upath;
	...
	if ((!isdir && curdir == sdir)
	    || (isdir && curdir->d_did == sdir->d_pdid)) {
		...
		if (stat(upath, st) == 0
		    || caseenumerate(vol, &path, curdir) == 0) {
			if (!stat(oldunixname, &nst)
	...
	if (!isdir) {
		...
		if (of_findname(vol, &path)) {
			...
			rc = renamefile(vol, sdir_fd, oldunixname, upath,
					newname, adp);
			if (rc == AFP_OK)
				of_rename(vol, opened, sdir, oldname,
					  curdir, newname);
		}
	} else {
		rc = renamedir(vol, sdir_fd, oldunixname, upath, sdir,
			       curdir, newname);
	}
	if (rc == AFP_OK && id) {
		...
		if (!ad_metadata(upath, adflags, adp)) {
			ad_setid(adp, st->st_dev, st->st_ino, id,
				 curdir->d_did, vol->v_stamp);
			ad_flush(adp);
			ad_close_metadata(adp);
		}

		/* fix up the catalog entry */
		cnid_update(vol->v_cdb, id, st, curdir->d_did, upath,
			    strlen(upath));
	}
...
/* -------------------------------------------- */
int afp_rename(AFPObj * obj, char *ibuf, size_t ibuflen _U_,
	       char *rbuf _U_, size_t *rbuflen)
{
	struct vol *vol;
	struct dir *sdir;
	...
	*rbuflen = 0;
	...
	if (*path->m_name != '\0') {
		...
		memcpy(oldname, cfrombstr(sdir->d_m_name),
...
/* ------------------------------- */
int afp_delete(AFPObj * obj, char *ibuf, size_t ibuflen _U_,
	       char *rbuf _U_, size_t *rbuflen)
{
	struct vol *vol;
	struct dir *dir;
		...
		if (s_path->st_valid && s_path->st_errno == ENOENT) {
			...
			if ((cachedfile =
			     dircache_search_by_name(vol, dir, upath,
						     strlen(upath)))) {
				dircache_remove(vol, cachedfile,
						DIRCACHE | DIDNAME_INDEX |
						QUEUE_INDEX);
				dir_free(cachedfile);
			}
		}
...
char *ctoupath(const struct vol *vol, struct dir *dir, char *name)
{
	if (vol == NULL || dir == NULL || name == NULL)
		return NULL;
	return absupath(vol, dir,
			mtoupath(vol, name, dir->d_did, utf8_encoding()));
}
		      ...
		      char *rbuf _U_, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	/* This does the work */
	LOG(log_debug, logtype_afpd,
	    "afp_move(oldname:'%s', newname:'%s', isdir:%u)", oldname,
	    newname, isdir);
	rc = moveandrename(vol, sdir, sdir_fd, oldname, newname, isdir);

	if (rc == AFP_OK) {
		char *upath =
		    mtoupath(vol, newname, pdid, utf8_encoding());
		...
		if (!isdir && !vol_unix_priv(vol)) {
			...
			setfilmode(vol, upath, admode,
				   path->st_valid ? &path->st : NULL);
			vol->vfs->vfs_setfilmode(vol, upath, admode,
						 path->st_valid ? &path->
						 st : NULL);
		}
		setvoltime(obj, vol);
