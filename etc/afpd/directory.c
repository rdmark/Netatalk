	...
	while ((de = readdir(dp))) {
		...
		if (ostatat(dirfd, spath, &st, vol_syml_opt(vol)) == 0) {
			...
			if (S_ISDIR(st.st_mode)) {
				if (AFP_OK !=
				    (err =
				     copydir(vol, dirfd, spath, dpath)))
					goto copydir_done;
			} else if (AFP_OK !=
				   (err =
				    copyfile(vol, vol, dirfd, spath, dpath,
					     NULL, NULL))) {
				goto copydir_done;
			  ...
			  struct path *ret, int toUTF8)
{
	...
	if (ret->u_name == NULL) {
		if ((ret->u_name =
		     mtoupath(vol, ret->m_name, dir->d_did,
			      utf8_encoding())) == NULL) {
			afp_errno = AFPERR_PARAM;
			return -1;
		}
				  ...
				  struct path *ret)
{
	...
	case AFPERR_ACCESS:
		...
		if (dir->d_m_name == dir->d_u_name) {
			...
			ret->u_name =
			    ret->m_name + blength(dir->d_m_name) + 1;
			memcpy(ret->u_name, cfrombstr(dir->d_u_name),
			       blength(dir->d_u_name) + 1);
		}
		...
		LOG(log_debug, logtype_afpd,
		    ...
		    cfrombstr(curdir->d_fullpath), ret->u_name);
...
struct dir *dirlookup_bypath(const struct vol *vol, const char *path)
{
	...
	LOG(log_debug, logtype_afpd, "dirlookup_bypath: rpath: \"%s\"",
	    cfrombstr(rpath));
...
struct dir *dirlookup(const struct vol *vol, cnid_t did)
{
	...
	LOG(log_debug, logtype_afpd, "dirlookup(did: %u): START",
	    ntohl(did));
	...
	if ((ret = dircache_search_by_did(vol, did)) != NULL) {	/* 2a */
		...
		if (lstat(cfrombstr(ret->d_fullpath), &st) != 0) {
			LOG(log_debug, logtype_afpd,
			    "dirlookup(did: %u, path: \"%s\"): lstat: %s",
			    ntohl(did), cfrombstr(ret->d_fullpath),
			    strerror(errno));
			switch (errno) {
			case ENOENT:
			case ENOTDIR:
				...
				LOG(log_debug, logtype_afpd,
				    "dirlookup(did: %u): calling dir_remove",
				    ntohl(did));
				dir_remove(vol, ret);
	...
	cnid = did;
	LOG(log_debug, logtype_afpd,
	    "dirlookup(did: %u): querying CNID database", ntohl(did));
	if ((upath =
	     cnid_resolve(vol->v_cdb, &cnid, buffer, buflen)) == NULL) {
		afp_errno = AFPERR_NOOBJ;
		err = 1;
		goto exit;
	}
	 ...
	 */
	LOG(log_debug, logtype_afpd,
	    "dirlookup(did: %u): recursion for did: %u", ntohl(did),
	    ntohl(pdid));
	...
	if ((ret = dir_new(mpath, upath, vol, pdid, did, fullpath, &st)) == NULL) {	/* 6 */
		LOG(log_error, logtype_afpd,
		    "dirlookup(did: %u) {%s, %s}: %s", ntohl(did), mpath,
	...
	if (err) {
		LOG(log_debug, logtype_afpd,
		    "dirlookup(did: %u) {exit_error: %s}", ntohl(did),
	...
	if (ret)
		LOG(log_debug, logtype_afpd,
		    ...
		    ntohl(ret->d_did), ntohl(ret->d_pdid),
		    cfrombstr(ret->d_fullpath));
		    ...
		    cnid_t pdid, cnid_t did, bstring path, struct stat *st)
{
	...
	if (convert_string_allocate
	    ((utf8_encoding())? CH_UTF8_MAC : vol->v_maccharset, CH_UCS2,
	     m_name, -1, (char **) &dir->d_m_name_ucs2) == (size_t) -1) {
		LOG(log_error, logtype_afpd,
		    "dir_new(did: %u) {%s, %s}: couldn't set UCS2 name",
		    ntohl(did), m_name, u_name);
 ...
 */
struct dir *dir_add(struct vol *vol, const struct dir *dir,
		    struct path *path, int len)
{
	...
	if ((cdir =
	     dircache_search_by_name(vol, dir, path->u_name,
	...
	if (path->m_name == NULL) {
		if ((path->m_name =
		     utompath(vol, path->u_name, id,
			      utf8_encoding())) == NULL) {
			LOG(log_error, logtype_afpd,
			    "dir_add(\"%s\"): can't assign macname",
			    path->u_name);
			err = 2;
			goto exit;
		}
	...
	if (((fullpath = bstrcpy(dir->d_fullpath)) == NULL)	/* 3 */
		...
		LOG(log_error, logtype_afpd, "dir_add: fullpath: %s",
		    strerror(errno));
		err = 3;
		goto exit;
	}
	...
	if ((cdir = dir_new(path->m_name, path->u_name, vol, dir->d_did, id, fullpath, &path->st)) == NULL) {	/* 3 */
		err = 4;
		goto exit;
	}
	...
	if ((dircache_add(vol, cdir)) != 0) {	/* 4 */
		LOG(log_error, logtype_afpd,
		    "dir_add: fatal dircache error: %s",
		    cfrombstr(fullpath));
		exit(EXITERR_SYS);
	}
	...
	if (err != 0) {
		...
		/* no error */
		LOG(log_debug, logtype_afpd,
		    "dir_add(did:%u,'%s/%s'): {cached: %u,'%s'}",
		    ntohl(dir->d_did), cfrombstr(dir->d_fullpath),
		    path->u_name, ntohl(cdir->d_did),
		    cfrombstr(cdir->d_fullpath));
	}
	       ...
	       const char *new_uname, bstring pdir_fullpath)
{
	...
	if (new_mname) {
		...
		if ((bassigncstr(dir->d_m_name, new_mname)) != BSTR_OK) {
			LOG(log_error, logtype_afpd,
			    "dir_modify: bassigncstr: %s",
			    strerror(errno));
			return -1;
		}

		if (new_mname == new_uname
		    || (strcmp(new_mname, new_uname) == 0)) {
			dir->d_u_name = dir->d_m_name;
		} else {
	...
	if (dir->d_m_name_ucs2)
		free(dir->d_m_name_ucs2);
	if ((size_t) -1 ==
	    convert_string_allocate((utf8_encoding())? CH_UTF8_MAC : vol->
				    v_maccharset, CH_UCS2, dir->d_m_name,
				    -1, (char **) &dir->d_m_name_ucs2))
		dir->d_m_name_ucs2 = NULL;
...
struct path *cname(struct vol *vol, struct dir *dir, char **cpath)
{
	...
	*cpath += len + size;
	...
	if (movecwd(vol, dir) < 0) {
		LOG(log_debug, logtype_afpd,
		    "cname(did:%u): failed to chdir to '%s'",
		    ntohl(dir->d_did), cfrombstr(dir->d_fullpath));
		if (len == 0)
	...
	while (len) {		/* 3 */
		...
		*p = 0;		/* Terminate string */
		...
		if (cname_mtouname(vol, dir, &ret, toUTF8) != 0) {	/* 7 */
			LOG(log_error, logtype_afpd,
			    "cname('%s'): error from cname_mtouname",
			    path);
			return NULL;
		}
		...
		if (check_name(vol, ret.u_name)) {
			...
			LOG(log_info, logtype_afpd,
			    "cname: illegal path: '%s'", ret.u_name);
			afp_errno = AFPERR_PARAM;
			return NULL;
		}
		...
		if (movecwd(vol, cdir) < 0) {	/* 16 */
			LOG(log_debug, logtype_afpd,
			    ...
			    cfrombstr(cdir->d_fullpath), strerror(errno));
			if (len == 0)
...
int movecwd(const struct vol *vol, struct dir *dir)
{
	...
	LOG(log_maxdebug, logtype_afpd,
	    "movecwd: from: curdir:\"%s\", cwd:\"%s\"",
	    curdir ? cfrombstr(curdir->d_fullpath) : "INVALID",
	    getcwdpath());
	...
	if ((ret =
	     ochdir(cfrombstr(dir->d_fullpath), vol_syml_opt(vol))) != 0) {
...
int file_access(struct path *path, int mode)
{
	...
	LOG(log_debug, logtype_afpd,
	    "file_access(\"%s\"): mapped user mode: 0x%02x", path->u_name,
	    ma.ma_user);

	if ((mode & OPENACC_WR) && !(ma.ma_user & AR_UWRITE)) {
		LOG(log_debug, logtype_afpd,
		    "file_access(\"%s\"): write access denied",
		    path->u_name);
		return -1;
	}
	if ((mode & OPENACC_RD) && !(ma.ma_user & AR_UREAD)) {
		LOG(log_debug, logtype_afpd,
		    "file_access(\"%s\"): read access denied",
		    path->u_name);
		return -1;
	}
	return 0;
...
int getdirparams(const struct vol *vol,
		 ...
		 struct dir *dir, char *buf, size_t *buflen)
{
	struct maccess ma;
	struct adouble ad;
	...
	while (bitmap != 0) {
		...
		case DIRPBIT_PDID:
			...
			data += sizeof(pdid);
			LOG(log_debug, logtype_afpd,
			    "metadata('%s'):     Parent DID: %u",
			    s_path->u_name, ntohl(pdid));
			break;

		case DIRPBIT_CDATE:
			if (!isad
			    || (ad_getdate(&ad, AD_DATE_CREATE, &aint) <
				0))
		...
		case DIRPBIT_OFFCNT:
			...
			if (diroffcnt(dir, st)) {
				ashort =
				    (dir->d_offcnt >
				     0xffff) ? 0xffff : dir->d_offcnt;
			} else
			    if ((ret =
				 for_each_dirent(vol, upath, NULL,
						 NULL)) >= 0) {
	...
	if (l_nameoff) {
		...
		data =
		    set_name(vol, data, pdid, cfrombstr(dir->d_m_name),
			     dir->d_did, 0);
	}
	if (utf_nameoff) {
		ashort = htons(data - buf);
 ...
 */
int setdirparams(struct vol *vol, struct path *path, u_int16_t d_bitmap,
		 char *buf)
{
	struct maccess ma;
	struct adouble ad;
	...
	while (bitmap != 0) {
		...
		case DIRPBIT_ACCESS:
			...
			if (dir_rx_set(mpriv)
			    && setdirmode(vol, upath, mpriv) < 0) {
				err =
				    set_dir_errors(path, "setdirmode",
						   errno);
				bitmap = 0;
			}
			break;
	...
	while (bitmap != 0) {
		...
		case DIRPBIT_UID:	/* What kind of loser mounts as root? */
			if ((dir->d_did == DIRDID_ROOT) &&
				...
				err =
				    set_dir_errors(path, "setdeskowner",
						   errno);
			...
			if (setdirowner(vol, upath, ntohl(owner), -1) < 0) {
				err =
				    set_dir_errors(path, "setdirowner",
						   errno);
				goto setdirparam_done;
			}
			break;
	...
	if (change_parent_mdate && dir->d_did != DIRDID_ROOT
		...
		if (movecwd(vol, dirlookup(vol, dir->d_pdid)) == 0) {
			...
			setdirparams(vol, &Cur_Path, bitmap,
				     (char *) &newdate);
...
int afp_syncdir(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		char *rbuf _U_, size_t *rbuflen)
{
#ifdef HAVE_DIRFD
	DIR *dp;
	...
	*rbuflen = 0;
	...
	if (ntohl(did) == 2) {
		...
		if (fsync(dfd) < 0)
			LOG(log_error, logtype_afpd,
			    "afp_syncdir(%s):  %s", dir->d_u_name,
			    strerror(errno));
		closedir(dp);	/* closes dfd too */
#endif

		if (-1 ==
		    (dfd = open(vol->ad_path(".", ADFLAGS_DIR), O_RDWR))) {
			switch (errno) {
			case ENOENT:
				return (AFPERR_NOOBJ);
			case EACCES:
		...
		if (fsync(dfd) < 0)
			LOG(log_error, logtype_afpd, "afp_syncdir(%s): %s",
			    vol->ad_path(cfrombstr(dir->d_u_name),
					 ADFLAGS_DIR), strerror(errno));
		close(dfd);
	}
		  ...
		  size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if ((dir =
	     dir_add(vol, curdir, s_path,
		     strlen(s_path->u_name))) == NULL) {
		return AFPERR_MISC;
	}
	...
	ad_setname(&ad, s_path->m_name);
	ad_setid(&ad, s_path->st.st_dev, s_path->st.st_ino, dir->d_did,
		 did, vol->v_stamp);
...
int renamedir(const struct vol *vol,
	      ...
	      char *src,
	      char *dst,
	      struct dir *dir, struct dir *newparent, char *newname)
{
	struct adouble ad;
	int err;
...
int deletecurdir(struct vol *vol)
{
	...
	if (err) {
		LOG(log_error, logtype_afpd,
		    "deletecurdir: error deleting .AppleDouble in \"%s\"",
		    cfrombstr(curdir->d_fullpath));
		return err;
	}
	...
	*rbuflen = 0;
	...
	case 3:		/* unicode */
		...
		if (id != 0) {
			...
			len =
			    convert_string_allocate(obj->options.
						    unixcharset,
						    ((!utf8) ? obj->
						     options.
						     maccharset :
	...
	case 6:		/* UUID -> groupname */
		...
		uuidtype_t type;
		len =
		    getnamefromuuid((unsigned char *) ibuf, &name, &type);
...
int afp_mapname(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		char *rbuf, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	LOG(log_debug, logtype_afpd,
	    "afp_mapname: sfunc: %d, afp_version: %d", sfunc, afp_version);
	switch (sfunc) {
	case 1:
	case 2:		/* unicode */
		if (afp_version < 30) {
	...
	if (len == 0)
		...
		case 3:
			if (NULL ==
			    (pw = (struct passwd *) getpwnam(ibuf))) {
				return (AFPERR_NOITEM);
			}
			id = pw->pw_uid;
			id = htonl(id);
		...
		case 4:
			LOG(log_debug, logtype_afpd,
			    "afp_mapname: getgrnam for name: %s", ibuf);
			if (NULL == (gr = (struct group *) getgrnam(ibuf))) {
				return (AFPERR_NOITEM);
			}
			id = gr->gr_gid;
			LOG(log_debug, logtype_afpd,
			    "afp_mapname: getgrnam for name: %s -> id: %d",
			    ibuf, id);
			id = htonl(id);
			memcpy(rbuf, &id, sizeof(id));
			*rbuflen = sizeof(id);
		...
		case 5:	/* username -> UUID */
			LOG(log_debug, logtype_afpd,
			    "afp_mapname: name: %s", ibuf);
			if (0 !=
			    getuuidfromname(ibuf, UUID_USER,
					    (unsigned char *) rbuf))
				return AFPERR_NOITEM;
		...
		case 6:	/* groupname -> UUID */
			LOG(log_debug, logtype_afpd,
			    "afp_mapname: name: %s", ibuf);
			if (0 !=
			    getuuidfromname(ibuf, UUID_GROUP,
					    (unsigned char *) rbuf))
				return AFPERR_NOITEM;
			*rbuflen = UUID_BINSIZE;
			break;
 ...
 */
int afp_opendir(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		char *rbuf, size_t *rbuflen)
{
	struct vol *vol;
