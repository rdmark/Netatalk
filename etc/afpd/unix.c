...
int ustatfs_getvolspace(const struct vol *vol, VolSpace * bfree,
			VolSpace * btotal, u_int32_t * bsize)
{
	...
	if (statfs(vol->v_path, &sfs) < 0) {
		LOG(log_error, logtype_afpd,
		    "ustatfs_getvolspace unable to stat %s", vol->v_path);
		return (AFPERR_PARAM);
	}
 ...
 */
void accessmode(const struct vol *vol, char *path, struct maccess *ma,
		struct dir *dir _U_, struct stat *st)
{
	struct stat sb;
...
int setdeskmode(const mode_t mode)
{
	...
	for (deskp = readdir(desk); deskp != NULL; deskp = readdir(desk)) {
		...
		for (subp = readdir(sub); subp != NULL;
		     subp = readdir(sub)) {
			...
			*m = '\0';
			...
			if (S_ISDIR(st.st_mode)) {
				if (chmod_acl
				    (modbuf,
				     (DIRBITS | mode) & ~default_options.
				     umask) < 0 && errno != EPERM) {
					LOG(log_error, logtype_afpd,
					    "setdeskmode: chmod %s: %s",
					    fullpathname(modbuf),
					    strerror(errno));
				}
			} else
		...
		if (chmod_acl
		    (deskp->d_name,
		     (DIRBITS | mode) & ~default_options.umask) < 0
		    && errno != EPERM) {
			LOG(log_error, logtype_afpd,
			    "setdeskmode: chmod %s: %s",
	...
	closedir(desk);
	if (chdir(wd) < 0) {
		LOG(log_error, logtype_afpd, "setdeskmode: chdir %s: %s",
		    wd, strerror(errno));
		return -1;
	}
	/* XXX: need to preserve special modes */
	if (chmod_acl
		...
		LOG(log_error, logtype_afpd, "setdeskmode: chmod %s: %s",
		    fullpathname(".AppleDesktop"), strerror(errno));
	}
	return (0);
}
...
int setfilunixmode(const struct vol *vol, struct path *path, mode_t mode)
{
	...
	return vol->vfs->vfs_setfilmode(vol, path->u_name, mode,
					&path->st);
}
...
int setdirmode(const struct vol *vol, const char *name, mode_t mode)
{
	...
	if ((dir = opendir(name)) == NULL) {
		LOG(log_error, logtype_afpd, "setdirmode: opendir: %s",
		    fullpathname(name), strerror(errno));
		return (-1);
	}
	...
	for (dirp = readdir(dir); dirp != NULL; dirp = readdir(dir)) {
		...
		if (ostat(dirp->d_name, &st, vol_syml_opt(vol)) < 0) {
			LOG(log_error, logtype_afpd,
			    "setdirmode: stat %s: %s", dirp->d_name,
			    strerror(errno));
			continue;
		}
		...
		if (!S_ISDIR(st.st_mode)) {
			...
			if (setfilmode(vol, dirp->d_name, setmode, &st) <
			    0) {
				closedir(dir);
				LOG(log_error, logtype_afpd,
				    "setdirmode: chmod %s: %s",
...
int setdeskowner(const uid_t uid, const gid_t gid)
{
	...
	for (deskp = readdir(desk); deskp != NULL; deskp = readdir(desk)) {
		...
		if (chown(deskp->d_name, uid, gid) < 0 && errno != EPERM) {
			LOG(log_error, logtype_afpd,
			    "setdeskowner: chown %s: %s", deskp->d_name,
			    strerror(errno));
		}
	}
	closedir(desk);
	if (chdir(wd) < 0) {
		LOG(log_error, logtype_afpd, "setdeskowner: chdir %s: %s",
		    wd, strerror(errno));
		return -1;
	}
		...
		struct path *path)
{
	...
	if (ochown(path->u_name, uid, gid, vol_syml_opt(vol)) < 0
	    && errno != EPERM) {
		LOG(log_debug, logtype_afpd,
		    "setfilowner: chown %d/%d %s: %s", uid, gid,
		    path->u_name, strerror(errno));
		return -1;
	}

	if (vol->vfs->vfs_chown(vol, path->u_name, uid, gid) < 0
	    && errno != EPERM) {
		LOG(log_debug, logtype_afpd,
		    "setfilowner: rf_chown %d/%d %s: %s", uid, gid,
		    path->u_name, strerror(errno));
		return -1;
	}
		...
		const gid_t gid)
{
	...
	for (dirp = readdir(dir); dirp != NULL; dirp = readdir(dir)) {
		...
		if (ostat(dirp->d_name, &st, vol_syml_opt(vol)) < 0) {
			LOG(log_error, logtype_afpd,
			    "setdirowner: stat %s: %s",
			    fullpathname(dirp->d_name), strerror(errno));
			continue;
		}
		if ((st.st_mode & S_IFMT) == S_IFREG) {
			if (ochown
			    (dirp->d_name, uid, gid, vol_syml_opt(vol)) < 0
			    && errno != EPERM) {
				LOG(log_debug, logtype_afpd,
				    "setdirowner: chown %s: %s",
				    fullpathname(dirp->d_name),
				    strerror(errno));
	...
	if (gid && gid != st.st_gid
	    && ochown(".", uid, gid, vol_syml_opt(vol)) < 0
	    && errno != EPERM) {
		LOG(log_debug, logtype_afpd,
...
static int recursive_chown(const char *path, uid_t uid, gid_t gid)
{
	...
	if (chown(path, uid, gid) < 0) {
		LOG(log_error, logtype_afpd,
		    "cannot chown() file [%s] (uid = %d): %s", path, uid,
		    strerror(errno));
		return -1;
	}
