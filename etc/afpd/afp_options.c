...
{
	if (opt->defaultvol.name
	    && (opt->defaultvol.name != save->defaultvol.name))
		free(opt->defaultvol.name);
	if (opt->defaultvol.full_name
	    && (opt->defaultvol.full_name != save->defaultvol.full_name))
		free(opt->defaultvol.full_name);

	if (opt->systemvol.name
	    && (opt->systemvol.name != save->systemvol.name))
		free(opt->systemvol.name);
	if (opt->systemvol.full_name
	    && (opt->systemvol.full_name != save->systemvol.full_name))
		free(opt->systemvol.full_name);

	if (opt->uservol.name && (opt->uservol.name != save->uservol.name))
		free(opt->uservol.name);
	if (opt->uservol.full_name
	    && (opt->uservol.full_name != save->uservol.full_name))
	...
	if (opt->adminauthuser
	    && (opt->adminauthuser != save->adminauthuser))
...
int afp_options_parseline(char *buf, struct afp_options *options)
{
	...
	if ((c = getoption(buf, "-ipaddr"))) {
		...
		if (inet_aton(c, &inaddr) && (opt = strdup(c))) {
			if (!gethostbyaddr
			    ((const char *) &inaddr, sizeof(inaddr),
			     AF_INET))
				LOG(log_info, logtype_afpd,
				    "WARNING: can't find %s", opt);
			options->ipaddr = opt;
		} else {
			LOG(log_error, logtype_afpd,
			    "Error parsing -ipaddr, is %s in numbers-and-dots notation?",
			    c);
		}
#endif
	...
	if ((c = getoption(buf, "-cnidserver"))) {
		...
		LOG(log_debug, logtype_afpd, "CNID Server: %s:%s",
		    Cnid_srv, Cnid_port);
	}
	...
	if ((c = getoption(buf, "-fqdn"))) {
		...
		if (gethostbyname(c)) {
			...
			LOG(log_error, logtype_afpd,
			    "error parsing -fqdn, gethostbyname failed for: %s",
			    c);
		}
	}
	...
	if ((c = getoption(buf, "-unixcodepage"))) {
		if ((charset_t) - 1 ==
		    (options->unixcharset = add_charset(c))) {
			options->unixcharset = CH_UNIX;
			LOG(log_warning, logtype_afpd,
			    "setting Unix codepage to '%s' failed", c);
		} else {
			if ((opt = strdup(c)))
	...
	if ((c = getoption(buf, "-maccodepage"))) {
		if ((charset_t) - 1 ==
		    (options->maccharset = add_charset(c))) {
			options->maccharset = CH_MAC;
			LOG(log_warning, logtype_afpd,
			    "setting Mac codepage to '%s' failed", c);
		} else {
			if ((opt = strdup(c)))
...
{
	fprintf(stderr,
		"Usage:\t%s [-duptDTI] [-f defaultvolumes] [-s systemvolumes] [-n nbpname]\n",
		name);
	fprintf(stderr,
