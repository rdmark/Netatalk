...
int of_flush(const struct vol *vol)
{
	...
	for (refnum = 0; refnum < nforks; refnum++) {
		if (oforks[refnum] != NULL
		    && (oforks[refnum]->of_vol == vol)
		    && flushfork(oforks[refnum]) < 0) {
			LOG(log_error, logtype_afpd, "of_flush: %s",
			    strerror(errno));
		}
	}
	return (0);
	      ...
	      struct dir *newdir, const char *newpath)
{
	...
	while ((of = next)) {
		...
		if (vol == of->of_vol
			...
			if (!done) {
				strlcpy(of_name(of), newpath,
					of->of_ad->ad_m_namelen);
				done = 1;
			}
			if (newdir != olddir)
...
struct ofork *of_alloc(struct vol *vol,
		       ...
		       char *path,
		       u_int16_t * ofrefnum,
		       const int eid, struct adouble *ad, struct stat *st)
{
	struct ofork *of;
	...
	if ((oforks[of_refnum] =
		...
		LOG(log_error, logtype_afpd, "of_alloc: malloc: %s",
		    strerror(errno));
		return NULL;
	}
	of = oforks[of_refnum];
	...
	if (!ad) {
		...
		if (!ad) {
			LOG(log_error, logtype_afpd,
			    "of_alloc: malloc: %s", strerror(errno));
			free(of);
		...
		if ((ad->ad_m_name =
		     (char *) malloc(ad->ad_m_namelen)) == NULL) {
			LOG(log_error, logtype_afpd,
			    "of_alloc: malloc: %s", strerror(errno));
			free(ad);
			free(of);
...
int of_stat(const struct vol *vol, struct path *path)
{
	...
	if ((ret = ostat(path->u_name, &path->st, vol_syml_opt(vol))) < 0) {
		LOG(log_debug, logtype_afpd, "of_stat('%s/%s': %s)",
		    cfrombstr(curdir->d_fullpath), path->u_name,
		    strerror(errno));
		path->st_errno = errno;
	}
...
int of_fstatat(int dirfd, struct path *path)
{
	...
	if ((ret =
	     fstatat(dirfd, path->u_name, &path->st,
...
int of_statdir(struct vol *vol, struct path *path)
{
	...
	LOG(log_debug, logtype_afpd, "of_statdir: stating: '%s'",
	    pathname);
	...
	if (errno == EACCES && (dir = dirlookup(vol, curdir->d_pdid))) {
		...
		if ((ret =
		     ostat(cfrombstr(path->d_dir->d_u_name), &path->st,
			   vol_syml_opt(vol))) < 0)
			path->st_errno = errno;
	}
