			     ...
			     char *mfilename, size_t prefix, char *ext)
{
	...
	if ((size_t) -1 ==
	    (len =
	     convert_charset(vol->v_volcharset, vol->v_maccharset, 0,
			     uname, strlen(uname), buffer, MAXPATHLEN,
			     &flags))) {
		return mfilename;
	}
	...
	if ((flags & CONV_REQMANGLE)) {
		if (len) {
			...
			if ((size_t) -1 ==
			    (len =
			     convert_charset(vol->v_maccharset,
					     CH_UTF8_MAC, 0, buffer, len,
					     buffer, MAXPATHLEN,
					     &flags))) {
				return mfilename;
			}
...
/* -------------------------------------------------------
*/
static char *private_demangle(const struct vol *vol, char *mfilename,
			      cnid_t did, cnid_t * osx)
{
	char *t;
	char *u_name;
	...
	if ((*t != 0 && *t != '.') || strlen(t) > MAX_EXT_LENGTH
	    || id < 17) {
		return mfilename;
	}
	...
	if ((dir = dirlookup(vol, id))) {
		...
		if (!osx) {
			...
			if (strcmp(cfrombstr(dir->d_m_name), mfilename) ==
			    0) {
				return cfrombstr(dir->d_u_name);
			}
		} else {
			return demangle_checks(vol,
					       cfrombstr(dir->d_u_name),
					       mfilename, prefix, t);
		}
	} else if (NULL !=
		   (u_name = cnid_resolve(vol->v_cdb, &id, buffer, len))) {
		if (id != did) {
			return mfilename;
		}
		if (!osx) {
			/* convert back to mac name and check it's the same */
			t = utompath(vol, u_name, file_id,
				     utf8_encoding());
			if (!strcmp(t, mfilename)) {
				return u_name;
			}
		} else {
			return demangle_checks(vol, u_name, mfilename,
					       prefix, t);
		}
	}
...
*/
char *mangle(const struct vol *vol, char *filename, size_t filenamelen,
	     char *uname, cnid_t id, int flags)
{
	char *m = NULL;
	...
	ext_len =
	    mangle_extension(vol, uname, ext,
			     (flags & 2) ? CH_UTF8_MAC : vol->
			     v_maccharset);
	m = mfilename;
