...
struct uam_mod *uam_load(const char *path, const char *name)
{
	...
	if ((module = mod_open(path)) == NULL) {
		LOG(log_error, logtype_afpd,
		    "uam_load(%s): failed to load: %s", name, mod_error());
		return NULL;
	}

	if ((mod =
	     (struct uam_mod *) malloc(sizeof(struct uam_mod))) == NULL) {
		LOG(log_error, logtype_afpd, "uam_load(%s): malloc failed",
		    name);
		goto uam_load_fail;
	}
	...
	if ((mod->uam_fcn = mod_symbol(module, buf)) == NULL) {
		LOG(log_error, logtype_afpd,
		    "uam_load(%s): mod_symbol error for symbol %s", name,
		    buf);
		goto uam_load_err;
	}
	...
	if (!mod->uam_fcn->uam_setup ||
		...
		LOG(log_error, logtype_afpd,
		    "uam_load(%s): uam_setup failed", name);
		goto uam_load_err;
	}
...
int uam_register(const int type, const char *path, const char *name, ...)
{
	...
	if ((uam = auth_uamfind(type, name, strlen(name)))) {
		if (strcmp(uam->uam_path, path)) {
			...
			LOG(log_error, logtype_afpd,
			    "uam_register: \"%s\" already loaded by %s",
			    name, path);
			return -1;
		}
		uam->uam_count++;
...
struct passwd *uam_getname(void *private, char *name, const int len)
{
	...
	if (obj->options.ntdomain && obj->options.ntseparator) {
		...
		size_t ulen =
		    strlen(obj->options.ntdomain) +
		    strlen(obj->options.ntseparator) + strlen(name);
		if ((p = malloc(ulen + 1))) {
			...
			if (pwent) {
				...
				if (len < MAXUSERLEN) {
					strncpy(name, pwent->pw_name,
						MAXUSERLEN);
				} else {
					LOG(log_error, logtype_uams,
					    "MAJOR:The name %s is longer than %d",
					    pwent->pw_name, MAXUSERLEN);
				}
	...
	while ((pwent = getpwent())) {
		...
		gecoslen =
		    convert_string(obj->options.unixcharset, CH_UCS2,
				   pwent->pw_gecos, -1, user,
				   sizeof(username));
		pwnamelen =
		    convert_string(obj->options.unixcharset, CH_UCS2,
				   pwent->pw_name, -1, pwname,
				   sizeof(username));
		...
		if ((namelen == gecoslen
		     && strncasecmp_w((ucs2_t *) user, (ucs2_t *) username,
				      len) == 0) || (namelen == pwnamelen
						     &&
						     strncasecmp_w((ucs2_t
								    *)
								   pwname,
								   (ucs2_t
								    *)
...
int uam_checkuser(const struct passwd *pwd)
{
	...
	if (!pwd->pw_shell || (*pwd->pw_shell == '\0')) {
		LOG(log_info, logtype_afpd,
		    "uam_checkuser: User %s does not have a shell",
		    pwd->pw_name);
		return -1;
	}
	...
	if (!p) {
		LOG(log_info, logtype_afpd, "illegal shell %s for %s",
...
int uam_random_string(AFPObj * obj, char *buf, int len)
{
	...
	if ((fd = open("/dev/urandom", O_RDONLY)) < 0) {
		...
		if (gettimeofday(&tv, &tz) < 0)
			return -1;
		srandom(tv.tv_sec + (uintptr_t) obj +
