...
static int posix_acl_rights(const char *path,
			    const struct stat *sb, uint32_t * result)
{
	EC_INIT;
	...
	EC_NULL_LOGSTR(acl = acl_get_file(path, ACL_TYPE_ACCESS),
		       "acl_get_file(\"%s\"): %s", fullpathname(path),
		       strerror(errno));
		...
		case ACL_USER_OBJ:
			if (sb->st_uid == uuid) {
				LOG(log_maxdebug, logtype_afpd,
				    "ACL_USER_OBJ: %u", sb->st_uid);
				rights |=
				    posix_permset_to_darwin_rights(e,
								   S_ISDIR
								   (sb->
								    st_mode));
			}
 ...
 * @returns                  0 or -1 on error
 */
static int posix_acls_to_uaperms(const char *path, struct stat *sb,
				 struct maccess *ma)
{
	EC_INIT;
	...
	while (acl_get_entry(acl, entry_id, &entry) == 1) {
		...
		case ACL_MASK:
			...
			LOG(log_maxdebug, logtype_afpd, "ACL_MASK: 0x%02x",
			    mask);
			break;
 ...
 */
static acl_perm_t map_darwin_right_to_posix_permset(uint32_t
						    darwin_ace_rights,
						    int is_dir)
{
	acl_perm_t perm = 0;
	...
	if (darwin_ace_rights &
	    (DARWIN_ACE_WRITE_DATA |
	     (is_dir ? DARWIN_ACE_DELETE_CHILD : 0)))
		perm |= ACL_WRITE;
			      ...
			      acl_perm_t perm)
{
	...
	int found = 0;
	for (; (!found) && acl_get_entry(*aclp, entry_id, &e) == 1;
	     entry_id = ACL_NEXT_ENTRY) {
		EC_ZERO_LOG(acl_get_tag_type(e, &tag));
				    ...
				    uint32_t * default_acl_flags)
{
	...
	for (; ace_count != 0; ace_count--, darwin_aces++) {
		...
		darwin_ace_rights = ntohl(darwin_aces->darwin_ace_rights);
		perm =
		    map_darwin_right_to_posix_permset(darwin_ace_rights,
						      (*def_aclp != NULL));
		if (perm == 0)
		...
		/* uid/gid */
		EC_ZERO_LOG(getnamefromuuid
			    (darwin_aces->darwin_ace_uuid, &name,
			     &uuidtype));
		switch (uuidtype) {
		case UUID_USER:
			EC_NULL_LOG(pwd = getpwnam(name));
		...
		if (darwin_ace_flags & DARWIN_ACE_INHERIT_CONTROL_FLAGS) {
			if (*def_aclp == NULL) {
				...
				LOG(log_warning, logtype_afpd,
				    "map_acl: unexpected ACE, flags: 0x%04x",
				    darwin_ace_flags);
				EC_FAIL;
			}
			/* add it as default ace */
			EC_ZERO_LOG(posix_acl_add_perm
				    (def_aclp, tag, id, perm));
			*default_acl_flags =
			    (HAS_DEFAULT_ACL | HAS_EXT_DEFAULT_ACL);
				   ...
				   darwin_ace_t * darwin_aces)
{
	...
	while (acl_get_entry(acl, entry_id, &e) == 1) {
		...
		case ACL_USER:
			...
			LOG(log_debug, logtype_afpd,
			    "map_aces_posix_to_darwin: uid: %d -> name: %s",
			    *uid, pwd->pw_name);
			EC_ZERO_LOG(getuuidfromname
				    (pwd->pw_name, UUID_USER,
				     darwin_aces->darwin_ace_uuid));
			acl_free(uid);
			uid = NULL;
			break;
		...
		case ACL_GROUP:
			...
			LOG(log_debug, logtype_afpd,
			    "map_aces_posix_to_darwin: gid: %d -> name: %s",
			    *gid, grp->gr_name);
			EC_ZERO_LOG(getuuidfromname
				    (grp->gr_name, UUID_GROUP,
				     darwin_aces->darwin_ace_uuid));
			acl_free(gid);
			gid = NULL;
			break;
...
static int map_acl(int type, void *acl, darwin_ace_t * buf, int ace_count)
{
	...
	case POSIX_DEFAULT_2_DARWIN:
		mapped_aces =
		    map_acl_posix_to_darwin(type, (const acl_t) acl, buf);
		break;
...
static int set_acl(const struct vol *vol,
		   const char *name,
		   int inherit _U_,
		   darwin_ace_t * daces, uint32_t ace_count)
{
	EC_INIT;
	struct stat st;
	...
	EC_NULL_LOG_ERR(access_acl =
			acl_get_file(name, ACL_TYPE_ACCESS), AFPERR_MISC);
	...
	while (acl_get_entry(access_acl, entry_id, &entry) == 1) {
		...
		if ((tag == ACL_USER) || (tag == ACL_GROUP)
		    || (tag == ACL_MASK)) {
			EC_ZERO_LOG_ERR(acl_delete_entry
					(access_acl, entry), AFPERR_MISC);
		}
	}			/* while */
	...
	if (S_ISDIR(st.st_mode)) {
		...
		if (default_acl) {
			...
			if (acl_get_entry
			    (default_acl, ACL_FIRST_ENTRY, &entry) == 1)
	...
	EC_ZERO_ERR(map_aces_darwin_to_posix
		    (daces, &default_acl, &access_acl, ace_count,
		     &default_acl_flags), AFPERR_MISC);
	...
	/* set it */
	EC_ZERO_LOG_ERR(acl_set_file(name, ACL_TYPE_ACCESS, access_acl),
			AFPERR_MISC);
	EC_ZERO_LOG_ERR(vol->vfs->
			vfs_acl(vol, name, ACL_TYPE_ACCESS, 0, access_acl),
			AFPERR_MISC);

	if (default_acl) {
		/* If the dir has an extended default acl it's ACL_MASK must be updated. */
		if (default_acl_flags & HAS_EXT_DEFAULT_ACL)
			EC_ZERO_LOG_ERR(acl_calc_mask(&default_acl),
					AFPERR_MISC);

		if (default_acl_flags) {
			EC_ZERO_LOG_ERR(acl_valid(default_acl),
					AFPERR_MISC);
			EC_ZERO_LOG_ERR(acl_set_file
					(name, ACL_TYPE_DEFAULT,
					 default_acl), AFPERR_MISC);
			EC_ZERO_LOG_ERR(vol->vfs->
					vfs_acl(vol, name,
						ACL_TYPE_DEFAULT, 0,
						default_acl), AFPERR_MISC);
			    ...
			    const uuidp_t uuid, uint32_t requested_rights)
{
	...
	case UUID_GROUP:
		LOG(log_warning, logtype_afpd,
		    "check_acl_access: afp_access not supported for groups");
		EC_STATUS(AFPERR_MISC);
		goto EC_CLEANUP;
	default:
		EC_STATUS(AFPERR_MISC);
...
#endif
#ifdef HAVE_POSIX_ACLS
			EC_ZERO_LOG(posix_acl_rights
				    (path, &st, &parent_rights));
#endif
			if (parent_rights &
			    (DARWIN_ACE_WRITE_DATA |
