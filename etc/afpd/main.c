...
static void fd_set_listening_sockets(void)
{
	...
	for (config = configs; config; config = config->next) {
		if (config->fd < 0)	/* for proxies */
			continue;
		fdset_add_fd(default_options.connections + AFP_LISTENERS +
			     FDSET_SAFETY, &fdset, &polldata, &fdset_used,
			     &fdset_size, config->fd, LISTEN_FD, config);
	}

	if (default_options.flags & OPTION_KEEPSESSIONS)
		fdset_add_fd(default_options.connections + AFP_LISTENERS +
			     FDSET_SAFETY, &fdset, &polldata, &fdset_used,
			     &fdset_size, disasociated_ipc_fd,
			     DISASOCIATED_IPC_FD, NULL);
}
...
static void fd_reset_listening_sockets(void)
{
	...
	for (config = configs; config; config = config->next) {
		if (config->fd < 0)	/* for proxies */
			continue;
		fdset_del_fd(&fdset, &polldata, &fdset_used, &fdset_size,
			     config->fd);
	}
	...
	case SIGQUIT:
		...
		case SIGTERM:
			LOG(log_note, logtype_afpd,
			    "AFP Server shutting down on SIGTERM");
			break;
		case SIGQUIT:
			if (default_options.flags & OPTION_KEEPSESSIONS) {
				LOG(log_note, logtype_afpd,
				    "AFP Server shutting down on SIGQUIT, NOT disconnecting clients");
			} else {
				LOG(log_note, logtype_afpd,
				    "AFP Server shutting down on SIGQUIT");
				sig = SIGTERM;
			}
			break;
		}
		if (server_children)
			server_child_kill(server_children, CHILD_DSIFORK,
					  sig);
...
#define WAIT_ANY (-1)
	...
	while ((pid = waitpid(WAIT_ANY, &status, WNOHANG)) > 0) {
		for (i = 0; i < server_children->nforks; i++) {
			if ((fd =
			     server_child_remove(server_children, i,
						 pid)) != -1) {
				fdset_del_fd(&fdset, &polldata,
					     &fdset_used, &fdset_size, fd);
				break;
			}
		}
		...
		if (WIFEXITED(status)) {
			...
			if (WIFSIGNALED(status))
				LOG(log_info, logtype_afpd,
				    "child[%d]: killed by signal %d", pid,
				    WTERMSIG(status));
			else
				LOG(log_info, logtype_afpd,
				    "child[%d]: died", pid);
...
static int setlimits(void)
{
	...
	if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
		LOG(log_warning, logtype_afpd,
		    "setlimits: reading current limits failed: %s",
		    strerror(errno));
		return -1;
	}
	if (rlim.rlim_cur != RLIM_INFINITY && rlim.rlim_cur < 65535) {
		...
		if (setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
			LOG(log_warning, logtype_afpd,
			    "setlimits: increasing limits failed: %s",
			    strerror(errno));
			return -1;
		}
	}
...
int main(int ac, char **av)
{
	...
	if (!(default_options.flags & OPTION_DEBUG)
	    && (daemonize(0, 0) != 0))
		exit(EXITERR_SYS);
	...
	if (!
	    (server_children =
	     server_child_alloc(default_options.connections,
				CHILD_NFORKS))) {
		LOG(log_error, logtype_afpd,
		    "main: server_child alloc: %s", strerror(errno));
		exit(EXITERR_SYS);
	}
	...
	if (sigaction(SIGXFSZ, &sv, NULL) < 0) {
		LOG(log_error, logtype_afpd, "main: sigaction: %s",
		    strerror(errno));
		exit(EXITERR_SYS);
	}
#endif
	 ...
	 * solution. */
	while (1) {
		LOG(log_maxdebug, logtype_afpd, "main: polling %i fds",
		    fdset_used);
		pthread_sigmask(SIG_UNBLOCK, &sigs, NULL);
		...
		for (int i = 0; i < fdset_used; i++) {
			if (fdset[i].
				...
				case IPC_FD:
					...
					if (ipc_server_read
						...
						if ((default_options.
							...
							LOG(log_note,
							    logtype_afpd,
							    "main: removing reattached child[%u]",
							    child->pid);
							server_child_remove
							    (server_children,
							     CHILD_DSIFORK,
				...
				case DISASOCIATED_IPC_FD:
					LOG(log_debug, logtype_afpd,
					    "main: IPC reconnect request");
					if ((recon_ipc_fd =
					     accept(disasociated_ipc_fd,
						    NULL, NULL)) == -1) {
						LOG(log_error,
						    logtype_afpd,
						    "main: accept: %s",
					...
					if ((child =
						...
						LOG(log_error,
						    logtype_afpd,
						    "main: server_child_add");
						close(recon_ipc_fd);
						break;
					}
					child->disasociated = 1;
					fdset_add_fd(default_options.
						     connections +
						     AFP_LISTENERS +
						     FDSET_SAFETY, &fdset,
						     &polldata,
