...
static void status_flags(char *data, const int notif, const int ipok,
			 ...
			 const int dirsrvcs _U_, int flags)
{
	uint16_t status;
	...
	status = AFPSRVRINFO_COPY
	    ...
	    | AFPSRVRINFO_SRVRDIR
	    | AFPSRVRINFO_SRVUTF8 | AFPSRVRINFO_EXTSLEEP;
			 ...
			 const struct afp_options *options)
{
	...
	if ((size_t) -1 ==
	    (len =
	     convert_string(options->unixcharset, options->maccharset, Obj,
			    -1, buf, sizeof(buf)))) {
		len = MIN(strlen(Obj), 31);
		*data++ = len;
		memcpy(data, Obj, len);
		LOG(log_error, logtype_afpd,
		    "Could not set servername, using fallback");
	} else {
		*data++ = len;
		memcpy(data, buf, len);
	}
...
static void status_machine(char *data)
{
	...
	if ((strlen(machine) + strlen(VERSION)) <= AFPSTATUS_MACHLEN) {
		len =
		    snprintf(buf, AFPSTATUS_MACHLEN + 1, machine, VERSION);
	} else {
		if (strlen(VERSION) > AFPSTATUS_MACHLEN) {
			len =
			    snprintf(buf, AFPSTATUS_MACHLEN + 1, VERSION);
		} else {
			(void) snprintf(buf, AFPSTATUS_MACHLEN + 1,
					machine, "");
			(void) snprintf(buf + AFPSTATUS_MACHLEN -
					strlen(VERSION),
	...
	*data++ = ((options->fqdn && dsi) ? 1 : 0) + (dsi ? 1 : 0) +
	...
	if (asp) {
		const struct sockaddr_at *ddpaddr =
		    atp_sockaddr(asp->asp_atp);

		/* ddp address */
		*data++ = 6;
		*data++ = 0x03;	/* ddp address */
		memcpy(data, &ddpaddr->sat_addr.s_net,
		       sizeof(ddpaddr->sat_addr.s_net));
		...
		memcpy(data, &ddpaddr->sat_port,
		       sizeof(ddpaddr->sat_port));
				    ...
				    const struct afp_options *options)
{
	...
	if (options->k5service && options->k5realm && options->fqdn) {
		...
		if (len > 255 || len + 2 > maxstatuslen - offset) {
			*data++ = 0;
			LOG(log_error, logtype_afpd,
			    "status: could not set directory service list, no more room");
		} else {
			*data++ = 1;	/* DirectoryNamesCount */
			*data++ = len;
			snprintf(data, len + 1, "%s/%s@%s",
				 options->k5service, options->fqdn,
				 options->k5realm);
			data += len;
			if (p)
				*p = ':';
				    ...
				    const struct afp_options *options)
{
	...
	if ((size_t) -1 ==
	    (len =
	     convert_string(options->unixcharset, CH_UTF8_MAC, Obj, -1,
			    data + sizeof(namelen),
			    maxstatuslen - offset))) {
		LOG(log_error, logtype_afpd,
		    "Could not set utf8 servername");
		 ...
		 const struct afp_options *options)
{
	...
	status_flags(status,
		     ...
		     (options->fqdn || ipok),
		     options->passwdbits,
		     (options->k5service && options->k5realm
		      && options->fqdn), options->flags);
	...
	if (options->flags & OPTION_CUSTOMICON)
		...
		status_icon(status, apple_atalk_icon,
			    sizeof(apple_atalk_icon), c);
	...
	if (statuslen < maxstatuslen)
		statuslen =
		    status_utf8servername(status, &c, dsi, options);
...
void set_signature(struct afp_options *options)
{
	...
	if (strcmp(options->signatureopt, "auto") == 0) {
		...
		LOG(log_warning, logtype_afpd,
		    "WARNING: option \"-signature host\" is obsoleted. Switching back to auto.",
		...
		LOG(log_error, logtype_afpd,
		    "ERROR: option \"-signature %s\" is not valid. Switching back to auto.",
		    options->signatureopt);
	...
	if (len <= 5) {
		LOG(log_warning, logtype_afpd,
		    "WARNING: option \"-signature %s\" is not valid. Switching back to auto.",
		    options->signatureopt);
	...
	if (len > 16) {
		LOG(log_warning, logtype_afpd,
		    "WARNING: signature user string %s is very long !",
		    usersign);
		len = 16;
	} else if (len >= 3) {
		LOG(log_info, logtype_afpd, "signature user string is %s.",
		    usersign);
	} else {
		LOG(log_warning, logtype_afpd,
		    "WARNING: signature user string %s is very short !",
		    usersign);
	}
			...
			while (fgets(buf, sizeof(buf), fp) != NULL) {
				...
				if (16 ==
				    sscanf(p,
					   ...
					   &options->signature[8],
					   &options->signature[9],
					   &options->signature[10],
					   &options->signature[11],
					   &options->signature[12],
					   &options->signature[13],
					   &options->signature[14],
					   &options->signature[15]
				    )) {
					fclose(fp);
