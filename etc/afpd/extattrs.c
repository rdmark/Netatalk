...
*/
int afp_listextattr(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
	...
	if ((maxreply == 0) || (buf_valid == 0)) {
		...
		if (NULL == (vol = getvolbyvid(vid))) {
			LOG(log_debug, logtype_afpd,
			    "afp_listextattr: getvolbyvid error: %s",
			    strerror(errno));
		...
		if (NULL == (dir = dirlookup(vol, did))) {
			LOG(log_debug, logtype_afpd,
			    "afp_listextattr: dirlookup error: %s",
			    strerror(errno));
			return afp_errno;
		}
		...
		if (ad_metadata(uname, adflags, adp) < 0) {
			...
			default:
				LOG(log_error, logtype_afpd,
				    "afp_listextattr(%s): error getting metadata: %s",
				    uname, strerror(errno));
				return AFPERR_MISC;
			}
		}
		...
		if (adp) {
			...
			if (memcmp(FinderInfo, emptyFinderInfo, 32) != 0) {
				...
				LOG(log_debug7, logtype_afpd,
				    "afp_listextattr(%s): sending com.apple.FinderInfo",
				    uname);
			}

			/* Now check for Ressource fork and add virtual EA "com.apple.ResourceFork" if size > 0 */
			LOG(log_debug7, logtype_afpd,
			    "afp_listextattr(%s): Ressourcefork size: %llu",
			    uname, adp->ad_rlen);

			if (adp->ad_rlen > 0) {
				LOG(log_debug7, logtype_afpd,
				    "afp_listextattr(%s): sending com.apple.RessourceFork.",
				    uname);
				strcpy(attrnamebuf + attrbuflen,
				       ea_resourcefork);
		...
		ret =
		    vol->vfs->vfs_ea_list(vol, attrnamebuf, &attrbuflen,
					  uname, oflag);
		...
		case AFPERR_BADTYPE:
			...
			LOG(log_debug, logtype_afpd,
			    "afp_listextattr(%s): encountered symlink with kXAttrNoFollow",
			    uname);
			attrbuflen = 0;
			buf_valid = 0;
			ret = AFP_OK;
...
int afp_getextattr(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		   char *rbuf, size_t *rbuflen)
{
	int ret, oflag = 0;
	...
	*rbuflen = 0;
	...
	if (NULL == (vol = getvolbyvid(vid))) {
		LOG(log_debug, logtype_afpd,
		    "afp_getextattr: getvolbyvid error: %s",
		    strerror(errno));
		return AFPERR_ACCESS;
	}
	...
	if (NULL == (dir = dirlookup(vol, did))) {
		LOG(log_debug, logtype_afpd,
		    "afp_getextattr: dirlookup error: %s",
		    strerror(errno));
		return afp_errno;
	}
	...
	if (NULL == (s_path = cname(vol, dir, &ibuf))) {
		LOG(log_debug, logtype_afpd,
		    "afp_getextattr: cname error: %s", strerror(errno));
		return AFPERR_NOOBJ;
	}
	...
	LOG(log_debug, logtype_afpd, "afp_getextattr(%s): EA: %s",
	    s_path->u_name, to_stringz(ibuf, attrnamelen));

	/* Convert EA name in utf8 to unix charset */
	if (0 >=
	    convert_string(CH_UTF8_MAC, obj->options.unixcharset, ibuf,
	...
	*rbuflen += sizeof(bitmap);
	...
	if (maxreply == 0)
		ret =
		    vol->vfs->vfs_ea_getsize(vol, rbuf, rbuflen,
					     s_path->u_name, oflag,
					     attruname);
	else
		ret =
		    vol->vfs->vfs_ea_getcontent(vol, rbuf, rbuflen,
						s_path->u_name, oflag,
						attruname, maxreply);
		   ...
		   char *rbuf _U_, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if (NULL == (vol = getvolbyvid(vid))) {
		LOG(log_debug, logtype_afpd,
		    "afp_setextattr: getvolbyvid error: %s",
		    strerror(errno));
		return AFPERR_ACCESS;
	}
	...
	if (NULL == (dir = dirlookup(vol, did))) {
		LOG(log_debug, logtype_afpd,
		    "afp_setextattr: dirlookup error: %s",
		    strerror(errno));
		return afp_errno;
	}
	...
	if (NULL == (s_path = cname(vol, dir, &ibuf))) {
		LOG(log_debug, logtype_afpd,
		    "afp_setextattr: cname error: %s", strerror(errno));
		return AFPERR_NOOBJ;
	}
	...
	/* Convert EA name in utf8 to unix charset */
	if (0 >=
	    convert_string(CH_UTF8_MAC, obj->options.unixcharset,
			   attrmname, attrnamelen, attruname, 256))
		return AFPERR_MISC;
	...
	LOG(log_debug, logtype_afpd,
	    "afp_setextattr(%s): EA: %s, size: %u", s_path->u_name,
	    to_stringz(attrmname, attrnamelen), attrsize);

	ret =
	    vol->vfs->vfs_ea_set(vol, s_path->u_name, attruname, ibuf,
				 attrsize, oflag);
		   ...
		   char *rbuf _U_, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if (NULL == (vol = getvolbyvid(vid))) {
		LOG(log_debug, logtype_afpd,
		    "afp_remextattr: getvolbyvid error: %s",
		    strerror(errno));
		return AFPERR_ACCESS;
	}
	...
	if (NULL == (dir = dirlookup(vol, did))) {
		LOG(log_debug, logtype_afpd,
		    "afp_remextattr: dirlookup error: %s",
		    strerror(errno));
		return afp_errno;
	}
	...
	if (0 >=
	    (convert_string
	     (CH_UTF8_MAC, obj->options.unixcharset, ibuf, attrnamelen,
	      attruname, 256)))
		return AFPERR_MISC;

	LOG(log_debug, logtype_afpd, "afp_remextattr(%s): EA: %s",
	    s_path->u_name, to_stringz(ibuf, attrnamelen));

	ret =
	    vol->vfs->vfs_ea_remove(vol, s_path->u_name, attruname, oflag);
