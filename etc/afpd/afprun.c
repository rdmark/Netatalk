...
static int setup_out_fd(void)
{
	...
	if (fd == -1) {
		LOG(log_error, logtype_afpd,
		    "setup_out_fd: Failed to create file %s. (%s)", path,
		    strerror(errno));
		return -1;
	}
...
int afprun(int root, char *cmd, int *outfd)
{
	...
	LOG(log_debug, logtype_afpd, "running %s as user %d", cmd,
	    root ? 0 : uid);
	...
	if ((pid = fork()) < 0) {
		LOG(log_error, logtype_afpd,
		    "afprun: fork failed with error %s", strerror(errno));
		if (outfd) {
			close(*outfd);
			*outfd = -1;
		}
	...
	if (chdir("/") < 0) {
		LOG(log_error, logtype_afpd,
		    "afprun: can't change directory to \"/\" %s",
		    strerror(errno));
		exit(83);
	}
