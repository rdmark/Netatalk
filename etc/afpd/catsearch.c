				...
				struct adouble *adp)
{
	...
	if (ad_metadata(path->u_name, ((isdir) ? ADFLAGS_DIR : 0), adp) <
	    0) {
...
static struct finderinfo *unpack_buffer(struct finderinfo *finfo,
					char *buffer)
{
	memcpy(&finfo->f_type, buffer + FINDERINFO_FRTYPEOFF,
	       sizeof(finfo->f_type));
	memcpy(&finfo->creator, buffer + FINDERINFO_FRCREATOFF,
	       sizeof(finfo->creator));
	memcpy(&finfo->attrs, buffer + FINDERINFO_FRFLAGOFF,
	       sizeof(finfo->attrs));
	memcpy(&finfo->label, buffer + FINDERINFO_FRFLAGOFF,
	       sizeof(finfo->label));
...
/* -------------------- */
static struct finderinfo *unpack_finderinfo(struct vol *vol,
					    struct path *path,
					    struct adouble **adp,
...
static int crit_check(struct vol *vol, struct path *path)
{
	...
	if (S_ISDIR(path->st.st_mode)) {
		...
		if (!
		    (path->m_name =
		     utompath(vol, path->u_name, 0, utf8_encoding()))) {
			/*retry with the right id */

			cnid_t id;

			adp = adl_lkup(vol, path, adp);
			id = get_id(vol, adp, &path->st,
				    path->d_dir->d_did, path->u_name,
				    strlen(path->u_name));
			if (!id) {
				/* FIXME */
				return 0;
	...
	if ((c1.rbitmap & (1U << DIRPBIT_LNAME))) {
		if ((size_t) (-1) ==
		    (len =
		     convert_string(vol->v_maccharset, CH_UCS2,
		...
		if ((c1.rbitmap & (1U << CATPBIT_PARTIAL))) {
			if (strcasestr_w
			    ((ucs2_t *) convbuf,
			     (ucs2_t *) c1.lname) == NULL)
				goto crit_check_ret;
		} else
		    if (strcasecmp_w
			((ucs2_t *) convbuf, (ucs2_t *) c1.lname) != 0)
			goto crit_check_ret;
	}
	...
	if ((c1.rbitmap & (1U << FILPBIT_PDINFO))) {
		if ((size_t) (-1) ==
		    (len =
		     convert_charset(CH_UTF8_MAC, CH_UCS2, CH_UTF8,
				     path->m_name, strlen(path->m_name),
				     convbuf, 512, &flags))) {
			goto crit_check_ret;
		}
		...
		if (c1.rbitmap & (1U << CATPBIT_PARTIAL)) {
			if (strcasestr_w
			    ((ucs2_t *) convbuf,
			     (ucs2_t *) c1.utf8name) == NULL)
				goto crit_check_ret;
		} else
		    if (strcasecmp_w
			((ucs2_t *) convbuf, (ucs2_t *) c1.utf8name) != 0)
			goto crit_check_ret;
	}
	...
	if ((c1.rbitmap & (1U << DIRPBIT_MDATE))) {
		if (path->st.st_mtime < c1.mdate
		    || path->st.st_mtime > c2.mdate)
			goto crit_check_ret;
	}
	...
	if ((c1.rbitmap & (1U << DIRPBIT_FINFO)) && c2.finfo.f_type != 0) {
		finfo =
		    unpack_finderinfo(vol, path, &adp, &finderinfo, islnk);
	...
	if ((c1.rbitmap & (1U << DIRPBIT_FINFO)) && c2.finfo.creator != 0) {
		if (!finfo) {
			finfo =
			    unpack_finderinfo(vol, path, &adp, &finderinfo,
					      islnk);
		}
...
/* ------------------------------ */
static int rslt_add(struct vol *vol, struct path *path, char **buf,
		    int ext)
{
	...
	*p++ = isdir ? FILDIRBIT_ISDIR : FILDIRBIT_ISFILE;	/* IsDir ? */
	...
	if (isdir) {
		ret =
		    getdirparams(vol, c1.dbitmap, path, path->d_dir, p,
				 &tbuf);
	} else {
		/* FIXME slow if we need the file ID, we already know it, done ? */
		ret =
		    getfilparams(vol, c1.fbitmap, path, path->d_dir, p,
				 &tbuf);
	}
...
static int catsearch(struct vol *vol,
		     ...
		     int rmatches,
		     uint32_t * pos,
		     char *rbuf, uint32_t * nrecs, int *rsize, int ext)
{
	...
	while ((cidx = reducestack()) != -1) {
		if ((currentdir =
		     dirlookup(vol, dstack[cidx].ds_did)) == NULL) {
			result = AFPERR_MISC;
			goto catsearch_end;
		}
		LOG(log_debug, logtype_afpd,
		    "catsearch: current struct dir: \"%s\"",
		    cfrombstr(currentdir->d_fullpath));
		...
		while ((entry = readdir(dirpos)) != NULL) {
			...
			case S_IFDIR:
				...
				path.m_name =
				    cfrombstr(path.d_dir->d_m_name);

				if (addstack(path.u_name, path.d_dir, cidx)
				    == -1) {
					result = AFPERR_MISC;
					goto catsearch_end;
				}
			...
			char *rbuf, uint32_t * nrecs, int *rsize, int ext)
{
	...
	LOG(log_debug, logtype_afpd,
	    "catsearch_db(req pos: %u): {pos: %u, name: %s}", *pos,
	    cur_pos, uname);
	...
	if (cur_pos == 0 || *pos == 0) {
		if (convert_charset(vol->v_volcharset,
				    ...
				    strlen(uname),
				    buffer,
				    MAXPATHLEN, &flags) == (size_t) -1) {
			LOG(log_error, logtype_afpd,
			    "catsearch_db: conversion error");
			result = AFPERR_MISC;
			goto catsearch_end;
		}
	...
	while (cur_pos < num_matches) {
		...
		memcpy(&cnid, resbuf + cur_pos * sizeof(cnid_t),
		       sizeof(cnid_t));
		did = cnid;

		if ((name =
		     cnid_resolve(vol->v_cdb, &did, resolvebuf,
				  12 + MAXPATHLEN + 1)) == NULL)
			goto next;
		LOG(log_debug, logtype_afpd,
		    "catsearch_db: {pos: %u, name:%s, cnid: %u}", cur_pos,
		    name, ntohl(cnid));
		...
		LOG(log_maxdebug, logtype_afpd,
		    "catsearch_db: dir: %s, cwd: %s, name: %s",
		...
		if ((ccr & 1)) {
			LOG(log_debug, logtype_afpd,
			    "catsearch_db: match: %s/%s", getcwdpath(),
			    path.u_name);
	...
	*rsize = rrbuf - rbuf;
	LOG(log_debug, logtype_afpd,
	    "catsearch_db(req pos: %u): {pos: %u}", *pos, cur_pos);
	return result;
}
			 ...
			 char *rbuf, size_t *rbuflen, int ext)
{
	...
	*rbuflen = 0;
	...
	if (c1.rbitmap & (1U << FILPBIT_LNAME)) {
		...
		memcpy(tmppath, bspec1 + spec1[1] + 1,
		       (bspec1 + spec1[1])[0]);
		tmppath[(bspec1 + spec1[1])[0]] = 0;
		len =
		    convert_string(vol->v_maccharset, CH_UCS2, tmppath, -1,
				   c1.lname, sizeof(c1.lname));
		if (len == (size_t) (-1))
			return AFPERR_PARAM;
	}
	/* UTF8 Name */
	if (c1.rbitmap & (1U << FILPBIT_PDINFO)) {
		...
		len =
		    convert_charset(CH_UTF8_MAC, CH_UCS2, CH_UTF8,
				    c1.utf8name, namelen, c1.utf8name, 512,
				    &flags);
		if (len == (size_t) (-1))
			return AFPERR_PARAM;
	}
	...
	*rbuflen = 24;
	if ((c1.rbitmap & (1U << FILPBIT_PDINFO))
		...
		ret =
		    catsearch_db(vol, vol->v_root, uname, rmatches,
				 &catpos[0], rbuf + 24, &nrecs, &rsize,
				 ext);
	else
		/* perform a slow filesystem tree search */
		ret =
		    catsearch(vol, vol->v_root, rmatches, &catpos[0],
