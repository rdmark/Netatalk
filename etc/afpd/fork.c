...
static int getforkparams(struct ofork *ofork, u_int16_t bitmap, char *buf,
			 size_t *buflen)
{
	struct path path;
	struct stat *st;
	...
	if (NULL ==
	    (path.u_name =
	     mtoupath(vol, of_name(ofork), dir->d_did, utf8_encoding()))) {
		return (AFPERR_MISC);
	}
...
/* -------------------------
*/
static int setforkmode(struct adouble *adp, int eid, int ofrefnum,
		       off_t what)
{
	return ad_lock(adp, eid, ADLOCK_RD | ADLOCK_FILELOCK, what, 1,
		       ofrefnum);
}
...
/* -------------------------
*/
static int fork_setmode(struct adouble *adp, int eid, int access,
			int ofrefnum)
{
	int ret;
	int readset;
	...
	if (!
		...
		return setforkmode(adp, eid, ofrefnum,
				   AD_FILELOCK_OPEN_NONE);
	}

	if ((access & (OPENACC_RD | OPENACC_DRD))) {
		if ((readset =
		     getforkmode(adp, eid, AD_FILELOCK_OPEN_RD)) < 0)
			return readset;
		if ((denyreadset =
		     getforkmode(adp, eid, AD_FILELOCK_DENY_RD)) < 0)
			return denyreadset;
	...
	if ((access & (OPENACC_WR | OPENACC_DWR))) {
		if ((writeset =
		     getforkmode(adp, eid, AD_FILELOCK_OPEN_WR)) < 0)
			return writeset;
		if ((denywriteset =
		     getforkmode(adp, eid, AD_FILELOCK_DENY_WR)) < 0)
			return denywriteset;
...
/* ----------------------- */
int afp_openfork(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		 char *rbuf, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	default:
		LOG(log_error, logtype_afpd,
		    "afp_openfork(%s): ad_open: %s", s_path->m_name,
		    strerror(errno));
		return AFPERR_PARAM;
	}
	...
	if (access & OPENACC_WR) {
		...
		if (ad_open(upath, adflags, O_RDONLY, 0, ofork->of_ad) < 0) {
			...
			default:
				LOG(log_error, logtype_afpd,
				    "afp_openfork('%s/%s'): ad_open: errno: %i (%s)",
				    getcwdpath, s_path->m_name, errno,
				    strerror(errno));
				goto openfork_err;
				break;
			}
	...
	if ((adflags & ADFLAGS_HF)
	    && (ad_get_HF_flags(ofork->of_ad) & O_CREAT)) {
	...
	if ((ret =
	     getforkparams(ofork, bitmap, rbuf + 2 * sizeof(u_int16_t),
			   &buflen)) != AFP_OK) {
	...
	*rbuflen = buflen + 2 * sizeof(u_int16_t);
	...
	if (ad_meta_fileno(ofork->of_ad) != -1) {	/* META */
		...
		if ((bshort & htons(ATTRBIT_NOWRITE))
		    && (access & OPENACC_WR)) {
			ad_close(ofork->of_ad, adflags);
			of_dealloc(ofork);
			ofrefnum = 0;
	...
	if ((eid == ADEID_DFORK) || (ad_meta_fileno(ofork->of_ad) != -1)) {	/* META */
		...
		if (ret < 0) {
			...
			default:
				*rbuflen = 0;
				LOG(log_error, logtype_afpd,
				    "afp_openfork(%s): ad_lock: %s",
				    s_path->m_name, strerror(ret));
				return (AFPERR_PARAM);
			}
		}
		if ((access & OPENACC_WR))
...
int afp_setforkparams(AFPObj * obj _U_, char *ibuf, size_t ibuflen,
		     ...
		     char *rbuf, size_t *rbuflen, int is64)
{
	...
	*rbuflen = 0;
	...
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_bytelock: of_find(%d) could not locate fork",
		    ofrefnum);
		return (AFPERR_PARAM);
	}
	...
	if (ad_lock(ofork->of_ad, eid, lockop, offset, length,
		...
		case EACCES:
		case EAGAIN:
			return UNLOCKBIT(flags) ? AFPERR_NORANGE :
			    AFPERR_LOCK;
			break;
		case ENOLCK:
			return AFPERR_NLOCK;
...
static int crlf(struct ofork *of)
{
	...
	if (ad_meta_fileno(of->of_ad) == -1 || !memcmp(ufinderi, ad_entry(of->of_ad, ADEID_FINDERI), 8)) {	/* META */
		...
		if (!(em = getextmap(of_name(of)))
		    || memcmp("TEXT", em->em_type, sizeof(em->em_type))) {
			return 0;
		}
		/* file type is TEXT */
		return 1;
...
static ssize_t read_file(struct ofork *ofork, int eid,
			 off_t offset, u_int8_t nlmask,
			 u_int8_t nlchar, char *rbuf,
			 size_t *rbuflen, const int xlate)
{
	...
	if (cc < 0) {
		LOG(log_error, logtype_afpd, "afp_read(%s): ad_read: %s",
		     ...
		     char *rbuf, size_t *rbuflen, int is64)
{
	...
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_read: of_find(%d) could not locate fork",
		    ofrefnum);
		err = AFPERR_PARAM;
		goto afp_read_err;
	}
	...
	if (ofork->of_flags & AFPFORK_DATA) {
		...
		xlate =
		    (ofork->of_vol->
		     v_flags & AFPVOL_CRLF) ? crlf(ofork) : 0;
	} else if (ofork->of_flags & AFPFORK_RSRC) {
		eid = ADEID_RFORK;
	...
	LOG(log_debug, logtype_afpd,
	    "afp_read(name: \"%s\", offset: %jd, reqcount: %jd)",
	    of_name(ofork), (intmax_t) offset, (intmax_t) reqcount);

	savereqcount = reqcount;
	saveoff = offset;
	if (ad_tmplock
	    (ofork->of_ad, eid, ADLOCK_RD, saveoff, savereqcount,
	     ofork->of_refnum) < 0) {
		err = AFPERR_LOCK;
		goto afp_read_err;
	}
	...
	*rbuflen = MIN(reqcount, *rbuflen);
	LOG(log_debug, logtype_afpd,
	    "afp_read(name: \"%s\", offset: %jd, reqcount: %jd): reading %jd bytes from file",
	    of_name(ofork), (intmax_t) offset, (intmax_t) reqcount,
	    (intmax_t) * rbuflen);
	err =
	    read_file(ofork, eid, offset, nlmask, nlchar, rbuf, rbuflen,
		      xlate);
	if (err < 0)
		goto afp_read_done;
	LOG(log_debug, logtype_afpd,
	...
	if ((obj->proto == AFPPROTO_DSI) && (*rbuflen < reqcount)
		 ...
		 * horrible happened. */
		if ((cc =
		     dsi_readinit(dsi, rbuf, *rbuflen, reqcount, err)) < 0)
			goto afp_read_exit;
		*rbuflen = cc;
		...
		if (!(xlate || obj->options.flags & OPTION_DEBUG)) {
			...
			fd = ad_readfile_init(ofork->of_ad, eid, &offset,
					      0);

			if (dsi_stream_read_file
			    (dsi, fd, offset, dsi->datasize) < 0) {
				if (errno == EINVAL || errno == ENOSYS)
					goto afp_read_loop;
				else {
					LOG(log_error, logtype_afpd,
					    "afp_read(%s): ad_readfile: %s",
					    of_name(ofork),
					    strerror(errno));
					goto afp_read_exit;
				}
		...
		/* fill up our buffer. */
		while (*rbuflen > 0) {
			cc = read_file(ofork, eid, offset, nlmask, nlchar,
				       rbuf, rbuflen, xlate);
			if (cc < 0)
				goto afp_read_exit;
		...
		goto afp_read_done;
	      ...
	      afp_read_exit:
		LOG(log_error, logtype_afpd, "afp_read(%s): %s",
		    of_name(ofork), strerror(errno));
		dsi_readdone(dsi);
		ad_tmplock(ofork->of_ad, eid, ADLOCK_CLR, saveoff,
			   savereqcount, ofork->of_refnum);
...
/* ---------------------- */
int afp_flush(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
	      char *rbuf _U_, size_t *rbuflen)
{
	struct vol *vol;
	u_int16_t vid;
		  ...
		  char *rbuf _U_, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_flushfork: of_find(%d) could not locate fork",
		    ofrefnum);
		return (AFPERR_PARAM);
	}

	if (flushfork(ofork) < 0) {
		LOG(log_error, logtype_afpd, "afp_flushfork(%s): %s",
		    of_name(ofork), strerror(errno));
	}
		 ...
		 char *rbuf _U_, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afpd_syncfork: of_find(%d) could not locate fork",
		    ofrefnum);
		return (AFPERR_PARAM);
	}

	if (flushfork(ofork) < 0) {
		LOG(log_error, logtype_afpd, "flushfork(%s): %s",
		    of_name(ofork), strerror(errno));
		return AFPERR_MISC;
	}
...
int flushfork(struct ofork *ofork)
{
	...
	if (ad_data_fileno(ofork->of_ad) != -1 &&
		...
		LOG(log_error, logtype_afpd, "flushfork(%s): dfile(%d) %s",
		    of_name(ofork), ad_data_fileno(ofork->of_ad),
		    strerror(errno));
		err = -1;
	}
	...
	if (ad_reso_fileno(ofork->of_ad) != -1 &&	/* HF */
		...
		if ((ofork->of_flags & AFPFORK_DIRTY)
			...
			ad_setdate(ofork->of_ad,
				   AD_DATE_MODIFY | AD_DATE_UNIX,
				   tv.tv_sec);
		...
		if (err < 0)
			LOG(log_error, logtype_afpd,
			    "flushfork(%s): hfile(%d) %s", of_name(ofork),
			    ad_reso_fileno(ofork->of_ad), strerror(errno));
	}

	return (err);
}

int afp_closefork(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		  char *rbuf _U_, size_t *rbuflen)
{
	struct ofork *ofork;
	u_int16_t ofrefnum;
	...
	*rbuflen = 0;
	...
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_closefork: of_find(%d) could not locate fork",
		    ofrefnum);
		return (AFPERR_PARAM);
	}
	if (of_closefork(ofork) < 0) {
		LOG(log_error, logtype_afpd,
		    "afp_closefork(%s): of_closefork: %s", of_name(ofork),
		    strerror(errno));
		return (AFPERR_PARAM);
	}
			  ...
			  size_t rbuflen, const int xlate)
{
	...
	if ((cc = ad_write(ofork->of_ad, eid, offset, 0,
		...
		case ENOSPC:
			LOG(log_error, logtype_afpd,
			    "write_file: DISK FULL");
			return (AFPERR_DFULL);
		case EACCES:
			return AFPERR_ACCESS;
		default:
			LOG(log_error, logtype_afpd,
			    "afp_write(%s): ad_write: %s", of_name(ofork),
			    strerror(errno));
			return (AFPERR_PARAM);
		}
	}
		      ...
		      char *rbuf, size_t *rbuflen, int is64)
{
	...
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_write: of_find(%d) could not locate fork",
		    ofrefnum);
		err = AFPERR_PARAM;
		goto afp_write_err;
	}
	...
	newsize =
	    ((offset + reqcount) >
	     oldsize) ? (offset + reqcount) : oldsize;
	...
	case AFPPROTO_ASP:
		if (asp_wrtcont(obj->handle, rbuf, rbuflen) < 0) {
			*rbuflen = 0;
			LOG(log_error, logtype_afpd,
			    "afp_write: asp_wrtcont: %s", strerror(errno));
			return (AFPERR_PARAM);
		}
	...
	case AFPPROTO_DSI:
		{
			...
			if (!cc
			    || (cc =
				write_file(ofork, eid, offset, rbuf, cc,
					   xlate)) < 0) {
				dsi_writeflush(dsi);
				*rbuflen = 0;
				ad_tmplock(ofork->of_ad, eid, ADLOCK_CLR,
					   saveoff, reqcount,
					   ofork->of_refnum);
				return cc;
			}
			...
			if (!(xlate || obj->options.flags & OPTION_DEBUG)) {
				if ((cc =
					...
					default:
						LOG(log_error,
						    logtype_afpd,
						    "afp_write: ad_writefile: %s",
						    strerror(errno));
						goto afp_write_loop;
					}
					dsi_writeflush(dsi);
					*rbuflen = 0;
					ad_tmplock(ofork->of_ad, eid,
						   ADLOCK_CLR, saveoff,
						   reqcount,
						   ofork->of_refnum);
					return cc;
			...
			while ((cc = dsi_write(dsi, rbuf, *rbuflen))) {
				if ((cc =
				     write_file(ofork, eid, offset, rbuf,
						cc, xlate)) < 0) {
					dsi_writeflush(dsi);
					*rbuflen = 0;
					ad_tmplock(ofork->of_ad, eid,
...
/* ---------------------------- */
int afp_getforkparams(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		      char *rbuf, size_t *rbuflen)
{
	struct ofork *ofork;
	int ret;
	...
	*rbuflen = 0;
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_getforkparams: of_find(%d) could not locate fork",
		    ofrefnum);
		return (AFPERR_PARAM);
	}
