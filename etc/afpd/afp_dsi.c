...
static void afp_dsi_close(AFPObj * obj)
{
	...
	if (geteuid() != obj->uid) {
		if (seteuid(obj->uid) < 0) {
			LOG(log_error, logtype_afpd,
			    "can't seteuid(%u) back %s: uid: %u, euid: %u",
			    obj->uid, strerror(errno), getuid(),
			    geteuid());
			exit(EXITERR_SYS);
		}
	}
	...
	LOG(log_note, logtype_afpd,
	    "AFP statistics: %.2f KB read, %.2f KB written",
...
static void afp_dsi_die(int sig)
{
	...
	if (dsi->flags & DSI_DISCONNECTED) {
		LOG(log_note, logtype_afpd,
		    "Disconnected session terminating");
		exit(0);
	}
...
static void ipc_reconnect_handler(int sig _U_)
{
	if (reconnect_ipc(AFPobj) != 0) {
		LOG(log_error, logtype_afpd,
		    "ipc_reconnect_handler: failed IPC reconnect");
		afp_dsi_close(AFPobj);
		exit(EXITERR_SYS);
	}

	if (ipc_child_write
	    (AFPobj->ipc_fd, IPC_GETSESSION, AFPobj->sinfo.clientid_len,
	     AFPobj->sinfo.clientid) != 0) {
		LOG(log_error, logtype_afpd,
		    "ipc_reconnect_handler: failed IPC ID resend");
		afp_dsi_close(AFPobj);
		exit(EXITERR_SYS);
	}
	LOG(log_note, logtype_afpd,
	    "ipc_reconnect_handler: IPC reconnect done");
}
...
static void afp_dsi_transfer_session(int sig _U_)
{
	...
	LOG(log_debug, logtype_afpd,
	    "afp_dsi_transfer_session: received socket fd: %i", socket);
	...
	if (!dsi_cmdreply(dsi, AFP_OK)) {
		LOG(log_error, logtype_afpd, "dsi_cmdreply: %s",
		    strerror(errno));
		afp_dsi_close(AFPobj);
		exit(EXITERR_CLNT);
	}
...
static void afp_dsi_timedown(int sig _U_)
{
	...
	if (dsi_attention
	    (AFPobj->handle,
	     AFPATTN_SHUTDOWN | AFPATTN_NORECONNECT | AFPATTN_MESG |
	     AFPATTN_TIME(5)) < 0) {
		DSI *dsi = (DSI *) AFPobj->handle;
		dsi->down_request = 1;
	}
	...
	if (setitimer(ITIMER_REAL, &it, NULL) < 0) {
		LOG(log_error, logtype_afpd, "afp_timedown: setitimer: %s",
		    strerror(errno));
	...
	if (sigaction(SIGALRM, &sv, NULL) < 0) {
		LOG(log_error, logtype_afpd, "afp_timedown: sigaction: %s",
		    strerror(errno));
		afp_dsi_die(EXITERR_SYS);
	}
	...
	if (sigaction(SIGUSR1, &sv, NULL) < 0) {
		LOG(log_error, logtype_afpd,
		    "afp_timedown: sigaction SIGHUP: %s", strerror(errno));
		afp_dsi_die(EXITERR_SYS);
	}
}
...
static void afp_dsi_getmesg(int sig _U_)
{
	...
	dsi->msg_request = 1;
	if (dsi_attention(AFPobj->handle, AFPATTN_MESG | AFPATTN_TIME(5)) <
	    0)
		dsi->msg_request = 2;
}
...
static void alarm_handler(int sig _U_)
{
	...
	LOG(log_maxdebug, logtype_afpd,
	    "alarm: tickles: %u, flags: %s|%s|%s|%s|%s|%s|%s|%s|%s",
	    dsi->tickle, (dsi->flags & DSI_DATA) ? "DSI_DATA" : "-",
	...
	if (dsi->flags & DSI_DISCONNECTED) {
		...
		if (dsi->tickle > AFPobj->options.disconnected) {
			LOG(log_error, logtype_afpd,
			    "afp_alarm: reconnect timer expired, goodbye");
			afp_dsi_die(EXITERR_CLNT);
		}
		return;
	}
	...
	if (err <= 0) {
		...
		LOG(log_error, logtype_afpd,
		    "afp_alarm: connection problem, entering disconnected state");
		if (dsi_disconnect(dsi) != 0)
			afp_dsi_die(EXITERR_CLNT);
	}
}
...
static void pending_request(DSI * dsi)
{
	...
	if (dsi->down_request) {
		dsi->down_request = 0;
		dsi_attention(AFPobj->handle,
			      AFPATTN_SHUTDOWN | AFPATTN_NORECONNECT |
			      AFPATTN_MESG | AFPATTN_TIME(5));
	}
}
...
void afp_over_dsi_sighandlers(AFPObj * obj)
{
	...
	if (sigaction(SIGHUP, &action, NULL) < 0) {
		LOG(log_error, logtype_afpd, "afp_over_dsi: sigaction: %s",
		    strerror(errno));
		afp_dsi_die(EXITERR_SYS);
	}
...
void afp_over_dsi(AFPObj * obj)
{
	...
	if (obj->options.tcp_rcvbuf) {
		if (setsockopt(dsi->socket,
			...
			LOG(log_error, logtype_dsi,
			    "afp_over_dsi: setsockopt(SO_RCVBUF): %s",
			    strerror(errno));
		}
	}
	if (obj->options.tcp_sndbuf) {
		if (setsockopt(dsi->socket,
			...
			LOG(log_error, logtype_dsi,
			    "afp_over_dsi: setsockopt(SO_SNDBUF): %s",
			    strerror(errno));
		}
	}
	...
	while (1) {
		...
		if (cmd == 0) {
			...
			if (dsi->flags & DSI_GOT_ECONNRESET) {
				LOG(log_note, logtype_afpd,
				    "afp_over_dsi: client connection reset");
				afp_dsi_close(obj);
				exit(0);
			}
#endif
		...
		if (debug_request) {
			...
			if (debugging) {
				...
				debugging = 1;
				sprintf(logstr,
					"default log_maxdebug /tmp/afpd.%u.XXXXXX",
					getpid());
				setuplog(logstr);
			}
		}
		...
		case DSIFUNC_CMD:
			...
			LOG(log_debug, logtype_dsi, "DSI request ID: %u",
			    dsi->clientID);
		...
		default:
			LOG(log_info, logtype_afpd,
			    "afp_dsi: spurious command %d", cmd);
