...
static void afp_authprint_remove(AFPObj * obj)
{
	...
	if (stat(addr_filename, &cap_st) == 0) {
		if (S_ISREG(cap_st.st_mode)) {
			...
			if ((len =
			     read(capfd, addr_filename_buff, 256)) > 0) {
				int file_pid;
				char *p_filepid;
				addr_filename_buff[len] = 0;
				if ((p_filepid =
				     strrchr(addr_filename_buff,
					     ':')) != NULL) {
					*p_filepid = '\0';
					p_filepid++;
					file_pid = atoi(p_filepid);
					if (file_pid == (int) getpid()) {
						if (unlink(addr_filename)
						    == 0) {
							LOG(log_info,
							    logtype_afpd,
							    "removed %s",
			...
			LOG(log_info, logtype_afpd,
			    "%s is not a regular file", addr_filename);
		}
	} else {
...
static void afp_asp_die(const int sig)
{
	...
	if (asp_shutdown(asp) < 0) {
		LOG(log_error, logtype_afpd, "afp_die: asp_shutdown: %s",
		    strerror(errno));
	}
...
static void afp_asp_timedown(int sig _U_)
{
	...
	if (sigaction(SIGUSR1, &sv, NULL) < 0) {
		LOG(log_error, logtype_afpd,
		    "afp_timedown: sigaction SIGUSR1: %s",
		    strerror(errno));
		afp_asp_die(EXITERR_SYS);
	}
}
...
void afp_over_asp(AFPObj * obj)
{
	...
	if (sigaction(SIGHUP, &action, NULL) < 0) {
		LOG(log_error, logtype_afpd, "afp_over_asp: sigaction: %s",
		    strerror(errno));
		afp_asp_die(EXITERR_SYS);
	}
	...
	if (dircache_init(obj->options.dircachesize) != 0) {
		LOG(log_error, logtype_afpd,
		    "afp_over_asp: dircache_init error");
		afp_asp_die(EXITERR_SYS);
	}
