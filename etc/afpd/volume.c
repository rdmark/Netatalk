...
static int readvolfile(AFPObj * obj, struct afp_volume_name *p1, char *p2,
		       int user, struct passwd *pwent);
	...
	if (save) {
		for (i = 0; i < VOLOPT_MAX; i++) {
			if (options[i].c_value
			    && (options[i].c_value != save[i].c_value))
...
static char *volxlate(AFPObj * obj,
		      ...
		      size_t destlen,
		      char *src,
		      struct passwd *pwd, char *path, char *volname)
{
	char *p, *r;
	const char *q;
	int len;
...
/* -------------------- */
static void setoption(struct vol_option *options, struct vol_option *save,
		      int opt, const char *val)
{
	if (options[opt].c_value
	    && (!save || options[opt].c_value != save[opt].c_value))
...
static void volset(struct vol_option *options, struct vol_option *save,
		   char *volname, int vlen, const char *tmp)
{
	char *val;
	...
	if (optionok(tmp, "allow:", val)) {
		...
		LOG(log_error, logtype_afpd,
		    "The old codepage system has been removed. Please make sure to read the documentation !!!!");
		...
		if (strcasecmp(val + 1, "tolower") == 0)
			...
			options[VOLOPT_CASEFOLD].i_value =
			    AFPVOL_UUPPERMLOWER;
		...
		while (p) {
			if (strcasecmp(p, "prodos") == 0)
				...
				options[VOLOPT_FLAGS].i_value |=
				    AFPVOL_MSWINDOWS | AFPVOL_USEDOTS;
				...
				options[VOLOPT_FLAGS].i_value |=
				    AFPVOL_USEDOTS | AFPVOL_INV_DOTS;
		...
		LOG(log_debug, logtype_afpd,
		    "CNID Server for volume '%s': %s:%s", volname, val + 1,
		    p ? p + 1 : Cnid_port);
		...
		options[VOLOPT_UMASK].i_value =
		    (int) strtol(val + 1, NULL, 8);
	} else if (optionok(tmp, "dperm:", val)) {
		options[VOLOPT_DPERM].i_value =
		    (int) strtol(val + 1, NULL, 8);
	} else if (optionok(tmp, "fperm:", val)) {
		options[VOLOPT_FPERM].i_value =
		    (int) strtol(val + 1, NULL, 8);
	} else if (optionok(tmp, "perm:", val)) {
		options[VOLOPT_DFLTPERM].i_value =
		    (int) strtol(val + 1, NULL, 8);
		...
		options[VOLOPT_LIMITSIZE].i_value =
		    (uint32_t) strtoul(val + 1, NULL, 10);

	} else {
		/* ignore unknown options */
		LOG(log_debug, logtype_afpd,
		    "ignoring unknown volume option: %s", tmp);
    ...
    )
{
	...
	if (options) {
		...
		if (options[VOLOPT_PASSWORD].c_value)
			volume->v_password =
			    strdup(options[VOLOPT_PASSWORD].c_value);

		if (options[VOLOPT_VETO].c_value)
			volume->v_veto =
			    strdup(options[VOLOPT_VETO].c_value);

		if (options[VOLOPT_ENCODING].c_value)
			volume->v_volcodepage =
			    strdup(options[VOLOPT_ENCODING].c_value);

		if (options[VOLOPT_MACCHARSET].c_value)
			volume->v_maccodepage =
			    strdup(options[VOLOPT_MACCHARSET].c_value);

		if (options[VOLOPT_DBPATH].c_value)
			volume->v_dbpath =
			    volxlate(obj, NULL, MAXPATHLEN,
				     options[VOLOPT_DBPATH].c_value, pwd,
				     path, name);

		if (options[VOLOPT_CNIDSCHEME].c_value)
			volume->v_cnidscheme =
			    strdup(options[VOLOPT_CNIDSCHEME].c_value);

		if (options[VOLOPT_CNIDSERVER].c_value)
			volume->v_cnidserver =
			    strdup(options[VOLOPT_CNIDSERVER].c_value);

		if (options[VOLOPT_CNIDPORT].c_value)
			volume->v_cnidport =
			    strdup(options[VOLOPT_CNIDPORT].c_value);

		if (options[VOLOPT_UMASK].i_value)
			volume->v_umask =
			    (mode_t) options[VOLOPT_UMASK].i_value;

		if (options[VOLOPT_DPERM].i_value)
			volume->v_dperm =
			    (mode_t) options[VOLOPT_DPERM].i_value;

		if (options[VOLOPT_FPERM].i_value)
			volume->v_fperm =
			    (mode_t) options[VOLOPT_FPERM].i_value;

		if (options[VOLOPT_DFLTPERM].i_value)
			volume->v_perm =
			    (mode_t) options[VOLOPT_DFLTPERM].i_value;

		if (options[VOLOPT_ADOUBLE].i_value)
			volume->v_adouble =
			    options[VOLOPT_ADOUBLE].i_value;
		else
		...
		if (options[VOLOPT_FORCEUID].c_value) {
			volume->v_forceuid =
			    strdup(options[VOLOPT_FORCEUID].c_value);
		} else {
		...
		if (options[VOLOPT_FORCEGID].c_value) {
			volume->v_forcegid =
			    strdup(options[VOLOPT_FORCEGID].c_value);
		} else {
	...
	if ((parent_or_child == 0) && (volume->v_flags & AFPVOL_TM)) {
		...
		if (!uuid) {
			LOG(log_error, logtype_afpd,
			    "Volume '%s': couldn't get UUID",
			    volume->v_localname);
		} else {
			volume->v_uuid = uuid;
			LOG(log_debug, logtype_afpd,
			    "Volume '%s': UUID '%s'", volume->v_localname,
			    volume->v_uuid);
		}
	}
...
static void setextmap(char *ext, char *type, char *creator, int user)
{
	...
	if (Extmap == NULL) {
		if ((Extmap = calloc(1, sizeof(struct extmap))) == NULL) {
			LOG(log_error, logtype_afpd,
			    "setextmap: calloc: %s", strerror(errno));
			return;
		}
	}
	ext++;
	...
	if (em->em_ext == NULL) {
		void *Extmap_check =
			realloc(Extmap, sizeof(struct extmap) * (cnt + 2));
		if (Extmap_check) {
			Extmap = Extmap_check;
		} else {
			LOG(log_error, logtype_afpd,
			    "setextmap: realloc: %s", strerror(errno));
			free(Extmap);
			return;
		}
	...
	if (!(em->em_ext = strdup(ext))) {
		LOG(log_error, logtype_afpd, "setextmap: strdup: %s",
		    strerror(errno));
		return;
	}
...
static void sortextmap(void)
{
	...
	if (Extmap_cnt) {
		qsort(Extmap, Extmap_cnt, sizeof(struct extmap),
		      extmap_cmp);
		if (*Extmap->em_ext == 0) {
		       ...
		       int user, struct passwd *pwent)
{
	...
	while (myfgets(buf, sizeof(buf), fp) != NULL) {
		...
		case ':':
			...
			if (strncmp
			    (path, VOLOPT_DEFAULT,
			     VOLOPT_DEFAULT_LEN) == 0) {
				volfree(default_options, save_options);
				memcpy(default_options, save_options,
				       sizeof(options));
				*tmp = '\0';
				for (i = 0; i < VOLOPT_NUM; i++) {
					if (parseline
					    (sizeof(path) -
					     VOLOPT_DEFAULT_LEN - 1,
					     path + VOLOPT_DEFAULT_LEN) <
					    0)
						break;
					volset(default_options, NULL, tmp,
					       sizeof(tmp) - 1,
					       path + VOLOPT_DEFAULT_LEN);
				}
			}
		...
		case '/':
			...
			if (volxlate
			    (obj, path, sizeof(path) - 1, tmp, pwent, NULL,
			     NULL) == NULL)
				continue;
			...
			*volname = '\0';
			...
			for (i = 0; i < VOLOPT_NUM; i++) {
				...
				volset(options, default_options, volname,
				       sizeof(volname) - 1, tmp);
			}
			...
			if (parent_or_child == 0
			    ||
			    (accessvol
			     (options[VOLOPT_ALLOW].c_value, obj->username)
			     &&
			     (accessvol
				...
				if (volxlate
				    (obj, tmp, sizeof(tmp) - 1, volname,
				     pwent, path, NULL) == NULL)
					continue;
	...
	sortextmap();
	if (fclose(fp) != 0) {
		LOG(log_error, logtype_afpd, "readvolfile: fclose: %s",
		    strerror(errno));
	}
	p1->loaded = 1;
	return (0);
		       ...
		       u_int32_t * bsize)
{
	...
	if ((rc =
	     ustatfs_getvolspace(vol, xbfree, xbtotal, bsize)) != AFP_OK) {
		return (rc);
	}
	...
	if (vol->v_limitsize) {
		...
		*xbfree =
		    MIN(*xbfree,
			*xbtotal <
			vol->v_tm_used ? 0 : *xbtotal - vol->v_tm_used);

		LOG(log_debug, logtype_afpd,
		    "volparams: total: %" PRIu64 ", used: %" PRIu64
		    ", free: %" PRIu64 " bytes", *xbtotal, vol->v_tm_used,
		    *xbfree);
	}
...
static void vol_setdate(u_int16_t id, struct adouble *adp, time_t date)
{
	...
	for (volume = Volumes; volume; volume = volume->v_next) {
		if (volume->v_vid == id) {
			vol = volume;
		} else if ((time_t) (AD_DATE_FROM_UNIX(date)) ==
			   volume->v_ctime) {
			date = date + 1;
...
/* ----------------------- */
static int getvolparams(u_int16_t bitmap, struct vol *vol, struct stat *st,
			char *buf, size_t *buflen)
{
	...
	while (bitmap != 0) {
		...
		case VOLPBIT_BDATE:
			if (!isad
			    || (ad_getdate(&ad, AD_DATE_BACKUP, &aint) <
				0))
				aint = AD_DATE_START;
			memcpy(data, &aint, sizeof(aint));
			data += sizeof(aint);
	...
	if (nameoff) {
		...
		aint =
		    ucs2_to_charset(vol->v_maccharset, vol->v_macname,
				    data + 1, AFPVOL_MACNAMELEN + 1);
		if (aint <= 0) {
			*buflen = 0;
			return AFPERR_MISC;
		}
...
/* ------------------------- */
static int stat_vol(u_int16_t bitmap, struct vol *vol, char *rbuf,
		    size_t *rbuflen)
{
	struct stat st;
	int ret;
...
void load_volumes(AFPObj * obj)
{
	...
	if (Volumes) {
		...
		if (obj->options.defaultvol.loaded) {
			changed =
			    volfile_changed(&obj->options.defaultvol);
		}
		if (obj->options.systemvol.loaded) {
			changed |=
			    volfile_changed(&obj->options.systemvol);
		}
	...
	if (parent_or_child == 0) {
		...
		LOG(log_debug, logtype_afpd, "load_volumes: user: %s",
		    obj->username);
		if ((pwent = getpwnam(obj->username))) {
			if (seteuid(0) < 0)
				LOG(log_error, logtype_afpd,
				    "could not seteuid(%i)", 0);
			ret = set_groups(obj, pwent);
			if (seteuid(obj->uid) < 0)
				LOG(log_error, logtype_afpd,
				    "could not seteuid(%i)", obj->uid);
			if (ret != 0) {
				LOG(log_error, logtype_afpd,
				    "load_volumes: set_groups: %s",
				    strerror(errno));
				return;
			}
		}
		LOG(log_debug, logtype_afpd,
		    "login: supplementary groups: %s",
	...
	if ((*obj->username == '\0')
	    || (obj->options.flags & OPTION_NOUSERVOL)) {
		...
		if (readvolfile
		    (obj, &obj->options.uservol, "AppleVolumes", 1,
		     pwent) < 0
		    && readvolfile(obj, &obj->options.uservol,
				   ".AppleVolumes", 1, pwent) < 0
		    && readvolfile(obj, &obj->options.uservol,
				   "applevolumes", 1, pwent) < 0
		    && readvolfile(obj, &obj->options.uservol,
				   ".applevolumes", 1, pwent) < 0
			...
			if (readvolfile
			    (obj, &obj->options.defaultvol, NULL, 1,
			     pwent) < 0)
				creatvol(obj, pwent, pwent->pw_dir, NULL,
					 NULL, 1);
		}
	}
		     ...
		     char *rbuf, size_t *rbuflen)
{
	...
	data = rbuf + 5;
	for (vcnt = 0, volume = Volumes; volume && vcnt < 255;
		...
		if (!(volume->v_flags & AFPVOL_NOSTAT)) {
			...
			if ((ma.ma_user & (AR_UREAD | AR_USEARCH)) !=
			    (AR_UREAD | AR_USEARCH)) {
		...
		if (utf8_encoding()) {
			len =
			    ucs2_to_charset_allocate(CH_UTF8_MAC, &namebuf,
						     volume->v_u8mname);
		} else {
			len =
	...
	*rbuflen = data - rbuf;
	...
	if (gettimeofday(&tv, NULL) < 0) {
		LOG(log_error, logtype_afpd,
		    "afp_getsrvrparms(%s): gettimeofday: %s",
		    volume->v_path, strerror(errno));
		*rbuflen = 0;
		return AFPERR_PARAM;
	}
...
static int volume_codepage(AFPObj * obj, struct vol *volume)
{
	...
	if ((charset_t) - 1 ==
	    (volume->v_volcharset = add_charset(volume->v_volcodepage))) {
		LOG(log_error, logtype_afpd,
		    "Setting codepage %s as volume codepage failed",
		    volume->v_volcodepage);
		return -1;
	}

	if (NULL ==
	    (charset = find_charset_functions(volume->v_volcodepage))
	...
	if ((charset_t) - 1 ==
	    (volume->v_maccharset = add_charset(volume->v_maccodepage))) {
		LOG(log_error, logtype_afpd,
		    "Setting codepage %s as mac codepage failed",
		    volume->v_maccodepage);
		return -1;
	}

	if (NULL ==
	    (charset = find_charset_functions(volume->v_maccodepage))
	    || !(charset->flags & CHARSET_CLIENT)) {
		LOG(log_error, logtype_afpd,
		    "Fatal error: mac charset %s not supported",
		    volume->v_maccodepage);
		return -1;
	}
...
static int volume_openDB(struct vol *volume)
{
	...
	if (volume->v_cnidscheme == NULL) {
		...
		LOG(log_info, logtype_afpd,
		    "Volume %s use CNID scheme %s.", volume->v_path,
		    volume->v_cnidscheme);
	}
...
/* Found this in branch dir-rewrite, maybe we want to use it sometimes */
	...
	if (volume->v_flags & AFPVOL_CDROM) {
		...
		if (strcmp(volume->v_cnidscheme, "tdb") != 0) {
			...
			volume->v_cnidscheme = strdup("tdb");
			LOG(log_info, logtype_afpd,
			    "Volume %s is ejectable, switching to scheme %s.",
	...
	if (!volume->v_cdb && !(flags & CNID_FLAG_MEMORY)) {
		...
		LOG(log_error, logtype_afpd,
		    "Can't open volume \"%s\" CNID backend \"%s\" ",
		    volume->v_path, volume->v_cnidscheme);
		LOG(log_error, logtype_afpd,
		    "Reopen volume %s using in memory temporary CNID DB.",
		    volume->v_path);
		flags |= CNID_FLAG_MEMORY;
		volume->v_cdb =
		    cnid_open(volume->v_path, volume->v_umask, "tdb",
			      flags, NULL, NULL);
#ifdef SERVERTEXT
...
static void check_ea_sys_support(struct vol *vol)
{
	...
	if (vol->v_vfs_ea == AFPVOL_EA_AUTO) {
		...
		if ((vol->v_flags & AFPVOL_RO) == AFPVOL_RO) {
			LOG(log_info, logtype_afpd,
			    "read-only volume '%s', can't test for EA support, disabling EAs",
			    vol->v_localname);
		...
		if ((sys_setxattr(vol->v_path, eaname, eacontent, 4, 0)) ==
		    0) {
		...
		size_t *rbuflen)
{
    ...
    *rbuflen = 0;
	...
	if (NULL == getcwd(path, MAXPATHLEN)) {
		...
		LOG(log_error, logtype_afpd,
		    "afp_openvol(%s): volume pathlen too long",
		    volume->v_path);
		return AFPERR_MISC;
	}
	...
	if (utf8_encoding()) {
		len =
		    convert_string_allocate(CH_UCS2, CH_UTF8_MAC,
					    volume->v_u8mname, namelen,
					    &vol_mname);
	} else {
		len =
		    convert_string_allocate(CH_UCS2,
					    obj->options.maccharset,
					    volume->v_macname, namelen,
					    &vol_mname);
	}
	if (!vol_mname || len <= 0) {
		ret = AFPERR_MISC;
	...
	if ((dir = dir_new(vol_mname,
			   ...
			   volume,
			   DIRDID_ROOT_PARENT,
			   DIRDID_ROOT, bfromcstr(volume->v_path), &st)
	    ) == NULL) {
		free(vol_mname);
		LOG(log_error, logtype_afpd, "afp_openvol(%s): malloc: %s",
		    volume->v_path, strerror(errno));
		ret = AFPERR_MISC;
		goto openvol_err;
	}
...
/* ------------------------- */
int afp_closevol(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
...
struct extmap *getextmap(const char *path)
{
	...
	em = bsearch(p, Extmap, Extmap_cnt, sizeof(struct extmap),
		     ext_cmp_key);
	if (em) {
		return (em);
	} else {
...
void setvoltime(AFPObj * obj, struct vol *vol)
{
	...
	if (gettimeofday(&tv, NULL) < 0) {
		LOG(log_error, logtype_afpd,
		    "setvoltime(%s): gettimeofday: %s", vol->v_path,
		    strerror(errno));
		return;
	}
...
/* ------------------------- */
int afp_getvolparams(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		     char *rbuf, size_t *rbuflen)
{
	struct vol *vol;
 ...
 */
static int create_special_folder(const struct vol *vol,
				 const struct _special_folder *folder)
{
	char *p, *q, *r;
	struct adouble ad;
	u_int16_t attr;
	...
	p = (char *) malloc(strlen(vol->v_path) + strlen(folder->name) +
			    2);
	if (p == NULL) {
	...
	if (!ret && folder->hide) {
		...
		if (ad_entry(&ad, ADEID_FINDERI)) {
			memcpy(&attr,
			       ad_entry(&ad,
					ADEID_FINDERI) +
			       FINDERINFO_FRFLAGOFF, sizeof(attr));
			attr |= htons(FINDERINFO_INVISIBLE);
			memcpy(ad_entry(&ad, ADEID_FINDERI) +
			       FINDERINFO_FRFLAGOFF, &attr, sizeof(attr));
		}
...
static char *get_vol_uuid(const AFPObj * obj, const char *volname)
{
	...
	if (stat(obj->options.uuidconf, &tmpstat)) {	/* no file */
		...
		LOG(log_error, logtype_afpd,
		    "Cannot create or append to %s (%s).",
		    obj->options.uuidconf, strerror(errno));
		return NULL;
	}
	...
	LOG(log_debug, logtype_afpd, "get_uuid('%s'): generated UUID '%s'",
	    volname, cp);
