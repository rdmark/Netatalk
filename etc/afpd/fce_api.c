...
static unsigned long fce_ev_enabled =
    (1 << FCE_FILE_MODIFY) |
    (1 << FCE_FILE_DELETE) |
    (1 << FCE_DIR_DELETE) | (1 << FCE_FILE_CREATE) | (1 << FCE_DIR_CREATE);
...
void fce_init_udp()
{
	...
	for (int i = 0; i < udp_sockets; i++) {
		...
		if ((rv =
		     getaddrinfo(udp_entry->addr, udp_entry->port, &hints,
				 &servinfo)) != 0) {
			LOG(log_error, logtype_afpd,
			    "fce_init_udp: getaddrinfo(%s:%s): %s",
 ...
 * */
static ssize_t build_fce_packet(struct fce_packet *packet, char *path,
				int mode, uint32_t event_id)
{
	size_t pathlen = 0;
	ssize_t data_len = 0;
	uint64_t *t;
 ...
 **/
static void pack_fce_packet(struct fce_packet *packet, unsigned char *buf,
			    int maxlen)
{
	unsigned char *p = buf;
...
static void send_fce_event(char *path, int mode)
{
	...
	ssize_t data_len =
	    build_fce_packet(&packet, path, mode, ++event_id);
	...
	for (int i = 0; i < udp_sockets; i++) {
		...
		if (udp_entry->sock == -1) {
			...
			/* Reopen socket */
			udp_entry->sock =
			    socket(udp_entry->addrinfo.ai_family,
			...
			if (udp_entry->sock == -1) {
				...
				LOG(log_error, logtype_afpd,
				    "Cannot recreate socket for fce UDP connection: errno %d",
				    errno);
			...
			data_len =
			    build_fce_packet(&packet, "", FCE_CONN_BROKEN,
					     0);
			pack_fce_packet(&packet, iobuf, MAXIOBUF);
			...
			data_len =
			    build_fce_packet(&packet, path, mode,
					     event_id);
			pack_fce_packet(&packet, iobuf, MAXIOBUF);
		}
		...
		if (sent_data != data_len) {
			...
			LOG(log_error, logtype_afpd,
			    "send_fce_event: error sending packet to %s:%s, transfered %d of %d: %s",
			    udp_entry->addr, udp_entry->port, sent_data,
...
static int add_udp_socket(const char *target_ip, const char *target_port)
{
	...
	if (udp_sockets >= FCE_MAX_UDP_SOCKS) {
		LOG(log_error, logtype_afpd,
		    "Too many file change api UDP connections (max %d allowed)",
		    FCE_MAX_UDP_SOCKS);
		return AFPERR_PARAM;
	}
	...
	memset(&udp_socket_list[udp_sockets].addrinfo, 0,
...
static int register_fce(const char *u_name, int is_dir, int mode)
{
	...
	if (mode == FCE_TM_SIZE) {
		...
		if (strlen(cwd) + strlen(u_name) + 1 >= MAXPATHLEN) {
			LOG(log_error, logtype_afpd,
			    "FCE file name too long: %s/%s", cwd, u_name);
			return AFPERR_PARAM;
		}
		...
		if (strlen(cwd) >= MAXPATHLEN) {
			LOG(log_error, logtype_afpd,
			    "FCE directory name too long: %s", cwd);
			return AFPERR_PARAM;
		}
	...
	if (!(mode & FCE_TM_SIZE)
	    && fce_handle_coalescation(full_path_buffer, is_dir, mode)) {
		LOG(log_debug9, logtype_afpd,
		    "Coalesced fc event <%d> for <%s>", mode,
		    full_path_buffer);
		return AFP_OK;
	}

	LOG(log_debug9, logtype_afpd, "Detected fc event <%d> for <%s>",
	    mode, full_path_buffer);
...
static void check_saved_close_events(int fmodwait)
{
	...
	if (last_close_event.time
	    && ((last_close_event.time + fmodwait) < now)) {
		LOG(log_debug, logtype_afpd,
		    "check_saved_close_events: sending event: %s",
...
int fce_register_file_modification(struct ofork *ofork)
{
	...
	if (NULL ==
	    (u_name =
	     mtoupath(vol, of_name(ofork), ofork->of_did,
		      utf8_encoding()))) {
		return AFPERR_MISC;
	}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "d:e:h:p:P:s:")) != -1) {
		...
		case '?':
			fprintf(stdout,
				"%s: [ -p Port -h Listener1 [ -h Listener2 ...] -P path -s Delay_between_events_in_us -e event_code -d Duration ]\n",
				argv[0]);
			exit(1);
			break;
		case 'd':
