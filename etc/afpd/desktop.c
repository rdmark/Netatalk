...
static char *icon_dtfile(struct vol *vol, u_int8_t creator[4])
{
	return dtfile(vol, creator, ".icon");
}

static int iconopen(struct vol *vol, u_int8_t creator[4], int flags,
		    int mode)
{
	char *dtf, *adt, *adts;

	if (si.sdt_fd != -1) {
		if (memcmp(si.sdt_creator, creator, sizeof(CreatorType)) ==
		    0 && si.sdt_vid == vol->v_vid) {
			return 0;
		}
		close(si.sdt_fd);
		si.sdt_fd = -1;
	...
	if ((si.sdt_fd = open(dtf, flags, ad_mode(dtf, mode))) < 0) {
		if (errno == ENOENT && (flags & O_CREAT)) {
			...
			*adts = '/';
			...
			if ((si.sdt_fd =
			     open(dtf, flags, ad_mode(dtf, mode))) < 0) {
				LOG(log_error, logtype_afpd,
				    "iconopen(%s): open: %s", dtf,
				    strerror(errno));
				return -1;
			}
		} else {
		...
		size_t *rbuflen)
{
	struct vol *vol;
	struct iovec iov[2];
	u_int8_t fcreator[4], imh[12], irh[12], *p;
	...
	*rbuflen = 0;
	...
	if (lseek(si.sdt_fd, (off_t) 0L, SEEK_SET) < 0) {
		close(si.sdt_fd);
		si.sdt_fd = -1;
		LOG(log_error, logtype_afpd, "afp_addicon(%s): lseek: %s",
	...
	*p++ = 0;
	...
	while ((cc = read(si.sdt_fd, irh, sizeof(irh))) > 0) {
		 ...
		 */
		if (memcmp(irh, imh, sizeof(irh) - sizeof(u_int16_t)) == 0) {
			/*
			 * Is the size correct?
			 */
			if (bsize != rsize)
		...
		if (lseek(si.sdt_fd, (off_t) rsize, SEEK_CUR) < 0) {
			LOG(log_error, logtype_afpd,
			    "afp_addicon(%s): lseek: %s", icon_dtfile(vol,
								      fcreator),
			    strerror(errno));
			cc = AFPERR_PARAM;
		}
	...
	case AFPPROTO_ASP:
		...
		if (writev(si.sdt_fd, iov, iovcnt) < 0) {
			LOG(log_error, logtype_afpd,
			    "afp_addicon(%s): writev: %s", icon_dtfile(vol,
								       fcreator),
			    strerror(errno));
			return (AFPERR_PARAM);
		}
...
static const u_int8_t utag[] = { 0, 0, 0, 0 };
static const u_int8_t ucreator[] = { 0, 0, 0, 0 };	/* { 'U', 'N', 'I', 'X' }; */
static const u_int8_t utype[] = { 0, 0, 0, 0 };	/* { 'T', 'E', 'X', 'T' }; */

static const int16_t usize = 256;

int afp_geticoninfo(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		    char *rbuf, size_t *rbuflen)
{
	struct vol *vol;
	u_int8_t fcreator[4], ih[12];
	...
	*rbuflen = 0;
	...
	for (;;) {
		...
		if (lseek(si.sdt_fd, (off_t) bsize, SEEK_CUR) < 0) {
			LOG(log_error, logtype_afpd,
			    "afp_iconinfo(%s): lseek: %s", icon_dtfile(vol,
								       fcreator),
			    strerror(errno));
			return (AFPERR_PARAM);
		}
		...
		size_t *rbuflen)
{
	...
	off_t offset;
	ssize_t rc, buflen;
	u_int8_t fcreator[4], ftype[4], itype, ih[12];
	...
	*rbuflen = 0;
	...
	if (lseek(si.sdt_fd, (off_t) 0L, SEEK_SET) < 0) {
		close(si.sdt_fd);
		si.sdt_fd = -1;
		LOG(log_error, logtype_afpd, "afp_geticon(%s): lseek: %s",
		    icon_dtfile(vol, fcreator), strerror(errno));
		return (AFPERR_PARAM);
	}
	...
	while ((rc = read(si.sdt_fd, ih, sizeof(ih))) > 0) {
		...
		if (lseek(si.sdt_fd, (off_t) rsize, SEEK_CUR) < 0) {
			LOG(log_error, logtype_afpd,
			    "afp_geticon(%s): lseek: %s", icon_dtfile(vol,
								      fcreator),
			    strerror(errno));
			return (AFPERR_PARAM);
		}
	...
	if (rc < 0) {
		LOG(log_error, logtype_afpd, "afp_geticon(%s): read: %s",
		    icon_dtfile(vol, fcreator), strerror(errno));
		return (AFPERR_PARAM);
	}
...
char *dtfile(const struct vol *vol, u_int8_t creator[], char *ext)
{
...
char *mtoupath(const struct vol *vol, char *mpath, cnid_t did, int utf8)
{
	...
	if ((size_t) -1 ==
	    (outlen =
	     convert_charset((utf8) ? CH_UTF8_MAC : vol->v_maccharset,
			     vol->v_volcharset, vol->v_maccharset, m,
			     inplen, u, outlen, &flags))) {
		LOG(log_error, logtype_afpd,
		    "conversion from %s to %s for %s failed.",
		    (utf8) ? "UTF8-MAC" : vol->v_maccodepage,
...
char *utompath(const struct vol *vol, char *upath, cnid_t id, int utf8)
{
	...
	if ((size_t) -1 ==
	    (outlen =
	     convert_charset(vol->v_volcharset,
			     (utf8) ? CH_UTF8_MAC : vol->v_maccharset,
			     vol->v_maccharset, u, outlen, mpath,
			     MAXPATHLEN, &flags))) {
		LOG(log_error, logtype_afpd,
		    "Conversion from %s to %s for %s (%u) failed.",
...
static int ad_addcomment(struct vol *vol, struct path *path, char *ibuf)
{
	...
	if (ad_open_metadata
	    (upath, ((isadir) ? ADFLAGS_DIR : 0), O_CREAT, adp) < 0) {
		return (AFPERR_ACCESS);
	}
...
/* ----------------------------- */
int afp_addcomment(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		   char *rbuf _U_, size_t *rbuflen)
{
	struct vol *vol;
	struct dir *dir;
...
/* -------------------- */
int afp_getcomment(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
		   char *rbuf, size_t *rbuflen)
{
	struct vol *vol;
	struct dir *dir;
...
static int ad_rmvcomment(struct vol *vol, struct path *path)
{
	...
	if (ad_open_metadata(upath, (isadir) ? ADFLAGS_DIR : 0, 0, adp) <
	    0) {
		switch (errno) {
		case ENOENT:
			return (AFPERR_NOITEM);
