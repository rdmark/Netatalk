	...
	if (sd->sd_last + len + lenm + 4 > end) {
		...
		if (!(buf = realloc(sd->sd_buf, sd->sd_buflen + SDBUFBRK))) {
			LOG(log_error, logtype_afpd,
			    "afp_enumerate: realloc: %s", strerror(errno));
			errno = ENOMEM;
			return -1;
		}
		sd->sd_buf = buf;
		     ...
		     char *rbuf, size_t *rbuflen, int ext)
{
	...
	if (sd.sd_buflen == 0) {
		if ((sd.sd_buf = (char *) malloc(SDBUFBRK)) == NULL) {
			LOG(log_error, logtype_afpd,
			    "afp_enumerate: malloc: %s", strerror(errno));
			*rbuflen = 0;
			return AFPERR_MISC;
		}
		sd.sd_buflen = SDBUFBRK;
	...
	if (NULL == (dir = dirlookup(vol, did))) {
		*rbuflen = 0;
		return (afp_errno ==
			AFPERR_NOOBJ) ? AFPERR_NODIR : afp_errno;
	}
	...
	*rbuflen = 0;
	...
	LOG(log_debug, logtype_afpd,
	    ...
	    getcwdpath(), o_path->u_name, fbitmap, dbitmap, reqcnt, sindex,
	    maxsz);
	...
	if (sindex == 1 || curdir->d_did != sd.sd_did || vid != sd.sd_vid) {
		...
		if ((!o_path->st_valid
		     && ostat(".", &o_path->st, vol_syml_opt(vol)) < 0)
		    || (ret =
			for_each_dirent(vol, ".", enumerate_loop,
					(void *) &sd)) < 0) {
			LOG(log_error, logtype_afpd,
			    "enumerate: loop error: %s (%d)",
	...
	while ((len = (unsigned char) *(sd.sd_last)) != 0) {
		...
		if (S_ISDIR(s_path.st.st_mode)) {
			...
			if ((dir =
				...
				if ((dir =
					...
					LOG(log_error, logtype_afpd,
					    ...
					    ntohs(vid), ntohl(did),
					    o_path->u_name, s_path.u_name);
					return AFPERR_MISC;
				}
			}
			if ((ret =
			     getdirparams(vol, dbitmap, &s_path, dir,
			...
			if (AFP_OK !=
			    (ret =
			     getfilparams(vol, fbitmap, &s_path, curdir,
					  data + header, &esz))) {
				return (ret);
			}
		}
		...
		*data++ =
		    S_ISDIR(s_path.st.
			    st_mode) ? FILDIRBIT_ISDIR : FILDIRBIT_ISFILE;
		if (ext) {
			*data++ = 0;
		}
		data += esz;
