...
static struct uam_mod uam_modules =
    { NULL, NULL, &uam_modules, &uam_modules };
...
static int set_auth_switch(int expired)
{
	...
	if (!expired) {
		...
		case 32:
#ifdef HAVE_ACLS
			uam_afpserver_action(AFP_GETACL,
					     UAM_AFPSERVER_POSTAUTH,
					     afp_getacl, NULL);
			uam_afpserver_action(AFP_SETACL,
					     UAM_AFPSERVER_POSTAUTH,
					     afp_setacl, NULL);
			uam_afpserver_action(AFP_ACCESS,
					     UAM_AFPSERVER_POSTAUTH,
					     afp_access, NULL);
#endif				/* HAVE_ACLS */
		...
		for (i = 0; i <= 0xff; i++) {
			uam_afpserver_action(i, UAM_AFPSERVER_PREAUTH,
					     afp_errpwdexpired, NULL);
		}
		uam_afpserver_action(AFP_LOGOUT, UAM_AFPSERVER_PREAUTH,
				     afp_logout, NULL);
		 ...
		 int expired)
{
	...
	if (obj->cnx_cnt > obj->cnx_max) {
		LOG(log_error, logtype_dsi,
		    "login: too many connections, limit: %d",
		    obj->cnx_max);
...
#ifdef ADMIN_GRP
	LOG(log_debug, logtype_afpd, "obj->options.admingid == %d",
	    obj->options.admingid);

	if (obj->options.admingid != 0) {
		int i;
		for (i = 0; i < ngroups; i++) {
			if (groups[i] == obj->options.admingid)
				admin = 1;
		}
	}
	if (admin) {
		ad_setfuid(0);
		LOG(log_info, logtype_afpd, "admin login -- %s",
		    pwd->pw_name);
	}
	if (!admin)
#endif				/* ADMIN_GRP */
		if (setegid(pwd->pw_gid) < 0 || seteuid(pwd->pw_uid) < 0) {
			LOG(log_error, logtype_afpd, "login: %s %s",
			    pwd->pw_name, strerror(errno));
			return AFPERR_BADUAM;
		}

	LOG(log_debug, logtype_afpd, "login: supplementary groups: %s",
...
const char *print_groups(int ngroups, gid_t * groups)
{
	...
	for (i = 0; (i < ngroups) && (s < &groupsstr[GROUPSTR_BUFSIZE]);
	     i++) {
		s += snprintf(s, &groupsstr[GROUPSTR_BUFSIZE] - s, " %u",
			      groups[i]);
	}
...
int set_groups(AFPObj * obj, struct passwd *pwd)
{
	if (initgroups(pwd->pw_name, pwd->pw_gid) < 0)
		LOG(log_error, logtype_afpd, "initgroups(%s, %d): %s",
		    pwd->pw_name, pwd->pw_gid, strerror(errno));

	if ((ngroups = getgroups(0, NULL)) < 0) {
		LOG(log_error, logtype_afpd, "login: %s getgroups: %s",
		    pwd->pw_name, strerror(errno));
		return -1;
	}
	...
	if (NULL == (groups = calloc(ngroups, GROUPS_SIZE))) {
		LOG(log_error, logtype_afpd, "login: %s calloc: %d",
		    ngroups);
		return -1;
	}
...
static int create_session_key(AFPObj * obj)
{
	...
	if (obj->sinfo.sessionkey == NULL) {
		if (NULL ==
		    (obj->sinfo.sessionkey = malloc(SESSIONKEY_LEN)))
			return AFPERR_MISC;
		uam_random_string(obj, obj->sinfo.sessionkey,
				  SESSIONKEY_LEN);
	...
	*rbuflen = 0;
	...
	case 4:
		if (ibuflen >= 8) {
			...
			if (ipc_child_write
			    (obj->ipc_fd, IPC_GETSESSION, idlen + 8,
			     p) != 0)
				return AFPERR_MISC;
			tklen = obj->sinfo.sessiontoken_len;
			token = obj->sinfo.sessiontoken;
		       ...
		       size_t len)
{
	...
	if (afp_version >= 30 && sizeof(off_t) != 8) {
		LOG(log_error, logtype_afpd,
		    "get_version: no LARGE_FILE support recompile!");
		return AFPERR_BADVERS;
	}
	      ...
	      size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	i = afp_uam->u.uam_login.login(obj, &pwd, ibuf, ibuflen, rbuf,
				       rbuflen);

	if (!pwd || (i != AFP_OK && i != AFPERR_PWDEXPR))
		return send_reply(obj, i);

	return send_reply(obj,
			  login(obj, pwd, afp_uam->u.uam_login.logout,
		  ...
		  size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if (!afp_uam->u.uam_login.login_ext) {
		LOG(log_error, logtype_afpd,
		    "login_ext: uam %s not AFP 3 ready!",
		    afp_uam->uam_name);
		return send_reply(obj, AFPERR_BADUAM);
	}
	/* user name */
	...
	if (len != 0) {
		LOG(log_error, logtype_afpd,
		    "login_ext: directory service path not null!");
		return send_reply(obj, AFPERR_PARAM);
	}
#endif
	ibuf += len;
	...
	i = afp_uam->u.uam_login.login_ext(obj, username, &pwd, ibuf,
					   ibuflen, rbuf, rbuflen);
...
/* FPGetUserInfo */
int afp_getuserinfo(AFPObj * obj _U_, char *ibuf, size_t ibuflen _U_,
	...
	if (bitmap & USERIBIT_UUID) {
		if (!(obj->options.flags & OPTION_UUID)) {
			...
			LOG(log_debug, logtype_afpd,
			    "afp_getuserinfo: get UUID for \'%s\'",
			    obj->username);
			int ret;
			atalk_uuid_t uuid;
			ret =
			    getuuidfromname(obj->username, UUID_USER,
					    uuid);
			if (ret != 0) {
				LOG(log_info, logtype_afpd,
				    "afp_getuserinfo: error getting UUID !");
				return AFPERR_NOITEM;
			}
			LOG(log_debug, logtype_afpd,
			    "afp_getuserinfo: got UUID: %s",
	...
	while (p) {
		...
		if (stat(name, &st) == 0) {
			if ((mod = uam_load(name, p))) {
				...
				LOG(log_error, logtype_afpd,
				    "uam: %s load failure", p);
			}
		} else {
			LOG(log_info, logtype_afpd,
			    "uam: uam not found (status=%d)", stat(name,
								   &st));
		}
		p = strtok(NULL, ",");
	}
