...
int cq_default(struct papfile *in, struct papfile *out)
{
	...
	for (;;) {
		...
		if (comgetflags() == 0) {	/* started */
			...
			/* return default */
			if (comcmp
			    (start, start + linelength, comment->c_end,
			     0) == 0) {
...
int cq_k4login(struct papfile *in, struct papfile *out)
{
	...
	if ((rc = krb_rd_req(&tkt, "LaserWriter", printer->p_name,
			     0, &ad, "")) != RD_AP_OK) {
		LOG(log_error, logtype_papd, "cq_k4login: %s",
		    krb_err_txt[rc]);
	...
	LOG(log_info, logtype_papd, "cq_k4login: %s.%s@%s", ad.pname,
	    ad.pinst, ad.prealm);
	lp_person(ad.pname);
	lp_host(ad.prealm);
	...
	for (;;) {
		...
		if (comgetflags() == 0) {	/* started */
			...
			for (gq = genqueries; gq->gq_name; gq++) {
				if ((strlen(gq->gq_name) ==
				     (size_t) (q - p))
				    && (strncmp(gq->gq_name, p, q - p) ==
					0)) {
					break;
				}
			}
	...
	for (;;) {
		...
		if (comgetflags() == 0) {
			...
			if (comgetflags() == 1 &&
				...
				comsetflags(2);
				if (comcmp(start, stop, comment->c_end, 0)
				    == 0) {
					append(out, "*\n", 2);
					compop();
					CONSUME(in,
...
int cq_printer(struct papfile *in, struct papfile *out)
{
	...
	for (;;) {
		...
		if (comgetflags() == 0) {
			...
			if ((pdpsver =
			     ppd_feature(psver, strlen(psver))) == NULL) {
				if (comswitch(queries, cq_default) < 0) {
					LOG(log_error, logtype_papd,
					    "cq_printer: can't find default!");
					exit(1);
				}
				return (CH_DONE);
			}
			...
			if (*p == '\0') {
				LOG(log_error, logtype_papd,
				    "cq_printer: can't parse PSVersion!");
				if (comswitch(queries, cq_default) < 0) {
					LOG(log_error, logtype_papd,
					    "cq_printer: can't find default!");
					exit(1);
			...
			if ((pdprod =
			     ppd_feature(prod, strlen(prod))) == NULL) {
				if (comswitch(queries, cq_default) < 0) {
					LOG(log_error, logtype_papd,
					    "cq_printer: can't find default!");
					exit(1);
			...
			/* version */
			append(out, pdpsver->pd_value,
			       p - pdpsver->pd_value);
			append(out, "\r", 1);

			/* product */
			append(out, pdprod->pd_value,
			       strlen(pdprod->pd_value));
			append(out, "\r", 1);
		} else {
			if (comcmp
			    (start, start + linelength, comment->c_end,
...
int cq_rbilogin(struct papfile *in, struct papfile *out)
{
	...
	for (;;) {
		...
		if (comgetflags() == 0) {	/* first line */
			...
			if (!*uamtype
			    || (papd_uam =
				auth_uamfind(UAM_SERVER_PRINTAUTH, uamtype,
					     strlen(uamtype))) == NULL) {
				LOG(log_info, logtype_papd,
				    "Could not find uam: %s", uamtype);
				append(out, rbiloginbad,
				       strlen(rbiloginbad));
				append(out, rbiloginerrstr,
				       strlen(rbiloginerrstr));
			} else {
				if ((papd_uam->u.
				     uam_printer(p, stop, username,
						 out)) == 0) {
					lp_person(username);
					append(out, rbiloginok,
					       strlen(rbiloginok));
					LOG(log_info, logtype_papd,
					    "RBILogin: Logged in '%s'",
					    username);
				} else {
					append(out, rbiloginbad,
					       strlen(rbiloginbad));
...
struct papd_comment queries[] = {
	...
	{ "%%?BeginUAMethodsQuery", "%%?EndUAMethodsQuery:", cq_uameth,
	 C_FULL },
#endif				/* KRB */
#ifndef HAVE_CUPS
