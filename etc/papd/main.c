...
static void die(int n)
{
	...
	for (pr = printers; pr; pr = pr->p_next) {
		if (pr->p_flags & P_REGISTERED) {
			if (nbp_unrgstr
			    (pr->p_name, pr->p_type, pr->p_zone,
			     &addr) < 0) {
				LOG(log_error, logtype_papd,
				    "can't unregister %s:%s@%s",
				    pr->p_name, pr->p_type, pr->p_zone);
				papd_exit(n + 1);
			}
			LOG(log_info, logtype_papd, "unregister %s:%s@%s",
			    pr->p_name, pr->p_type, pr->p_zone);
		}
#ifdef HAVE_CUPS
		if (pr->p_flags & P_SPOOLED && pr->p_flags & P_CUPS_PPD) {
			LOG(log_info, logtype_papd,
			    "Deleting CUPS temp PPD file for %s (%s)",
...
int main(int ac, char **av)
{
	...
	*p = '\0';
	...
	set_processname(p);
	syslog_setup(log_debug, logtype_default,
		     logoption_ndelay | logoption_pid | debug ?
		     logoption_perror : 0, logfacility_lpr);

	LOG(log_info, logtype_papd, "restart (%s)", version);
#ifdef HAVE_CUPS
	LOG(log_info, logtype_papd, "CUPS support enabled (%s)",
	    CUPS_API_VERSION);
#endif
	...
	for (pr = printers; pr; pr = pr->p_next) {
		if ((pr->p_flags & P_SPOOLED) && rprintcap(pr) < 0) {
			LOG(log_error, logtype_papd,
			    "printcap problem: %s", pr->p_printer);
		}

		if (!(pr->p_flags & P_CUPS)) {
			if ((size_t) -1 !=
			    convert_string_allocate(CH_UNIX, CH_MAC,
						    pr->p_name, -1,
						    &atname)) {
		...
		if ((pr->p_atp =
		     atp_open(ATADDR_ANYPORT, &pr->p_addr)) == NULL) {
			LOG(log_error, logtype_papd, "atp_open: %s",
			    strerror(errno));
			papd_exit(1);
		}
		if (nbp_rgstr
		    (atp_sockaddr(pr->p_atp), pr->p_name, pr->p_type,
		     pr->p_zone) < 0) {
			LOG(log_error, logtype_papd,
			    "can't register %s:%s@%s", pr->p_u_name,
			    pr->p_type, pr->p_zone);
			die(1);
		}
		if (pr->p_flags & P_AUTH) {
			LOG(log_info, logtype_papd,
			    "Authentication enabled: %s", pr->p_u_name);
		} else {
			LOG(log_info, logtype_papd,
			    "Authentication disabled: %s", pr->p_u_name);
		}
		LOG(log_info, logtype_papd, "register %s:%s@%s",
		    pr->p_u_name, pr->p_type, pr->p_zone);
	...
	if (sigaction(SIGTERM, &sv, NULL) < 0) {
		LOG(log_error, logtype_papd, "sigaction: %s",
		    strerror(errno));
		papd_exit(1);
	}
	...
	for (;;) {
		...
		if ((c = select(FD_SETSIZE, &fdset, NULL, NULL, NULL)) < 0) {
			...
			LOG(log_error, logtype_papd, "select: %s",
			    strerror(errno));
			papd_exit(1);
		}
			...
			if (FD_ISSET(atp_fileno(pr->p_atp), &fdset)) {
				...
				case PAP_OPEN:
					...
					if ((pr->p_flags & P_SPOOLED)
						...
						LOG(log_error,
						    logtype_papd,
						    "CUPS_PAP_OPEN: %s is not accepting jobs",
						    pr->p_printer);
						rbuf[2] = rbuf[3] = 0xff;
						err = 1;
					}
...
static void getprinters(char *cf)
{
	char buf[PF_CONFBUFFER], area[PF_CONFBUFFER], *a, *p, *name, *type,
	    *zone;
	struct printer *pr;
	int c;
	...
	while ((c = getprent(cf, buf, PF_CONFBUFFER)) > 0) {
		...
		if ((pr =
		     (struct printer *) malloc(sizeof(struct printer)))
		    == NULL) {
			perror("malloc");
			exit(1);
		}
		...
		if (name != defprinter.p_name) {
			if ((pr->p_name =
			     (char *) malloc(strlen(name) + 1)) == NULL) {
				perror("malloc");
				exit(1);
			}
		...
		if (type != defprinter.p_type) {
			if ((pr->p_type =
			     (char *) malloc(strlen(type) + 1)) == NULL) {
				perror("malloc");
				exit(1);
			}
		...
		if (zone != defprinter.p_zone) {
			if ((pr->p_zone =
			     (char *) malloc(strlen(zone) + 1)) == NULL) {
				perror("malloc");
				exit(1);
			}
		...
		if ((p = pgetstr("pa", &a)) == NULL)
			memcpy(&pr->p_addr, &defprinter.p_addr,
			       sizeof(pr->p_addr));
		else
		...
		if ((p = pgetstr("co", &a)) != NULL) {
			...
			LOG(log_error, logtype_papd,
			    "enabling cups-options for %s: %s", pr->p_name,
			    pr->p_cupsoptions);
		}
#endif
		...
		if (pgetflag("fo") == 1) {
			...
			LOG(log_error, logtype_papd,
			    "enabling foomatic hack for %s", pr->p_name);
		}
...
#else
			LOG(log_error, logtype_papd,
			    "cupsautoadd: Cups support not compiled in");
#endif				/* HAVE_CUPS */
		} else {
#ifdef HAVE_CUPS
...
int rprintcap(struct printer *pr)
{
	...
	if (pr->p_flags & P_SPOOLED && !(pr->p_flags & P_CUPS_AUTOADDED)) {	/* Skip check if autoadded */
		if (cups_printername_ok(pr->p_printer) != 1) {
			LOG(log_error, logtype_papd,
			    "No such CUPS printer: '%s'", pr->p_printer);
			return (-1);
		}
	}
	...
	if (pr->p_ppdfile == NULL) {
		if ((p =
		     (char *) cups_get_printer_ppd(pr->p_printer)) !=
		    NULL) {
			if ((pr->p_ppdfile =
			     (char *) malloc(strlen(p) + 1)) == NULL) {
			...
			LOG(log_info, logtype_papd,
			    "PPD File for %s set to %s", pr->p_printer,
			    pr->p_ppdfile);
		}
	}
	...
	if (pr->p_flags & P_SPOOLED) {
		if (pgetent(printcap, buf, pr->p_printer) != 1) {
			LOG(log_error, logtype_papd, "No such printer: %s",
			    pr->p_printer);
			return (-1);
		}
