...
static struct ppdent *getppdent(FILE * stream)
{
	...
	while ((p = my_fgets(buf, sizeof(buf), stream)) != NULL) {
		...
		if (*p == '\n') {
			*p = '\0';
			ppdent.pe_option = ppdent.pe_translation =
			    ppdent.pe_value = NULL;
			return (&ppdent);
		}
...
int read_ppd(char *file, int fcnt)
{
	...
	if (fcnt > 20) {
		LOG(log_error, logtype_papd,
		    "read_ppd: %s: Too many files!", file);
		return (-1);
	}

	if ((ppdfile = fopen(file, "r")) == NULL) {
		LOG(log_error, logtype_papd, "read_ppd %s: %s", file,
		    strerror(errno));
		return (-1);
	}
	...
	while ((pe = getppdent(ppdfile)) != NULL) {
		...
		if (strcmp(pe->pe_main, "*Font") == 0 && pe->pe_option) {
			for (pfo = ppd_fonts; pfo; pfo = pfo->pd_next) {
				if (strcmp(pfo->pd_font, pe->pe_option) ==
				    0) {
					break;
				}
			}
			...
			if ((pfo->pd_font =
			     (char *) malloc(strlen(pe->pe_option) + 1)) ==
			    NULL) {
				LOG(log_error, logtype_papd, "malloc: %s",
				    strerror(errno));
				exit(1);
		...
		if (pfe->pd_name && pe->pe_value) {
			if ((pfe->pd_value =
			     (char *) malloc(strlen(pe->pe_value) + 1)) ==
			    NULL) {
				LOG(log_error, logtype_papd, "malloc: %s",
				    strerror(errno));
				exit(1);
...
struct ppd_feature *ppd_feature(const char *feature, int len)
{
	...
	*q = '\0';
	...
	for (pfe = ppd_features; pfe->pd_name; pfe++) {
		if ((strcmp(pfe->pd_name, ppd_feature_main) == 0)
		    && pfe->pd_value) {
			return (pfe);
		}
	}
