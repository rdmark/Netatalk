...
int session(ATP atp, struct sockaddr_at *sat)
{
	...
	if (atp_sreq(atp, &atpb, oquantum, ATP_XO)) {
		LOG(log_error, logtype_papd, "atp_sreq: %s",
		    strerror(errno));
		return (-1);
	}
	...
	for (;;) {
		...
		case ATP_TRESP:
			...
			if (atp_rresp(atp, &atpb) < 0) {
				LOG(log_error, logtype_papd,
				    "atp_rresp: %s", strerror(errno));
				return (-1);
			}

			/* sanity */
			if (((unsigned char *) niov[0].iov_base)[0] !=
			    connid
			    || ((char *) niov[0].iov_base)[1] !=
			    PAP_DATA) {
				LOG(log_error, logtype_papd,
		...
		default:
			LOG(log_error, logtype_papd, "atp_rsel: %s",
			    strerror(errno));
			return (-1);
		}
