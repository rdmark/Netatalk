...
static char *papargv[] =
    { "pap", "-sstatus", NULL, NULL, NULL, NULL, NULL, NULL };
...
int main(int ac, char **av)
{
	...
	if (*prog != 'i' && *prog != 'o' && *(prog + 1) == 'f') {
		...
		if ((c = pexecv(_PATH_PSFILTER, filtargv)) < 0) {
			syslog(LOG_ERR, "%s: %s", _PATH_PSFILTER,
			       strerror(errno));
			exit(2);
		}
...
#define WEXITSTATUS(x)	((x).w_status)
			...
			if (WEXITSTATUS(status) != 0) {
				syslog(LOG_ERR, "%d died with %d", c,
				       WEXITSTATUS(status));
	...
	do {
		...
		while (p < end) {
			if ((state & ST_PAGE) == 0 && *p != '\031'
				...
				printf("%%%%Page: ? %d\n", ++npages);
				printf("%d %f %f /%s %d SP\n", indent,
				       papersize.win, papersize.lin, font,
				       point);
				state |= ST_PAGE;
			}
