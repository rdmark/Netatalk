...
int dbd_check_indexes(DBD * dbd, char *dbdir)
{
	...
	LOG(log_note, logtype_cnid,
	    "CNID database at `%s' is being checked (quick)", dbdir);
	...
	if (c_cnid != c_devino) {
		LOG(log_error, logtype_cnid,
		    "CNID database at `%s' corrupted (%u/%u)", dbdir,
		    c_cnid, c_devino);
		return 1;
	}
	...
	LOG(log_note, logtype_cnid,
	    "CNID database at `%s' seems ok, %u entries.", dbdir, c_cnid);
	return 0;
}
