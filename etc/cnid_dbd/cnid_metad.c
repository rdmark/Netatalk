...
static void sigterm_handler(int sig)
{
	switch (sig) {
	case SIGTERM:
		LOG(log_info, logtype_afpd, "shutting down on signal %d",
		    sig);
		break;
	default:
...
static struct server *test_usockfn(struct volinfo *volinfo)
{
	...
	for (i = 0; i < maxvol; i++) {
		if ((srv[i].volinfo)
		    && (strcmp(srv[i].volinfo->v_path, volinfo->v_path) ==
			0)) {
			return &srv[i];
		}
	}
...
static int maybe_start_dbd(char *dbdpn, struct volinfo *volinfo)
{
	...
	LOG(log_maxdebug, logtype_cnid,
	    "maybe_start_dbd: no cnid_dbd for that volume yet");
		...
		if (up->count > 0) {
			...
			if (t < (up->tm + TESTTIME)) {
				...
				if (up->count > MAXSPAWN) {
					...
					LOG(log_maxdebug, logtype_cnid,
					    "maybe_start_dbd: respawning too fast");
	...
	if (socketpair(PF_UNIX, SOCK_STREAM, 0, sv) < 0) {
		LOG(log_error, logtype_cnid, "error in socketpair: %s",
		    strerror(errno));
		return -1;
	}

	if ((pid = fork()) < 0) {
		LOG(log_error, logtype_cnid, "error in fork: %s",
		    strerror(errno));
		return -1;
	}
	if (pid == 0) {
		...
		if (up->count == MAXSPAWN) {
			...
			LOG(log_warning, logtype_cnid,
			    ...
			    up->volinfo->v_path);
			ret =
			    execlp(dbdpn, dbdpn, "-d", volpath, buf1, buf2,
				   logconfig, NULL);
		} else {
			ret =
...
static int set_dbdir(char *dbdir)
{
	...
	if (stat(dbdir, &st) < 0 && mkdir(dbdir, 0755) < 0) {
		LOG(log_error, logtype_cnid,
		    "set_dbdir: mkdir failed for %s", dbdir);
		return -1;
	}
...
static void set_signal(void)
{
	...
	if (sigaction(SIGTERM, &sv, NULL) < 0) {
		LOG(log_error, logtype_afpd, "sigaction: %s",
		    strerror(errno));
		daemon_exit(EXITERR_SYS);
	}
...
static int setlimits(void)
{
	...
	if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
		LOG(log_error, logtype_afpd, "setlimits: %s",
		    strerror(errno));
		exit(1);
	}
...
int main(int argc, char *argv[])
{
	...
	/* switch uid/gid */
	if (uid || gid) {
		LOG(log_debug, logtype_cnid, "Setting uid/gid to %i/%i",
		    uid, gid);
		if (gid) {
			if (SWITCH_TO_GID(gid) < 0) {
				LOG(log_info, logtype_cnid,
				    "unable to switch to group %d", gid);
				daemon_exit(1);
			}
		}
		if (uid) {
			if (SWITCH_TO_UID(uid) < 0) {
				LOG(log_info, logtype_cnid,
				    "unable to switch to user %d", uid);
				daemon_exit(1);
			}
		}
	}
	...
	while (1) {
		...
		while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
			...
			if (WIFEXITED(status)) {
				LOG(log_info, logtype_cnid,
				    "cnid_dbd[%i] exited with exit code %i",
			...
			if (WIFSIGNALED(status)) {
				LOG(log_info, logtype_cnid,
				    "cnid_dbd[%i] received signal %i", pid,
				    WTERMSIG(status));
			}
			sigchild = 0;
		}
		...
		if (!ret) {
			...
			LOG(log_severe, logtype_cnid, "error read: %s",
			    strerror(errno));
			goto loop_end;
		} else if (ret != sizeof(int)) {
			LOG(log_error, logtype_cnid, "short read: got %d",
			    ret);
			goto loop_end;
		}
		/*
		...
		if (!len || len + DBHOMELEN + 2 > MAXPATHLEN) {
			LOG(log_error, logtype_cnid,
			    "wrong len parameter: %d", len);
			goto loop_end;
		}

		actual_len = readt(rqstfd, volpath, len, 1, 5);
		if (actual_len < 0) {
			LOG(log_severe, logtype_cnid, "Read(2) error : %s",
			    strerror(errno));
			goto loop_end;
		}
		if (actual_len != len) {
			LOG(log_error, logtype_cnid,
			    "error/short read (dir): %s", strerror(errno));
			goto loop_end;
		}
		volpath[len] = '\0';
		...
		if ((volinfo = allocvolinfo(volpath)) == NULL) {
			LOG(log_severe, logtype_cnid,
			    "allocvolinfo(\"%s\"): %s", volpath,
			    strerror(errno));
			goto loop_end;
		}
