...
static void default_params(struct db_param *dbp, char *dir)
{
	...
	if (make_pathname
	    (dbp->usock_file, dir, DEFAULT_USOCK_FILE,
	     usock_maxlen()) < 0) {
...
static int parse_int(char *val)
{
	...
	if (tmp[0] != '\0') {
		LOG(log_error, logtype_cnid,
		    "invalid characters in token %s", val);
		parse_err++;
	}
	return result;
}
...
struct db_param *db_param_read(char *dir)
{
	...
	if ((fp = fopen(pfn, "r")) == NULL) {
		if (errno == ENOENT) {
			...
			LOG(log_error, logtype_cnid,
			    "error opening %s: %s", pfn, strerror(errno));
			return NULL;
		}
	}
	parse_err = 0;
	...
	while ((items = fscanf(fp, " %s %s", key, val)) != EOF) {
		...
		if (!strcmp(key, "usock_file")) {
			if (make_pathname
			    (params.usock_file, dir, val,
			     usock_maxlen()) < 0) {
				LOG(log_error, logtype_cnid,
				    "usock filename %s too long", val);
				parse_err++;
			} else
