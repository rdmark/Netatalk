...
/* --------------- */
int didname(DB * dbp _U_, const DBT * pkey _U_, const DBT * pdata,
	    DBT * skey)
{
	int len;
...
/* --------------- */
int devino(DB * dbp _U_, const DBT * pkey _U_, const DBT * pdata,
	   DBT * skey)
{
	memset(skey, 0, sizeof(DBT));
...
/* --------------- */
int idxname(DB * dbp _U_, const DBT * pkey _U_, const DBT * pdata,
	    DBT * skey)
{
	...
	if (convert_charset(volinfo.v_volcharset,
			    ...
			    buffer, MAXPATHLEN, &flags) == (size_t) -1) {
