...
static int loop(struct db_param *dbp)
{
	...
	strftime(timebuf, 63, "%b %d %H:%M:%S.",
		 localtime(&time_next_flush));
	...
	while (1) {
		timeout =
		    min(time_next_flush,
			time_last_rqst + dbp->idle_timeout);
		if (timeout > now)
			timeout -= now;
		else
			timeout = 1;
...
static void switch_to_user(char *dir)
{
	...
	if (chdir(dir) < 0) {
		LOG(log_error, logtype_cnid, "chdir to %s failed: %s", dir,
		    strerror(errno));
		exit(1);
	}

	if (stat(".", &st) < 0) {
		LOG(log_error, logtype_cnid, "error in stat for %s: %s",
		    dir, strerror(errno));
		exit(1);
	}
	if (!getuid()) {
		...
		if (setgid(st.st_gid) < 0 || setuid(st.st_uid) < 0) {
			LOG(log_error, logtype_cnid, "uid/gid: %s",
			    strerror(errno));
			exit(1);
		}
	}
...
static void set_signal(void)
{
	...
	if (sigaction(SIGINT, &sv, NULL) < 0
	    || sigaction(SIGTERM, &sv, NULL) < 0) {
		LOG(log_error, logtype_cnid, "main: sigaction: %s",
		    strerror(errno));
		exit(1);
	}
...
int main(int argc, char *argv[])
{
	...
	if (loadvolinfo(argv[optind], &volinfo) == -1) {
		LOG(log_error, logtype_cnid,
		    "Cant load volinfo for \"%s\"", argv[1]);
		exit(EXIT_FAILURE);
	}
	...
	if ((strlen(volinfo.v_dbpath) + strlen("/.AppleDB")) > MAXPATHLEN) {
		LOG(log_error, logtype_cnid,
		    "CNID db pathname too long: \"%s\"", volinfo.v_dbpath);
		exit(EXIT_FAILURE);
	}
	...
	if (delete_bdb && (db_locked == LOCK_EXCL)) {
		LOG(log_warning, logtype_cnid,
		    "main: too many CNID db opening attempts, wiping the slate clean");
		if (chdir(dbpath) < 0)
	...
	if (dbif_env_open(dbd,
			  dbp,
			  (db_locked ==
			   LOCK_EXCL) ? DBOPTIONS | DB_RECOVER : DBOPTIONS)
	    < 0)
	...
	if (err)
		...
		LOG(log_info, logtype_cnid, "main: Exiting on signal %i",
		    exit_sig);
	else
