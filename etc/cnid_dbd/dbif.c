...
static int dbif_stamp(DBD * dbd, void *buffer, int size)
{
	...
	if ((cwd = open(".", O_RDONLY)) < 0) {
		LOG(log_error, logtype_cnid, "error opening cwd: %s",
		    strerror(errno));
		EC_FAIL;
	}
	...
	if ((chdir(dbd->db_envhome)) != 0) {
		LOG(log_error, logtype_cnid,
		    "error chdiring to db_env '%s': %s", dbd->db_envhome,
		    strerror(errno));
		EC_FAIL;
	}
	...
	if (stat(dbd->db_table[DBIF_CNID].name, &st) < 0) {
		LOG(log_error, logtype_cnid,
		    ...
		    dbd->db_table[DBIF_CNID].name, db_strerror(errno));
		EC_FAIL;
	}

	LOG(log_maxdebug, logtype_cnid, "stamp: %s",
	...
	if (cwd != -1) {
		if (fchdir(cwd) != 0) {
			LOG(log_error, logtype_cnid,
			    "error chdiring back: %s", strerror(errno));
			EC_STATUS(-1);
		}
		close(cwd);
	}
...
static int dbif_init_rootinfo(DBD * dbd, int version)
{
	...
	LOG(log_debug, logtype_cnid, "Setting CNID database version to %u",
	    version);
...
static int dbif_getversion(DBD * dbd, uint32_t * version)
{
	...
	*version = -1;
	...
	case 1:		/* found */
		memcpy(version, (char *) data.data + CNID_DID_OFS,
		       sizeof(uint32_t));
		*version = ntohl(*version);
		LOG(log_debug, logtype_cnid, "CNID database version %u",
		    *version);
		ret = 1;
		break;
	case 0:		/* not found */
...
static int dbif_logautorem(DBD * dbd)
{
	...
	if ((ret =
	     dbd->db_env->log_archive(dbd->db_env, &logfiles, 0)) != 0) {
		LOG(log_error, logtype_cnid,
...
int get_lock(int cmd, const char *dbpath)
{
	...
	case LOCK_SHRD:
		if (lockfd == -1) {
			if ((strlen(dbpath) + (strlen(LOCKFILENAME) + 1)) >
			    (PATH_MAX - 1)) {
				LOG(log_error, logtype_cnid,
			...
			if ((lockfd =
			     open(lockpath, O_RDWR | O_CREAT, 0644)) < 0) {
				LOG(log_error, logtype_cnid,
...
int dbif_env_open(DBD * dbd, struct db_param *dbp, uint32_t dbenv_oflags)
{
	...
	if ((ret = db_env_create(&dbd->db_env, 0))) {
		LOG(log_error, logtype_cnid,
		    "error creating DB environment: %s", db_strerror(ret));
		dbd->db_env = NULL;
		return -1;
	}
	...
	if (dbenv_oflags & DB_RECOVER) {
		...
		if ((ret =
		     dbd->db_env->open(dbd->db_env, dbd->db_envhome,
				       dbenv_oflags | DB_PRIVATE, 0))) {
			LOG(log_error, logtype_cnid,
	...
	if ((ret =
	     dbd->db_env->set_cachesize(dbd->db_env, 0,
					1024 * dbp->cachesize, 0))) {
		LOG(log_error, logtype_cnid,
	...
	if ((ret =
	     dbd->db_env->set_lk_max_locks(dbd->db_env, dbp->maxlocks))) {
		LOG(log_error, logtype_cnid,
	...
	if ((ret =
	     dbd->db_env->set_lk_max_objects(dbd->db_env,
					     dbp->maxlockobjs))) {
		LOG(log_error, logtype_cnid,
	...
	if ((ret =
	     dbd->db_env->open(dbd->db_env, dbd->db_envhome, dbenv_oflags,
			       0))) {
		LOG(log_error, logtype_cnid,
	...
	if (dbp->logfile_autoremove) {
		...
		if ((ret =
		     dbd->db_env->log_set_config(dbd->db_env,
...
#else
		if ((ret =
		     dbd->db_env->set_flags(dbd->db_env, DB_LOG_AUTOREMOVE,
					    1))) {
			LOG(log_error, logtype_cnid,
			    "error setting DB_LOG_AUTOREMOVE flag: %s",
			    db_strerror(ret));
			dbd->db_env->close(dbd->db_env, 0);
			dbd->db_env = NULL;
			return -1;
...
int dbif_open(DBD * dbd, struct db_param *dbp, int reindex)
{
	...
	if (dbd->db_envhome) {
		...
		if ((cwd = open(".", O_RDONLY)) < 0) {
			LOG(log_error, logtype_cnid,
			    "error opening cwd: %s", strerror(errno));
			return -1;
		}
		...
		if ((stat(dbd->db_filename, &st)) == 0) {
			...
			if ((ret =
			     dbd->db_env->txn_checkpoint(dbd->db_env, 0, 0,
							 DB_FORCE))) {
				LOG(log_error, logtype_cnid,
	...
	for (i = 0; i != DBIF_DB_CNT; i++) {
		if ((ret =
		     db_create(&dbd->db_table[i].db, dbd->db_env, 0))) {
			LOG(log_error, logtype_cnid,
		...
		if (dbd->db_table[i].flags) {
			if ((ret =
			     dbd->db_table[i].db->set_flags(dbd->
							    db_table[i].db,
							    dbd->
							    db_table[i].
							    flags))) {
		...
		if (!dbd->db_env) {	/* In memory db */
			if ((ret = dbd->db_table[i].db->set_cachesize(dbd->db_table[i].db, 0, dbp->cachesize, 4))	/* split in 4 memory chunks */
			    <0) {
				LOG(log_error, logtype_cnid,
		...
		if (reindex && i > 0) {
			...
			if ((ret =
			     dbd->db_table[i].db->truncate(dbd->
							   db_table[i].db,
							   NULL, &count,
							   0))) {
				LOG(log_error, logtype_cnid,
	...
	if ((ret =
	     dbd->db_table[0].db->associate(dbd->db_table[DBIF_CNID].db,
					    dbd->db_txn,
					    dbd->
					    db_table[DBIF_IDX_DIDNAME].db,
					    didname,
					    (reindex) ? DB_CREATE : 0))
	    != 0) {
		LOG(log_error, logtype_cnid,
...
static int dbif_closedb(DBD * dbd)
{
	...
	for (i = DBIF_DB_CNT - 1; i >= 0; i--) {
		if (dbd->db_table[i].db != NULL
		    && (ret =
			dbd->db_table[i].db->close(dbd->db_table[i].db,
						   0))) {
			LOG(log_error, logtype_cnid,
...
int dbif_close(DBD * dbd)
{
	...
	if (dbd->db_env != NULL
	    && (ret = dbd->db_env->close(dbd->db_env, 0))) {
		LOG(log_error, logtype_cnid,
		    "error closing DB environment: %s", db_strerror(ret));
		err++;
	}
	if (dbd->db_errlog != NULL && fclose(dbd->db_errlog) == EOF) {
		LOG(log_error, logtype_cnid,
		    "error closing DB logfile: %s", strerror(errno));
		err++;
	}
...
int dbif_env_remove(const char *path)
{
	...
	if (get_lock(LOCK_EXCL, path) != LOCK_EXCL) {
		LOG(log_debug, logtype_cnid,
		    "CNID db \"%s\" in use, can't remove BerkeleyDB environment",
		    path);
		return 0;
	}
	...
	if ((ret = dbd->db_env->open(dbd->db_env,
				     dbd->db_envhome,
				     DB_CREATE | DB_INIT_LOG |
				     DB_INIT_MPOOL | DB_INIT_LOCK |
				     DB_INIT_TXN | DB_RECOVER | DB_PRIVATE,
				     0))) {
		LOG(log_error, logtype_cnid,
		    "error opening DB environment: %s", db_strerror(ret));
		dbd->db_env->close(dbd->db_env, 0);
		dbd->db_env = NULL;
		return -1;
	}
	...
	/* Remove logfiles */
	if ((ret =
	     dbd->db_env->log_archive(dbd->db_env, NULL,
				      DB_ARCH_REMOVE))) {
		LOG(log_error, logtype_cnid,
	...
	if ((ret = db_env_create(&dbd->db_env, 0))) {
		LOG(log_error, logtype_cnid,
		    "error acquiring db_end handle: %s", db_strerror(ret));
		dbd->db_env = NULL;
		return -1;
	}
...
int dbif_get(DBD * dbd, const int dbi, DBT * key, DBT * val,
	     u_int32_t flags)
{
	int ret;

	ret = dbd->db_table[dbi].db->get(dbd->db_table[dbi].db,
					 dbd->db_txn, key, val, flags);
...
int dbif_search(DBD * dbd, DBT * key, char *resbuf)
{
	...
	/* Get a cursor */
	ret =
	    dbd->db_table[DBIF_IDX_NAME].db->cursor(dbd->
						    db_table
						    [DBIF_IDX_NAME].db,
						    NULL, &cursorp, 0);
	if (ret != 0) {
		LOG(log_error, logtype_cnid, "Couldn't create cursor: %s",
		    db_strerror(ret));
		ret = -1;
		goto exit;
	}
	...
	while (count < DBD_MAX_SRCH_RSLTS && ret != DB_NOTFOUND) {
		...
		cnids += sizeof(cnid_t);
		LOG(log_debug, logtype_cnid, "match: CNID %" PRIu32,
		    ntohl(cnid));
...
int dbif_txn_begin(DBD * dbd)
{
	...
	if (ret) {
		LOG(log_error, logtype_cnid,
		    "error starting transaction: %s", db_strerror(ret));
		return -1;
	} else
		return 0;
}
...
int dbif_txn_commit(DBD * dbd)
{
	...
	if (ret) {
		LOG(log_error, logtype_cnid,
		    "error committing transaction: %s", db_strerror(ret));
		return -1;
	} else
		return 1;
}
...
int dbif_txn_abort(DBD * dbd)
{
	...
	if (ret) {
		LOG(log_error, logtype_cnid,
		    "error aborting transaction: %s", db_strerror(ret));
		return -1;
	} else
		return 0;
}
...
int dbif_dump(DBD * dbd, int dumpindexes)
{
	...
	while (rc == 0) {
		...
		/* Rootinfo node ? */
		if (cnid == 0) {
			memcpy(&stamp, (char *) data.data + CNID_DEV_OFS,
			       sizeof(time_t));
			memcpy(&lastid, (char *) data.data + CNID_TYPE_OFS,
			       CNID_TYPE_LEN);
			lastid = ntohl(lastid);
			memcpy(&version, (char *) data.data + CNID_DID_OFS,
			       CNID_DID_LEN);
			...
			strftime(timebuf, sizeof(timebuf),
				 "%b %d %Y %H:%M:%S", localtime(&stamp));
			printf
			    ("CNID db version: %u, dbd stamp: 0x%08x (%s), next free CNID: %u\n",
			     version, (unsigned int) stamp, timebuf,
			     lastid + 1);
		} else {
			/* dev */
			...
			ino = ntoh64(ino);
			/* type */
			memcpy(&type, (char *) data.data + CNID_TYPE_OFS,
			       4);
			type = ntohl(type);
			/* did */
	...
	if (rc != DB_NOTFOUND) {
		LOG(log_error, logtype_cnid,
		    "Error iterating over btree: %s", db_strerror(rc));
		return -1;
	}

	rc = cur->c_close(cur);
	if (rc) {
		LOG(log_error, logtype_cnid, "Couldn't close cursor: %s",
		    db_strerror(rc));
		return -1;
	}
	...
	if (dumpindexes) {
		...
		if (rc) {
			LOG(log_error, logtype_cnid,
			    "Couldn't create cursor: %s", db_strerror(rc));
			return -1;
		}
		...
		while (rc == 0) {
			...
			if (cnid == 0) {
				...
				/* ino */
				memcpy(&ino,
				       (char *) key.data + CNID_DEV_LEN,
				       CNID_INO_LEN);
				ino = ntoh64(ino);

				printf
				    ("id: %10u <== dev: 0x%llx, ino: 0x%016llx\n",
				     cnid, (unsigned long long int) dev,
		...
		if (rc) {
			LOG(log_error, logtype_cnid,
			    "Couldn't close cursor: %s", db_strerror(rc));
			return -1;
		}
		printf("%u items\n", count);
