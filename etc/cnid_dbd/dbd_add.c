...
int add_cnid(DBD * dbd, struct cnid_dbd_rqst *rqst,
	...
	if ((rc = dbif_put(dbd, DBIF_CNID, &key, &data, DB_NOOVERWRITE))) {
		...
		case -1:
			...
			LOG(log_error, logtype_cnid,
			    "add_cnid: duplicate %x %s", rply->cnid,
...
int get_cnid(DBD * dbd, struct cnid_dbd_rply *rply)
{
	...
	if (id == 0) {
		if ((rc =
		     dbif_get(dbd, DBIF_CNID, &rootinfo_key,
			      &rootinfo_data, 0)) < 0) {
			rply->result = CNID_DBD_RES_ERR_DB;
			return -1;
		}
		if (rc == 0) {
			...
			memcpy(buf, (char *) rootinfo_data.data,
			       ROOTINFO_DATALEN);
...
int dbd_add(DBD * dbd, struct cnid_dbd_rqst *rqst,
	    struct cnid_dbd_rply *rply, int nolookup)
{
	rply->namelen = 0;
