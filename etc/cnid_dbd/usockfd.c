...
int usockfd_create(char *usock_fn, mode_t mode, int backlog)
{
	...
	if (unlink(usock_fn) < 0 && errno != ENOENT) {
		LOG(log_error, logtype_cnid,
		    "error unlinking unix socket file %s: %s", usock_fn,
		    strerror(errno));
		return -1;
	}
	...
	if (bind
		...
		LOG(log_error, logtype_cnid,
		    "error binding to socket for %s: %s", usock_fn,
		    strerror(errno));
		return -1;
	}
	...
	if (chmod(usock_fn, mode) < 0) {
		LOG(log_error, logtype_cnid,
		    "error changing permissions for %s: %s", usock_fn,
		    strerror(errno));
		close(sockfd);
		return -1;
	}
	...
	for (p = servinfo; p != NULL; p = p->ai_next) {
		if ((sockfd =
			...
			LOG(log_info, logtype_default,
			    "tsockfd_create: socket: %s", strerror(errno));
			continue;
		}
...
#ifdef SO_REUSEADDR
		flag = 1;
		setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &flag,
			   sizeof(flag));
#endif
...
#define SOL_TCP IPPROTO_TCP
#endif
		flag = 1;
		setsockopt(sockfd, SOL_TCP, TCP_NODELAY, &flag,
			   sizeof(flag));
	...
	if (p == NULL) {
		LOG(log_error, logtype_default,
		    "tsockfd_create: no suitable network config %s:%s",
		    host, port);
...
int usockfd_check(int sockfd, const sigset_t * sigset)
{
	...
	if ((ret =
	     pselect(sockfd + 1, &readfds, NULL, NULL, NULL,
		     sigset)) < 0) {
		if (errno == EINTR)
			return 0;
