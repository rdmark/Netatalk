...
static void set_signal(void)
{
	...
	if (sigaction(SIGTERM, &sv, NULL) < 0) {
		dbd_log(LOGSTD, "error in sigaction(SIGTERM): %s",
			strerror(errno));
		exit(EXIT_FAILURE);
	}
	if (sigaction(SIGINT, &sv, NULL) < 0) {
		dbd_log(LOGSTD, "error in sigaction(SIGINT): %s",
			strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	if (sigaction(SIGABRT, &sv, NULL) < 0) {
		dbd_log(LOGSTD, "error in sigaction(SIGABRT): %s",
			strerror(errno));
		exit(EXIT_FAILURE);
	}
	if (sigaction(SIGHUP, &sv, NULL) < 0) {
		dbd_log(LOGSTD, "error in sigaction(SIGHUP): %s",
			strerror(errno));
		exit(EXIT_FAILURE);
	}
	if (sigaction(SIGQUIT, &sv, NULL) < 0) {
		dbd_log(LOGSTD, "error in sigaction(SIGQUIT): %s",
			strerror(errno));
		exit(EXIT_FAILURE);
	}
}
...
int main(int argc, char **argv)
{
	int c;
	int ret = -1;
	int dump = 0, scan = 0, rebuild = 0, prep_upgrade =
	    0, rebuildindexes = 0, dumpindexes = 0;
	dbd_flags_t flags = 0;
	char *volpath;
	int cdir;
	...
	if (loadvolinfo(volpath, &volinfo) == -1) {
		dbd_log(LOGSTD,
			"Not a Netatalk volume at '%s', no .volinfo file at '%s/.AppleDesktop/.volinfo' or unknown volume options",
			volpath, volpath);
		exit(EXIT_FAILURE);
	}
	...
	if (volinfo.v_vfs_ea != AFPVOL_EA_AD
		...
		dbd_log(LOGSTD, "Unknown Extended Attributes option: %u",
			volinfo.v_vfs_ea);
		exit(EXIT_FAILURE);
	}
	...
	if (stat(volinfo.v_dbpath, &st) != 0) {
		if (errno != ENOENT) {
			dbd_log(LOGSTD, "Can't stat dbpath \"%s\": %s",
				volinfo.v_dbpath, strerror(errno));
			exit(EXIT_FAILURE);
		}
		if ((mkdir(volinfo.v_dbpath, 0755)) != 0) {
			dbd_log(LOGSTD, "Can't create dbpath \"%s\": %s",
	...
	strncpy(dbpath, volinfo.v_dbpath,
		MAXPATHLEN - strlen("/.AppleDB"));
	...
	if (dbdirfd == -1 && errno == ENOENT) {
		if (errno == ENOENT) {
			if ((mkdir(dbpath, 0755)) != 0) {
				dbd_log(LOGSTD,
					"Can't create .AppleDB for \"%s\": %s",
					dbpath, strerror(errno));
				exit(EXIT_FAILURE);
			}
		} else {
			dbd_log(LOGSTD,
				"Somethings wrong with .AppleDB for \"%s\", giving up: %s",
				dbpath, strerror(errno));
			exit(EXIT_FAILURE);
		}
	} else {
	...
	if ((flags & DBD_FLAGS_FORCE) && rebuild
		...
		dbd_log(LOGDEBUG, "Removing old database of volume: '%s'",
			volpath);
		if (system(cmd) < 0)
			dbd_log(LOGSTD, "Could not execute \"%s\" (%s)",
				cmd, strerror(errno));
		if ((mkdir(dbpath, 0755)) != 0) {
			dbd_log(LOGSTD, "Can't create dbpath \"%s\": %s",
				dbpath, strerror(errno));
	...
	if (!nocniddb) {
		...
		if (dbif_env_open(dbd,
				  &db_param,
				  (db_locked ==
				   LOCK_EXCL) ? (DBOPTIONS | DB_RECOVER) :
				  DBOPTIONS) < 0) {
	...
	if (dbif_env_remove(dbpath) < 0) {
		dbd_log(LOGSTD,
			"Error removing BerkeleyDB database environment");
		ret++;
	}
	get_lock(0, NULL);
