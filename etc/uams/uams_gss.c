		       ...
		       OM_uint32 minor_status)
{
	...
	while (1) {
		maj_status = gss_display_status(&min_status, major_status,
						GSS_C_GSS_CODE,
						GSS_C_NULL_OID, &maj_ctx,
						&msg);
		LOG(log_info, logtype_uams,
		    "uams_gss.c :do_gss_auth: %s %.*s (error %s)", s,
	...
	while (1) {
		maj_status = gss_display_status(&min_status, minor_status, GSS_C_MECH_CODE, GSS_C_NULL_OID,	// gss_mech_krb5,
						&min_ctx, &msg);
		LOG(log_info, logtype_uams,
		    "uams_gss.c :do_gss_auth: %s %.*s (error %s)", s,
		    (int) msg.length, msg.value, strerror(errno));
...
static void log_ctx_flags(OM_uint32 flags)
{
	if (flags & GSS_C_DELEG_FLAG)
		LOG(log_debug, logtype_uams,
		    "uams_gss.c :context flag: GSS_C_DELEG_FLAG");
	if (flags & GSS_C_MUTUAL_FLAG)
		LOG(log_debug, logtype_uams,
		    "uams_gss.c :context flag: GSS_C_MUTUAL_FLAG");
	if (flags & GSS_C_REPLAY_FLAG)
		LOG(log_debug, logtype_uams,
		    "uams_gss.c :context flag: GSS_C_REPLAY_FLAG");
	if (flags & GSS_C_SEQUENCE_FLAG)
		LOG(log_debug, logtype_uams,
		    "uams_gss.c :context flag: GSS_C_SEQUENCE_FLAG");
	if (flags & GSS_C_CONF_FLAG)
		LOG(log_debug, logtype_uams,
		    "uams_gss.c :context flag: GSS_C_CONF_FLAG");
	if (flags & GSS_C_INTEG_FLAG)
		LOG(log_debug, logtype_uams,
		    "uams_gss.c :context flag: GSS_C_INTEG_FLAG");
}
...
static void log_principal(gss_name_t server_name)
{
	...
	major_status =
	    gss_export_name(&minor_status, server_name, &exported_name);
	LOG(log_debug, logtype_uams,
	    "log_principal: exported server name is %s",
	    (char *) exported_name.value);
...
static int get_afpd_principal(void *obj, gss_name_t * server_name)
{
	...
	if (uam_afpserver_option
	    (obj, UAM_OPTION_FQDN, (void *) &fqdn, &fqdnlen) < 0)
		return 1;
	LOG(log_debug, logtype_uams, "get_afpd_principal: fqdn: %s", fqdn);

	if (uam_afpserver_option
	    (obj, UAM_OPTION_KRB5SERVICE, (void *) &service,
	     &servicelen) < 0)
		return 1;
	LOG(log_debug, logtype_uams, "get_afpd_principal: service: %s",
	    service);
	...
	LOG(log_debug, logtype_uams,
	    "get_afpd_principal: importing principal `%s'", principal);
	major_status =
	    gss_import_name(&minor_status, &s_princ_buffer,
...
/* get the username */
static int get_client_username(char *username, int ulen,
			       gss_name_t * client_name)
{
	...
	LOG(log_debug, logtype_uams, "get_client_username: user is `%s'",
	...
	if (namelen >= ulen) {
		...
		LOG(log_error, logtype_uams,
		    "get_client_username: username `%s' too long",
		    client_name_buffer.value);
		ret = 1;
	} else {
...
static int wrap_sessionkey(gss_ctx_id_t context,
			   struct session_info *sinfo)
{
	...
	gss_wrap(&status, context, 1, GSS_C_QOP_DEFAULT, &sesskey_buff,
		 NULL, &wrap_buff);

	if (status != GSS_S_COMPLETE) {
		LOG(log_error, logtype_uams,
		    "wrap_sessionkey: failed to gss_wrap sessionkey");
	...
	if (NULL == (sinfo->cryptedkey = malloc(wrap_buff.length))) {
		...
		memcpy(sinfo->cryptedkey, wrap_buff.value,
		       wrap_buff.length);
...
/*-------------*/
static int acquire_credentials(gss_name_t * server_name,
			      ...
			      gss_buffer_desc * authenticator_buff)
{
	...
	LOG(log_debug, logtype_uams,
	    "accept_context: accepting context (ticketlen: %u)",
	...
	if (major_status != GSS_S_COMPLETE) {
		log_status("accept_sec_context", major_status,
			   minor_status);
		return 1;
	}
		       ...
		       struct session_info *sinfo)
{
	...
	ret =
	    accept_sec_context(&context_handle, server_creds,
			       &ticket_buffer, &client_name,
			       &authenticator_buff);
	...
	if (!ret) {
		...
		if (0 ==
		    (ret =
		     get_client_username(username, ulen, &client_name))) {
		...
		if (!ret) {
			...
			u_int16_t auth_len =
			    htons(authenticator_buff.length);
			...
			*rbuflen += sizeof(auth_len);
			...
			memcpy(rbuf, authenticator_buff.value,
			       authenticator_buff.length);
			 ...
			 char *rbuf, size_t *rbuflen)
{
	...
	if (ibuflen < 1 + sizeof(login_id)) {
		LOG(log_info, logtype_uams,
		    "uams_gss.c :LoginCont: received incomplete packet");
		return AFPERR_PARAM;
	}
	ibuf++, ibuflen--;	/* ?? */
	...
	if (ibuflen < 4) {
		LOG(log_info, logtype_uams,
		    "uams_gss.c :LoginCont: user is %s, no ticket", p);
		return AFPERR_PARAM;
	}
	...
	if ((ibuf - p + 1) % 2)
		ibuf++, ibuflen--;	/* deal with potential padding */

	LOG(log_debug, logtype_uams,
	    "uams_gss.c :LoginCont: client thinks user is %s", p);
	...
	if (ticket_len > ibuflen) {
		LOG(log_info, logtype_uams,
		    "uams_gss.c :LoginCont: invalid ticket length (%u > %u)",
		    ticket_len, ibuflen);
		return AFPERR_PARAM;
	}
	...
	if (!do_gss_auth
	    (obj, ibuf, ticket_len, rbuf, &rblen, username, userlen,
	     sinfo)) {
		...
		if ((pwd = uam_getname(obj, username, userlen)) == NULL) {
			LOG(log_info, logtype_uams,
			    "uam_getname() failed for %s", username);
			return AFPERR_NOTAUTH;
		}
		if (uam_checkuser(pwd) < 0) {
			LOG(log_info, logtype_uams, "%s not a valid user",
			    username);
			return AFPERR_NOTAUTH;
		}
		*rbuflen = rblen;
...
int uam_setup(const char *path)
{
	if (uam_register(UAM_SERVER_LOGIN_EXT, path, "Client Krb v2",
			 gss_login, gss_logincont, gss_logout,
			 gss_login_ext) < 0)
		if (uam_register
		    (UAM_SERVER_LOGIN, path, "Client Krb v2", gss_login,
		     gss_logincont, gss_logout) < 0)
			return -1;
