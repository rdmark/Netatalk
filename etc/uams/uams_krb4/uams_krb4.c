		      ...
		      char *rbuf, size_t *rbuflen)
{
	...
	if (uam_afpserver_option
	    (obj, UAM_OPTION_USERNAME, &username, &ulen) < 0)
		return AFPERR_MISC;

	if (uam_afpserver_option(obj, UAM_OPTION_PROTOCOL, &proto, NULL) <
	    0)
		return AFPERR_MISC;
	...
	case KRB4CMD_SESS:
		...
		if (tkt.length <= 0 || tkt.length > MAX_KTXT_LEN) {
			*rbuflen = 0;
			LOG(log_info, logtype_default,
			    "krb4_login: tkt.length = %d", tkt.length);
		...
		if ((rc = krb_rd_req(&tkt, princ, inst, 0, &ad, ""))
			...
			LOG(log_error, logtype_default,
			    "krb4_login: krb_rd_req(): %s",
			    krb_err_txt[rc]);
			*rbuflen = 0;
			return (AFPERR_BADUAM);
		}

		LOG(log_info, logtype_default, "krb4_login: %s.%s@%s",
			  ...
			  char *rbuf, size_t *rbuflen)
{
	...
	if (uam_afpserver_option
	    (obj, UAM_OPTION_HOSTNAME, &servername, &snlen) < 0)
		return AFPERR_MISC;
	...
	default:
		...
		case KRB4WRT_SESS:
			...
			if ((rc = krb_rd_req(&tkt, "afpserver", servername,
					     0, &ad, "")) != RD_AP_OK) {
				LOG(log_error, logtype_default,
				    "krb4_logincont: krb_rd_req(): %s",
				    krb_err_txt[rc]);
				return (AFPERR_BADUAM);
			}

			LOG(log_info, logtype_default,
			    "krb4_login: %s.%s@%s", ad.pname, ad.pinst,
			    ad.prealm);
			...
			*rbuflen = 1;
			...
			if ((pwd =
			     uam_getname(obj, ad.pname, strlen(ad.pname)))
			    == NULL) {
				return (AFPERR_PARAM);
			}
			*uam_pwd = pwd;
		...
		default:
			LOG(log_info, logtype_default,
			    "krb4_logincont: bad command %d", rc);
			*rbuflen = 0;
			return (AFPERR_NOTAUTH);
			break;
		}
