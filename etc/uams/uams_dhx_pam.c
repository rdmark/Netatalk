...
#define COPY_STRING(s) (s) ? strdup(s) : NULL
	...
	if (num_msg < 1) {
		...
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM DHX Conversation Err -- %s",
		    strerror(errno));
		/* Log Entry */
		return PAM_CONV_ERR;
	}
	...
	for (count = 0; count < num_msg; count++) {
		...
		case PAM_PROMPT_ECHO_ON:
			if (!
			    (string =
			     COPY_STRING((const char *) PAM_username))) {
				/* Log Entry */
				LOG(log_info, logtype_uams,
				    "uams_dhx_pam.c :PAM: username failure -- %s",
				    strerror(errno));
				/* Log Entry */
				goto pam_fail_conv;
			}
		...
		case PAM_PROMPT_ECHO_OFF:
			if (!
			    (string =
			     COPY_STRING((const char *) PAM_password))) {
				/* Log Entry */
				LOG(log_info, logtype_uams,
				    "uams_dhx_pam.c :PAM: passwd failure: --: %s",
				    strerror(errno));
				/* Log Entry */
				goto pam_fail_conv;
			}
		...
		default:
			/* Log Entry */
			LOG(log_info, logtype_uams,
			    "uams_dhx_pam.c :PAM: Binary_Prompt -- %s",
			    strerror(errno));
			/* Log Entry */
			goto pam_fail_conv;
		}
		     ...
		     unsigned char *rbuf, size_t *rbuflen)
{
	...
	if (!(bn = BN_bin2bn(ibuf, KEYSIZE, NULL))) {
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM No Public Key -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	if (!(gbn = BN_bin2bn(&g, sizeof(g), NULL))) {
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM No Primes: GBN -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	if (!(pbn = BN_bin2bn(p, sizeof(p), NULL))) {
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM No Primes: PBN -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	if (!(dh = DH_new())) {
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM DH was equal to DH_New... Go figure... -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	if (!DH_set0_pqg(dh, pbn, NULL, gbn)) {
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM DH_set0_pqg() mysteriously failed  -- %s",
		    strerror(errno));
		/* Log Entry */
		goto pam_fail;
	}
	...
	if (DH_generate_key(dh) == 0) {
		...
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: Err Generating Key (OpenSSL error code: %u, %s)",
		    dherror, errbuf);
	...
	if (BN_num_bytes(pub_key) > KEYSIZE) {
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: Err Generating Key -- Not enough Space? -- %s",
		    strerror(errno));
		goto pam_fail;
	}
	...
	*rbuflen += KEYSIZE;
	...
	if (uam_afpserver_option(obj, UAM_OPTION_RANDNUM, (void *) randbuf,
		...
		*rbuflen = 0;
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: Buffer Encryption Err. -- %s",
		    strerror(errno));
		/* Log Entry */
		goto pam_fail;
	}
	...
	*rbuflen += CRYPTBUFLEN;
	...
	/* Log Entry */
	LOG(log_info, logtype_uams,
	    "uams_dhx_pam.c :PAM: Fail - Cast Encryption -- %s",
	    strerror(errno));
	/* Log Entry */
	return AFPERR_PARAM;
}
			 ...
			 unsigned char *rbuf, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if (PAM_error != PAM_SUCCESS) {
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: PAM_Error: %s",
...
static int pam_changepw(void *obj, unsigned char *username,
			...
			size_t ibuflen, unsigned char *rbuf,
			size_t *rbuflen)
{
	BIGNUM *bn1, *bn2, *bn3;
	...
	if (uam_afpserver_option(obj, UAM_OPTION_HOSTNAME,
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: Hostname Null?? -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_MISC;
	}
	...
	if (!(bn1 = BN_bin2bn(ibuf, KEYSIZE, NULL))) {
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: Random Number Not the same or not incremented-- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	if (!(bn3 = BN_new())) {
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: Random Number did not Zero -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	if (!BN_is_one(bn3)) {
		...
		/* Log Entry */
		LOG(log_info, logtype_uams,
		    "uams_dhx_pam.c :PAM: After Random Number not Zero, is it one more? -- %s",
		    strerror(errno));
		/* Log Entry */
		return AFPERR_PARAM;
	}
	...
	PAM_error =
	    pam_start("netatalk", (char *) username, &PAM_conversation,
		      &lpamh);
	if (PAM_error != PAM_SUCCESS) {
		/* Log Entry */
		LOG(log_info, logtype_uams,
...
{
	if (uam_register
	    (UAM_SERVER_LOGIN_EXT, path, "DHCAST128", pam_login,
