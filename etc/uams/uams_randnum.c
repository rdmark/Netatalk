		       ...
		       unsigned char *passwd, const int len, const int set)
{
	...
	if (!S_ISREG(st.st_mode) || (pwd->pw_uid != st.st_uid) ||
		...
		LOG(log_info, logtype_uams,
		    "Insecure permissions found for %s.", path);
		goto home_passwd_fail;
	}
...
static int afppasswd(const struct passwd *pwd,
		     ...
		     unsigned char *passwd, int len, const int set)
{
	...
	if (keyfd > -1) {
		...
		if (set) {
			...
			DES_ecb_encrypt((DES_cblock *) passwd,
					(DES_cblock *) passwd, &schedule,
					DES_ENCRYPT);
		} else {
...
static int rand_login(void *obj, char *username, int ulen,
		      struct passwd **uam_pwd _U_, char *ibuf _U_,
		      size_t ibuflen _U_, char *rbuf, size_t *rbuflen)
{
	...
	LOG(log_info, logtype_uams, "randnum/rand2num login: %s",
	    username);
			    ...
			    size_t *rbuflen _U_)
{
	...
	if (!err)
		err =
		    randpass(pwd, passwdfile,
			     (unsigned char *) ibuf + PASSWDLEN,
			     sizeof(seskey), 1);
...
/* randnum login ext */
static int randnum_login_ext(void *obj, char *uname,
			     struct passwd **uam_pwd, char *ibuf,
...
static int uam_setup(const char *path)
{
	if (uam_register(UAM_SERVER_LOGIN_EXT, path, "Randnum exchange",
			 randnum_login, randnum_logincont, NULL,
			 randnum_login_ext) < 0)
		return -1;

	if (uam_register
	    (UAM_SERVER_LOGIN_EXT, path, "2-Way Randnum exchange",
	     randnum_login, rand2num_logincont, NULL,
	     randnum_login_ext) < 0) {
