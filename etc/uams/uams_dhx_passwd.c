...
static int pwd_login(void *obj, char *username, int ulen,
		     struct passwd **uam_pwd _U_, char *ibuf,
		     size_t ibuflen _U_, char *rbuf, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	if ((sp = getspnam(dhxpwd->pw_name)) == NULL) {
		LOG(log_info, logtype_uams,
		    "no shadow passwd entry for %s", username);
		return AFPERR_NOTAUTH;
	}
	...
	*rbuflen += KEYSIZE;
	...
	/* encrypt using cast */
	CAST_cbc_encrypt((unsigned char *) rbuf, (unsigned char *) rbuf,
			 CRYPTBUFLEN, &castkey, iv, CAST_ENCRYPT);
	*rbuflen += CRYPTBUFLEN;
	BN_free(bn);
	DH_free(dh);
			    ...
			    char *rbuf, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	...
	CAST_cbc_encrypt((unsigned char *) ibuf, (unsigned char *) rbuf,
			 CRYPT2BUFLEN, &castkey, iv, CAST_DECRYPT);
...
static int uam_setup(const char *path)
{
	if (uam_register(UAM_SERVER_LOGIN_EXT, path, "DHCAST128",
			 passwd_login, passwd_logincont, NULL,
			 passwd_login_ext) < 0)
		return -1;
