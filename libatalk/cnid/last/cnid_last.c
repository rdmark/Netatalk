...
cnid_t cnid_last_add(struct _cnid_db *cdb, const struct stat *st,
		     const cnid_t did _U_, const char *name _U_,
		     const size_t len _U_, cnid_t hint _U_)
{
	...
	if (S_ISDIR(st->st_mode))
		...
		return htonl((st->st_dev << 16) |
			     (st->st_ino & 0x0000ffff));
}
...
cnid_t cnid_last_get(struct _cnid_db *cdb _U_, const cnid_t did _U_,
...
/* */
cnid_t cnid_last_lookup(struct _cnid_db *cdb _U_,
			const struct stat *st _U_, const cnid_t did _U_,
			char *name _U_, const size_t len _U_)
{
...
static struct _cnid_db *cnid_last_new(const char *volpath)
{
	...
	if ((cdb->_private =
	     calloc(1, sizeof(struct _cnid_last_private))) == NULL) {
		free(cdb->volpath);
		free(cdb);
		return NULL;
	}
...
int cnid_last_update(struct _cnid_db *cdb _U_, const cnid_t id _U_,
		     const struct stat *st _U_, const cnid_t did _U_,
		     char *name _U_, const size_t len _U_)
{
	return 0;
