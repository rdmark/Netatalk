	...
	for (p = servinfo; p != NULL; p = p->ai_next) {
		if ((sock =
			...
			LOG(log_info, logtype_default,
			    "tsock_getfd: socket CNID server %s:: %s",
			    host, strerror(errno));
			continue;
		}
		...
		if (setnonblock(sock, 1) != 0) {
			LOG(log_error, logtype_cnid,
			    "getfd: setnonblock: %s", strerror(errno));
			close(sock);
			sock = -1;
			return -1;
		}
		...
		if (connect(sock, p->ai_addr, p->ai_addrlen) == -1) {
			if (errno == EINPROGRESS) {
				...
				if ((err =
				     select(sock + 1, NULL, &wfds, NULL,
					    &tv)) == 0) {
					/* timeout */
					LOG(log_error, logtype_cnid,
					    "getfd: select timed out for CNID server %s",
					    host);
					close(sock);
					sock = -1;
					continue;
				...
				if (err == -1) {
					/* select failed */
					LOG(log_error, logtype_cnid,
					    "getfd: select failed for CNID server %s",
					    host);
					close(sock);
					sock = -1;
					continue;
				...
				if (!FD_ISSET(sock, &wfds)) {
					/* give up */
					LOG(log_error, logtype_cnid,
					    "getfd: socket not ready connecting to %s",
					    host);
					close(sock);
					sock = -1;
					continue;
				...
				if ((err =
					...
					if (err != 0) {
						...
						LOG(log_error,
						    logtype_cnid,
						    "getfd: getsockopt error with CNID server %s: %s",
						    host, strerror(errno));
					} else {
						errno = optval;
						LOG(log_error,
						    logtype_cnid,
						    "getfd: getsockopt says: %s",
						    strerror(errno));
					}
					close(sock);
					sock = -1;
...
static int write_vec(int fd, struct iovec *iov, ssize_t towrite, int vecs)
{
	...
	while (1) {
		...
		if (len == -1)
			LOG(log_error, logtype_cnid, "write_vec: %s",
			    strerror(errno));
		else
			LOG(log_error, logtype_cnid,
			    "write_vec: short write: %d", len);
		return len;
	}
...
static int init_tsock(CNID_private * db)
{
	...
	LOG(log_debug, logtype_cnid,
	    "init_tsock: BEGIN. Opening volume '%s', CNID Server: %s/%s",
	...
	if (write_vec(fd, iov, len + sizeof(int), 2) != len + sizeof(int)) {
		LOG(log_error, logtype_cnid,
		    "init_tsock: Error/short write: %s", strerror(errno));
		close(fd);
		return -1;
	}
...
static int send_packet(CNID_private * db, struct cnid_dbd_rqst *rqst)
{
	...
	if (write_vec(db->fd, iov, towrite, vecs) != towrite) {
		LOG(log_warning, logtype_cnid,
		    "send_packet: Error writev rqst (db_dir %s): %s",
 ...
 */
static int dbd_rpc(CNID_private * db, struct cnid_dbd_rqst *rqst,
		   struct cnid_dbd_rply *rply)
{
	...
	ret =
	    readt(db->fd, rply, sizeof(struct cnid_dbd_rply), 0,
		  ONE_DELAY);

	if (ret != sizeof(struct cnid_dbd_rply)) {
		LOG(log_debug, logtype_cnid,
		    "dbd_rpc: Error reading header from fd (db_dir %s): %s",
	...
	if (rply->namelen
	    && (ret =
		readt(db->fd, rply->name, rply->namelen, 0,
		      ONE_DELAY)) != (ssize_t) rply->namelen) {
		LOG(log_error, logtype_cnid,
		    "dbd_rpc: Error reading name from fd (db_dir %s): %s",
		    db->db_dir, ret == -1 ? strerror(errno) : "closed");
		return -1;
	}
...
/* -------------------- */
static int transmit(CNID_private * db, struct cnid_dbd_rqst *rqst,
		    struct cnid_dbd_rply *rply)
{
	time_t orig, t;
	...
	while (1) {
		if (db->fd == -1) {
			...
			LOG(log_maxdebug, logtype_cnid,
			    "transmit: connecting to cnid_dbd ...");
			...
			if (db->notfirst) {
				LOG(log_debug7, logtype_cnid,
				    "transmit: reconnected to cnid_dbd, comparing database stamps...");
				if (memcmp
				    (stamp, db->stamp, ADEDLEN_PRIVSYN)) {
					LOG(log_error, logtype_cnid,
					    "transmit: ... not the same db!");
					db->changed = 1;
					return -1;
				}
				LOG(log_debug7, logtype_cnid,
				...
				db->notfirst = 1;
				if (db->client_stamp)
					memcpy(db->client_stamp, stamp,
					       ADEDLEN_PRIVSYN);
				memcpy(db->stamp, stamp, ADEDLEN_PRIVSYN);
			}
			LOG(log_debug, logtype_cnid,
			    "transmit: attached to '%s', stamp: '%08lx'.",
		...
		if (errno == ECONNREFUSED) {	/* errno carefully injected in tsock_getfd */
			/* give up */
			LOG(log_error, logtype_cnid,
			    "transmit: connection refused (db_dir %s)",
			    db->db_dir);
			return -1;
		}
		...
		if (!clean) {	/* don't sleep if just got disconnected by cnid server */
			...
			if (t - orig > MAX_DELAY) {
				LOG(log_error, logtype_cnid,
				    "transmit: Request to dbd daemon (db_dir %s) timed out.",
				    db->db_dir);
				return -1;
			}
...
struct _cnid_db *cnid_dbd_open(struct cnid_open_args *args)
{
	...
	if ((db =
	     (CNID_private *) calloc(1, sizeof(CNID_private))) == NULL) {
		LOG(log_error, logtype_cnid,
		    "cnid_open: Unable to allocate memory for database");
	...
	LOG(log_debug, logtype_cnid,
	    "cnid_dbd_open: Finished initializing cnid dbd module for volume '%s'",
	    db->db_dir);
		    ...
		    cnid_t hint)
{
	...
	if (len > MAXPATHLEN) {
		LOG(log_error, logtype_cnid,
		    "cnid_add: Path name is too long");
		errno = CNID_ERR_PATH;
		return CNID_INVALID;
	}
	...
	LOG(log_debug, logtype_cnid,
	    "cnid_dbd_add: CNID: %u, name: '%s', inode: 0x%llx, type: %d (0=file, 1=dir)",
	...
	case CNID_DBD_RES_OK:
		id = rply.cnid;
		LOG(log_debug, logtype_cnid, "cnid_dbd_add: got CNID: %u",
		    ntohl(id));
		break;
		    ...
		    const size_t len)
{
	...
	if (len > MAXPATHLEN) {
		LOG(log_error, logtype_cnid,
		    "cnid_dbd_get: Path name is too long");
		errno = CNID_ERR_PATH;
		return CNID_INVALID;
	}

	LOG(log_debug, logtype_cnid, "cnid_dbd_get: DID: %u, name: '%s'",
	    ntohl(did), name);
	...
	case CNID_DBD_RES_OK:
		id = rply.cnid;
		LOG(log_debug, logtype_cnid, "cnid_dbd_get: got CNID: %u",
		    ntohl(id));
		break;
	case CNID_DBD_RES_NOTFOUND:
		id = CNID_INVALID;
		       ...
		       size_t len)
{
	...
	if (!cdb || !(db = cdb->_private) || !id || !(*id)) {
		LOG(log_error, logtype_cnid,
		    "cnid_resolve: Parameter error");
		errno = CNID_ERR_PARAM;
		return NULL;
	}

	LOG(log_debug, logtype_cnid,
	    "cnid_dbd_resolve: resolving CNID: %u", ntohl(*id));
	...
	case CNID_DBD_RES_OK:
		*id = rply.did;
		...
		LOG(log_debug, logtype_cnid,
		    "cnid_dbd_resolve: resolved did: %u, name: '%s'",
		    ntohl(*id), name);
		break;
	case CNID_DBD_RES_NOTFOUND:
		*id = CNID_INVALID;
...
/* ---------------------- */
cnid_t cnid_dbd_lookup(struct _cnid_db *cdb, const struct stat *st,
		       const cnid_t did, char *name, const size_t len)
{
	...
	if (len > MAXPATHLEN) {
		LOG(log_error, logtype_cnid,
		    "cnid_lookup: Path name is too long");
		errno = CNID_ERR_PATH;
		return CNID_INVALID;
	}
	...
	case CNID_DBD_RES_OK:
		id = rply.cnid;
		LOG(log_debug, logtype_cnid,
		    "cnid_dbd_lookup: got CNID: %u", ntohl(id));
		break;
	case CNID_DBD_RES_NOTFOUND:
		id = CNID_INVALID;
		break;
		  ...
		  void *buffer, size_t buflen)
{
	...
	if (namelen > MAXPATHLEN) {
		LOG(log_error, logtype_cnid,
		    "cnid_find: Path name is too long");
		errno = CNID_ERR_PATH;
		return CNID_INVALID;
	}
	...
	case CNID_DBD_RES_OK:
		...
		LOG(log_debug, logtype_cnid, "cnid_find: got %d matches",
		    count);
		break;
	case CNID_DBD_RES_NOTFOUND:
		count = 0;
...
/* ---------------------- */
int cnid_dbd_update(struct _cnid_db *cdb, const cnid_t id,
			    ...
			    cnid_t hint)
{
	...
	if (!cdb || !(db = cdb->_private) || !st || !name
	    || hint == CNID_INVALID) {
		LOG(log_error, logtype_cnid,
		    "cnid_rebuild_add: Parameter error");
		errno = CNID_ERR_PARAM;
		return CNID_INVALID;
	}
	...
	if (len > MAXPATHLEN) {
		LOG(log_error, logtype_cnid,
		    "cnid_rebuild_add: Path name is too long");
		errno = CNID_ERR_PATH;
		return CNID_INVALID;
	}
	...
	case CNID_DBD_RES_OK:
		id = rply.cnid;
		LOG(log_debug, logtype_cnid,
		    "cnid_dbd_rebuild_add: got CNID: %u", ntohl(id));
		break;
	case CNID_DBD_RES_ERR_MAX:
		errno = CNID_ERR_MAX;
		id = CNID_INVALID;
...
int cnid_dbd_delete(struct _cnid_db *cdb, const cnid_t id)
{
	...
	if (!cdb || !(db = cdb->_private) || !id) {
		LOG(log_error, logtype_cnid,
		    "cnid_delete: Parameter error");
		errno = CNID_ERR_PARAM;
		return -1;
	}

	LOG(log_debug, logtype_cnid, "cnid_dbd_delete: delete CNID: %u",
	    ntohl(id));
	...
	case CNID_DBD_RES_OK:
		LOG(log_debug, logtype_cnid,
		    "cnid_dbd_delete: deleted CNID: %u", ntohl(id));
	case CNID_DBD_RES_NOTFOUND:
		return 0;
	case CNID_DBD_RES_ERR_DB:
		errno = CNID_ERR_DB;
