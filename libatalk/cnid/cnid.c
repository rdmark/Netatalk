...
void cnid_register(struct _cnid_module *module)
{
	...
	list_for_each(ptr, &modules)
	    if (0 ==
		strcmp(list_entry(ptr, cnid_module, db_list)->name,
		       module->name)) {
		LOG(log_error, logtype_afpd,
		    "Module with name [%s] is already registered !",
		    module->name);
		return;
	}

	LOG(log_info, logtype_afpd, "Registering CNID module [%s]",
	    module->name);
	ptr = &(module->db_list);
...
static int cnid_dir(const char *dir, mode_t mask)
{
	...
	if (stat(dir, &st) < 0) {
		...
		LOG(log_info, logtype_cnid, "Setting uid/gid to %d/%d",
		    st.st_uid, st.st_gid);
			   ...
			   const char *cnidport)
{
	...
	struct cnid_open_args args =
	    { volpath, mask, flags, cnidsrv, cnidport };
	...
	if (NULL == db) {
		LOG(log_error, logtype_afpd,
		    "Cannot open CNID db at [%s].", volpath);
		return NULL;
	}
...
static cnid_t valide(cnid_t id)
{
	...
	if (id < CNID_START) {
		...
		if (!err) {
			err = 1;
			LOG(log_error, logtype_afpd,
			    "Error: Invalid cnid, corrupted DB?");
		}
		return CNID_INVALID;
	}
	return id;
...
void cnid_close(struct _cnid_db *db)
{
	...
	if (NULL == db) {
		LOG(log_error, logtype_afpd,
		    "Error: cnid_close called with NULL argument !");
		return;
	}
	/* cnid_close free db */
	flags = db->flags;
	      ...
	      void *buffer, size_t buflen)
{
	...
	if (cdb->cnid_find == NULL) {
		LOG(log_error, logtype_cnid,
		    "cnid_find not supported by CNID backend");
		return -1;
	}
		   ...
		   size_t len)
{
	...
	if (ret && !strcmp(ret, "..")) {
		LOG(log_error, logtype_afpd,
		    "cnid_resolve: name is '..', corrupted db? ");
		ret = NULL;
	}
	return ret;
}
