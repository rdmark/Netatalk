		 ...
		 const struct vol *vol, void *data, int flag)
{
	...
	if (NULL == (dp = opendir(name))) {
		if (!flag) {
			LOG(log_error, logtype_afpd, "%s: opendir %s: %s",
			    from, fullpathname(name), strerror(errno));
			return -1;
		}
		return 0;
	}
...
/* ----------------- */
static int deletecurdir_adouble_loop(const struct vol *vol,
				     struct dirent *de, char *name,
				     void *data _U_, int flag _U_)
{
	struct stat st;
	int err;
...
static int RF_deletecurdir_adouble(VFS_FUNC_ARGS_DELETECURDIR)
{
	...
	/* delete stray .AppleDouble files. this happens to get .Parent files
	   as well. */
	if ((err =
	     for_each_adouble("deletecurdir", ".AppleDouble",
...
{
	return adouble_setfilmode(vol, vol->ad_path(name, ADFLAGS_HF),
				  mode, st);
}
...
static int RF_setdirunixmode_adouble(VFS_FUNC_ARGS_SETDIRUNIXMODE)
{
	...
	if (dir_rx_set(mode)) {
		if (stickydirmode
		    (ad_dir(adouble), DIRBITS | mode, dropbox,
		     vol->v_umask) < 0)
			return -1;
	}

	if (adouble_setfilmode
	    (vol, vol->ad_path(name, ADFLAGS_DIR), mode, st) < 0)
		return -1;
...
/* ----------------- */
static int setdirmode_adouble_loop(const struct vol *vol,
				   struct dirent *de _U_, char *name,
				   void *data, int flag)
{
...
static int RF_setdirmode_adouble(VFS_FUNC_ARGS_SETDIRMODE)
{
	...
	if (for_each_adouble
	    ("setdirmode", adouble_p, setdirmode_adouble_loop, vol,
	     &hf_mode, vol_noadouble(vol)))
		return -1;
...
/* ----------------- */
static int setdirowner_adouble_loop(const struct vol *vol,
				    struct dirent *de _U_, char *name,
				    void *data, int flag _U_)
{
	struct perm *owner = data;

	if (chown(name, owner->uid, owner->gid) < 0 && errno != EPERM) {
		LOG(log_debug, logtype_afpd,
		    "setdirowner: chown %d/%d %s: %s", owner->uid,
		    owner->gid, fullpathname(name), strerror(errno));
...
static int RF_setdirowner_adouble(VFS_FUNC_ARGS_SETDIROWNER)
{
	...
	if (for_each_adouble
	    ("setdirowner", adouble_p, setdirowner_adouble_loop, vol,
	     &owner, noadouble))
		return -1;
	...
	if (gid && gid != st.st_gid && chown(".AppleDouble", uid, gid) < 0
	    && errno != EPERM) {
		LOG(log_debug, logtype_afpd,
		    "setdirowner: chown %d/%d %s: %s", uid, gid,
		    fullpathname(".AppleDouble"), strerror(errno));
		/* return ( -1 ); Sometimes this is okay */
	}
	return 0;
...
static int RF_renamefile_adouble(VFS_FUNC_ARGS_RENAMEFILE)
{
	...
	if (unix_rename(dirfd, adsrc, -1, vol->ad_path(dst, 0)) < 0) {
		...
		if (errno == ENOENT) {
			...
			if (!ad_open
			    (dst, ADFLAGS_HF, O_RDWR | O_CREAT, 0666,
			     &ad)) {
 ...
 * sfm adouble format
 *********************************************************************************/
static int ads_chown_loop(const struct vol *vol, struct dirent *de _U_,
			  char *name, void *data, int flag _U_)
{
	struct perm *owner = data;
...
static int RF_chown_ads(VFS_FUNC_ARGS_CHOWN)
{
	...
	return for_each_adouble("chown_ads", ad_p, ads_chown_loop, vol,
				&owner, 1);
}

/* --------------------------------- */
static int deletecurdir_ads1_loop(const struct vol *vol _U_,
				  struct dirent *de _U_, char *name,
				  void *data _U_, int flag _U_)
{
...
static int ads_delete_rf(char *name)
{
	...
	if ((err =
	     for_each_adouble("deletecurdir", name, deletecurdir_ads1_loop,
			      NULL, NULL, 1)))
		return err;
	/* FIXME 
...
static int deletecurdir_ads_loop(const struct vol *vol, struct dirent *de,
				 char *name, void *data _U_, int flag _U_)
{
	struct stat st;
...
static int RF_deletecurdir_ads(VFS_FUNC_ARGS_DELETECURDIR)
{
	...
	if ((err =
	     for_each_adouble("deletecurdir", ".AppleDouble",
			      deletecurdir_ads_loop, vol, NULL, 1)))
		return err;
...
static int ads_setfilmode_loop(const struct vol *vol,
			       struct dirent *de _U_, char *name,
			       void *data, int flag _U_)
{
...
static int ads_setfilmode(const struct vol *vol, const char *name,
			  mode_t mode, struct stat *st)
{
	...
	if (for_each_adouble
	    ("setfilmode_ads", name, ads_setfilmode_loop, vol, &param,
	     0) < 0)
		return -1;
...
{
	return ads_setfilmode(vol, ad_dir(vol->ad_path(name, ADFLAGS_HF)),
			      mode, st);
}
...
static int RF_setdirunixmode_ads(VFS_FUNC_ARGS_SETDIRUNIXMODE)
{
	...
	if (dir_rx_set(mode)) {
		...
		/* .AppleDouble */
		if (stickydirmode
		    (ad_dir(ad_p), DIRBITS | mode, dropbox,
		     vol->v_umask) < 0)
			return -1;

		/* .AppleDouble/.Parent */
		if (stickydirmode
		    (ad_p, DIRBITS | mode, dropbox, vol->v_umask) < 0)
			return -1;
	}

	if (ads_setfilmode
	    (vol, ad_dir(vol->ad_path(name, ADFLAGS_DIR)), mode, st) < 0)
		return -1;
...
static int setdirmode_ads_loop(const struct vol *vol,
			       struct dirent *de _U_, char *name,
			       void *data, int flag)
{
	...
	if (dir_rx_set(param->mode)) {
		if (stickydirmode
		    (name, DIRBITS | param->mode, param->dropbox,
		     vol->v_umask) < 0) {
			if (flag) {
				return 0;
			}
...
static int RF_setdirmode_ads(VFS_FUNC_ARGS_SETDIRMODE)
{
	...
	if (dir_rx_set(mode)) {
		/* .AppleDouble */
		if (stickydirmode
		    (ad_dir(ad_p), DIRBITS | mode, param.dropbox,
		     vol->v_umask) < 0)
			return -1;
	}

	if (for_each_adouble
	    ("setdirmode_ads", ad_dir(ad_p), setdirmode_ads_loop, vol,
	     &param, vol_noadouble(vol)))
		return -1;
...
/* ------------------- */
static int setdirowner_ads1_loop(const struct vol *vol _U_,
				 struct dirent *de _U_, char *name,
				 void *data, int flag _U_)
{
	struct perm *owner = data;
...
static int setdirowner_ads_loop(const struct vol *vol,
				struct dirent *de _U_, char *name,
				void *data, int flag)
{
	struct perm *owner = data;
	...
	if (for_each_adouble
	    ("setdirowner", name, setdirowner_ads1_loop, vol, data,
	     flag) < 0)
		return -1;
...
static int RF_setdirowner_ads(VFS_FUNC_ARGS_SETDIROWNER)
{
	...
	strlcpy(adouble_p, ad_dir(vol->ad_path(name, ADFLAGS_DIR)),
		sizeof(adouble_p));

	if (for_each_adouble
	    ("setdirowner", ad_dir(adouble_p), setdirowner_ads_loop, vol,
	     &owner, noadouble))
		return -1;
...
static int RF_deletefile_ads(VFS_FUNC_ARGS_DELETEFILE)
{
	...
	if (dirfd != -1 && fchdir(cwd) != 0) {
		LOG(log_error, logtype_afpd,
		    "RF_deletefile_ads: cant chdir back. exit!");
		exit(EXITERR_SYS);
	}
...
{
	return strncmp(name, "._", 2) && ((vol->
					   v_flags & AFPVOL_USEDOTS) ?
					  netatalk_name(name) : name[0] !=
					  '.');
}
...
static int RF_renamedir_osx(VFS_FUNC_ARGS_RENAMEDIR)
{
	...
	char tempbuf[258] = "._";
	return unix_rename(dirfd, vol->ad_path(oldpath, 0), -1,
			   strcat(tempbuf, newpath));
}
...
{
	return adouble_setfilmode(vol, vol->ad_path(name, ADFLAGS_DIR),
				  mode, st);
}
...
#define VFS_MFUNC(name, args, vars) \
    ...
    VFS_MFUNC(setdirowner, VFS_FUNC_ARGS_SETDIROWNER,
...
static int vfs_validupath(VFS_FUNC_ARGS_VALIDUPATH)
{
	return vol->vfs_modules[0]->
	    vfs_validupath(VFS_FUNC_VARS_VALIDUPATH);
}
