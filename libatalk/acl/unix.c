	...
	*retAces = NULL;
	...
	if (lstat(name, &st) != 0) {
		LOG(log_debug, logtype_afpd,
		    "get_nfsv4_acl(\"%s/%s\"): %s", getcwdpath(), name,
		    strerror(errno));
		return -1;
	}
	...
	if (!(S_ISREG(st.st_mode) || S_ISDIR(st.st_mode))) {
		LOG(log_debug, logtype_afpd,
		    "get_nfsv4_acl(\"%s/%s\"): special", getcwdpath(),
		    name);
		return 0;
	}
	...
	if ((ace_count = acl(name, ACE_GETACLCNT, 0, NULL)) == 0) {
		LOG(log_debug, logtype_afpd,
		    "get_nfsv4_acl(\"%s/%s\"): 0 ACEs", getcwdpath(),
		    name);
		return 0;
	}
	...
	if (aces == NULL) {
		LOG(log_error, logtype_afpd,
		    "get_nfsv4_acl: malloc error");
		return -1;
	}

	if ((acl(name, ACE_GETACL, ace_count, aces)) == -1) {
		LOG(log_error, logtype_afpd,
		    "get_nfsv4_acl: acl(ACE_GETACL) error");
		free(aces);
		return -1;
	}

	LOG(log_debug9, logtype_afpd,
	    "get_nfsv4_acl: file: %s -> No. of ACEs: %d", name, ace_count);
	*retAces = aces;
...
/*
  Concatenate ACEs
*/
ace_t *concat_aces(ace_t * aces1, int ace1count, ace_t * aces2,
		   int ace2count)
{
	ace_t *new_aces;
	int i, j;
	...
	if ((new_aces =
	     malloc((ace1count + ace2count) * sizeof(ace_t))) == NULL) {
		LOG(log_error, logtype_afpd, "combine_aces: malloc %s",
		    strerror(errno));
		return NULL;
	}
...
int strip_trivial_aces(ace_t ** saces, int sacecount)
{
	...
	for (i = 0; i < sacecount;) {
		if (!
		    (aces[i].
		     a_flags & (ACE_OWNER | ACE_GROUP | ACE_EVERYONE)))
			nontrivaces++;
		i++;
	}
	/* malloc buffer for new ACL */
	if ((new_aces = malloc(nontrivaces * sizeof(ace_t))) == NULL) {
		LOG(log_error, logtype_afpd,
		    "strip_trivial_aces: malloc %s", strerror(errno));
		return -1;
	}
	...
	*saces = new_aces;
	...
	LOG(log_debug7, logtype_afpd,
	    "strip_trivial_aces: non-trivial ACEs: %d", nontrivaces);
...
int strip_nontrivial_aces(ace_t ** saces, int sacecount)
{
	...
	if ((new_aces = malloc(trivaces * sizeof(ace_t))) == NULL) {
		LOG(log_error, logtype_afpd,
		    "strip_nontrivial_aces: malloc %s", strerror(errno));
		return -1;
	}
	...
	*saces = new_aces;
	...
	LOG(log_debug7, logtype_afpd,
	    "strip_nontrivial_aces: trivial ACEs: %d", trivaces);
...
int nfsv4_chmod(char *name, mode_t mode)
{
	...
	if ((ret = acl(name, ACE_SETACL, noaces + nnaces, cacl)) != 0) {
		if (errno != EACCES) {
			LOG(log_error, logtype_afpd,
			    "nfsv4_chmod: error setting acl: %s",
			    strerror(errno));
			goto exit;
		}
		become_root();
	...
	LOG(log_debug, logtype_afpd,
	    "nfsv4_chmod(\"%s/%s\", %04o): result: %d", getcwdpath(), name,
	    mode, ret);
...
int posix_chmod(const char *name, mode_t mode)
{
	...
	LOG(log_maxdebug, logtype_afpd, "posix_chmod: %s mode: 0x%08x",
	    name, mode);
	...
	if (acl) {
		...
		while ((acl_get_entry(acl, entry_id, &entry) == 1)
			...
			if (ret) {
				LOG(log_error, logtype_afpd,
				    "posix_chmod: Failed to get tag type.");
				goto cleanup;
			}
		...
		if (!not_found) {
			...
			if (ret) {
				LOG(log_error, logtype_afpd,
				    "posix_chmod: acl_calc_mask failed.");
				goto cleanup;
			}
...
int posix_fchmod(int fd, mode_t mode)
{
	...
	if (acl) {
		...
		while ((acl_get_entry(acl, entry_id, &entry) == 1)
			...
			if (ret) {
				LOG(log_error, logtype_afpd,
				    "posix_fchmod: Failed to get tag type.");
				goto cleanup;
			}
		...
		if (!not_found) {
			...
			if (ret) {
				LOG(log_error, logtype_afpd,
				    "posix_fchmod: Can't get permset.");
				goto cleanup;
			}
			ret = acl_clear_perms(permset);
			...
			if (ret) {
				LOG(log_error, logtype_afpd,
				    "posix_fchmod: Can't set permset.");
				goto cleanup;
			}
			/* also update ACL_MASK */
			ret = acl_calc_mask(&acl);
			...
			if (ret) {
				LOG(log_error, logtype_afpd,
				    "posix_fchmod: acl_calc_mask failed.");
				goto cleanup;
			}
