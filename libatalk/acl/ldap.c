						  ...
						  char **result)
{
	...
	if (ld == NULL) {
		...
		if ((ld = ldap_init(ldap_server, LDAP_PORT)) == NULL) {
			LOG(log_error, logtype_default,
			    "ldap: ldap_init error: %s", strerror(errno));
			return -1;
		}
		if (ldap_set_option
	...
	if (!ldapconnected) {
		if (LDAP_AUTH_NONE == ldap_auth_method) {
			if (ldap_bind_s(ld, "", "", LDAP_AUTH_SIMPLE) !=
			    LDAP_SUCCESS) {
				LOG(log_error, logtype_default,
				    "ldap: ldap_bind failed, auth_method: \'%d\'",
				    ldap_auth_method);
				free(ld);
				ld = NULL;
				return -1;
	...
	LOG(log_maxdebug, logtype_afpd,
	    "ldap: start search: base: %s, filter: %s, attr: %s",
	    searchbase, filter, attributes[0]);

	/* start LDAP search */
	ldaperr =
	    ldap_search_st(ld, searchbase, scope, filter, attributes, 0,
			   &timeout, &msg);
	LOG(log_maxdebug, logtype_default,
	    "ldap: ldap_search_st returned: %s", ldap_err2string(ldaperr));
	if (ldaperr != LDAP_SUCCESS) {
		LOG(log_error, logtype_default,
		    "ldap: ldap_search_st failed: %s, retrycount: %i",
	...
	/* parse search result */
	LOG(log_maxdebug, logtype_default,
	    "ldap: got %d entries from ldap search", ldap_count_entries(ld,
									msg));
	...
	*result = strdup(attribute_values[0]);
	if (*result == NULL) {
		LOG(log_error, logtype_default, "ldap: strdup error: %s",
		    strerror(errno));
		ret = -1;
		goto cleanup;
	}
			 ...
			 char **uuid_string)
{
	...
	if (len >= 256 || len == -1) {
		LOG(log_error, logtype_default,
		    "ldap_getnamefromuuid: filter error:%d, \"%s\"", len,
		    filter);
		return -1;
	}
	...
	if (type == UUID_GROUP) {
		ret =
		    ldap_getattr_fromfilter_withbase_scope(ldap_groupbase,
							   filter,
							   attributes,
							   ldap_groupscope,
							   KEEPALIVE,
		...
		ret =
		    ldap_getattr_fromfilter_withbase_scope(ldap_userbase,
							   filter,
							   attributes,
							   ldap_userscope,
							   KEEPALIVE,
 ...
 */
int ldap_getnamefromuuid(const char *uuidstr, char **name,
			 uuidtype_t * type)
{
	int ret;
	int len;
	...
	if (len >= 256 || len == -1) {
		LOG(log_error, logtype_default,
		    "ldap_getnamefromuuid: filter overflow:%d, \"%s\"",
		    len, filter);
		return -1;
	}
	...
	ret =
	    ldap_getattr_fromfilter_withbase_scope(ldap_groupbase, filter,
						   attributes,
						   ldap_groupscope,
						   KEEPALIVE, name);
	if (ret == -1)
	...
	ret =
	    ldap_getattr_fromfilter_withbase_scope(ldap_userbase, filter,
						   attributes,
						   ldap_userscope,
						   KEEPALIVE, name);
	if (ret == 1) {
