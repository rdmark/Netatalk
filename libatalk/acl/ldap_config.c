...
int acl_ldap_readconfig(char *name)
{
	...
	while (!feof(f)) {
		...
		if (ldap_prefs[i].pref == NULL)
			LOG(log_error, logtype_afpd,
			    "afp_ldap.conf: Unknown option: \"%s\"", pref);
	}			/*  EOF */
	...
	while (ldap_prefs[i].pref != NULL) {
		if (ldap_prefs[i].valid != 0) {
			LOG(log_debug, logtype_afpd,
			    "afp_ldap.conf: Missing option: \"%s\"",
			    ldap_prefs[i].name);
			ldap_config_valid = 0;
			break;
		}
	...
	if (ldap_config_valid) {
		if (ldap_auth_method == LDAP_AUTH_NONE)
			LOG(log_debug, logtype_afpd,
			    "afp_ldap.conf: Using anonymous bind.");
		else if (ldap_auth_method == LDAP_AUTH_SIMPLE)
			LOG(log_debug, logtype_afpd,
			    "afp_ldap.conf: Using simple bind.");
		else {
			ldap_config_valid = 0;
			LOG(log_error, logtype_afpd,
			    "afp_ldap.conf: SASL not yet supported.");
		}
	} else
