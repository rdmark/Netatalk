		...
		if ((entry = namecache[i]) != NULL) {
			do {
				...
				LOG(log_debug, logtype_default,
				    ...
				    (entry->type & UUID_ENOENT) ==
				    UUID_ENOENT ? "[negative]" : "",
				    uuidtype[entry->
					     type & UUIDTYPESTR_MASK],
				    timestr);
	...
	while (entry) {
		...
		if (ret == 0 && *type == (entry->type & UUIDTYPESTR_MASK)) {
			...
			if ((tim - entry->creationtime) > CACHESECONDS) {
				LOG(log_debug, logtype_default,
				    "search_cachebyname: expired: name:\"%s\"",
				    entry->name);
				/* remove item */
				if (entry->prev) {
...
int search_cachebyuuid(uuidp_t uuidp, char **name, uuidtype_t * type)
{
	...
	while (entry) {
		...
		if (ret == 0) {
			...
			if ((tim - entry->creationtime) > CACHESECONDS) {
				LOG(log_debug, logtype_default,
				    "search_cachebyuuid: expired: name:\'%s\' in queue {%d}",
				    entry->name, hash);
				if (entry->prev) {
					/* 2nd to last in queue */
					entry->prev->next = entry->next;
