	...
	while (1) {
		...
		if (dsi->eof < dsi->end) {
			...
			if (!warned) {
				...
				LOG(log_note, logtype_dsi,
				    "dsi_peek: dsireadbuf: %d, DSI quantum: %d, effective buffer size: %d",
				    dsi->dsireadbuf,
				    dsi->server_quantum ? dsi->
				    server_quantum : DSI_SERVQUANT_DEF,
				    dsi->end - dsi->buffer);
			}
		}
		...
		if ((ret =
		     select(maxfd, &readfds, &writefds, NULL,
			    NULL)) <= 0) {
			...
			LOG(log_error, logtype_dsi,
			    "dsi_peek: unexpected select return: %d %s",
		...
		if (FD_ISSET(dsi->socket, &readfds)) {
			...
			if ((len = read(dsi->socket, dsi->eof, len)) <= 0) {
				...
				LOG(log_error, logtype_dsi,
				    "dsi_peek: read: %s", strerror(errno));
				if (errno == EAGAIN)
					continue;
				return -1;
			}
			LOG(log_debug, logtype_dsi,
			    "dsi_peek: read %d bytes", len);
...
static size_t from_buf(DSI * dsi, u_int8_t * buf, size_t count)
{
	...
	LOG(log_debug, logtype_dsi,
	    ...
	    dsi->start - dsi->buffer, dsi->eof - dsi->start,
	    dsi->end - dsi->eof, nbe);
...
static ssize_t buf_read(DSI * dsi, u_int8_t * buf, size_t count)
{
	...
	LOG(log_maxdebug, logtype_dsi, "buf_read(%u bytes): got: %d",
	    count, len);
 ...
 */
static size_t dsi_buffered_stream_read(DSI * dsi, u_int8_t * data,
				       const size_t length)
{
	size_t len;
	size_t buflen;
	...
	LOG(log_maxdebug, logtype_dsi,
	    "dsi_buffered_stream_read: %u bytes", length);
			 ...
			 int mode)
{
	...
	LOG(log_maxdebug, logtype_dsi,
	    "dsi_stream_write(send: %zd bytes): START", length);
	...
	while (written < length) {
		len =
		    send(dsi->socket, (u_int8_t *) data + written,
			 length - written, flags);
		if (len >= 0) {
			written += len;
			continue;
		}
		...
		if (errno == EAGAIN || errno == EWOULDBLOCK) {
			LOG(log_debug, logtype_dsi,
			    "dsi_stream_write: send: %s", strerror(errno));
		...
		LOG(log_error, logtype_dsi, "dsi_stream_write: %s",
		    strerror(errno));
		written = -1;
		goto exit;
	}
	...
	LOG(log_maxdebug, logtype_dsi,
	    "dsi_stream_write(send: %zd bytes): END", length);
...
*/
#ifdef WITH_SENDFILE
ssize_t dsi_stream_read_file(DSI * dsi, int fromfd, off_t offset,
			     const size_t length)
{
	int ret = 0;
	size_t written;
	...
	LOG(log_maxdebug, logtype_dsi,
	    "dsi_stream_read_file(send %zd bytes): START", length);
	...
	while (written < length) {
		len =
		    sys_sendfile(dsi->socket, fromfd, &pos,
				 length - written);
		...
		if (len < 0) {
			...
			LOG(log_error, logtype_dsi,
			    "dsi_stream_read_file: %s", strerror(errno));
			break;
		} else if (!len) {
	...
	dsi->write_count += written;
      ...
      exit:
	dsi->in_write--;
	LOG(log_maxdebug, logtype_dsi, "dsi_stream_read_file: sent: %zd",
	    written);
	if (ret != 0)
		return -1;
	return written;
...
size_t dsi_stream_read(DSI * dsi, void *data, const size_t length)
{
	...
	LOG(log_maxdebug, logtype_dsi, "dsi_stream_read(%u bytes)",
	    length);

	stored = 0;
	while (stored < length) {
		len =
		    buf_read(dsi, (u_int8_t *) data + stored,
			     length - stored);
		if (len == -1 && (errno == EINTR || errno == EAGAIN)) {
			LOG(log_maxdebug, logtype_dsi,
			    "dsi_stream_read: select read loop");
			continue;
		} else if (len > 0) {
			stored += len;
		} else {	/* eof or error */
	...
	LOG(log_maxdebug, logtype_dsi,
	    "dsi_stream_read(%u bytes): got: %u", length, stored);
	return stored;
}
...
int dsi_stream_send(DSI * dsi, void *buf, size_t length)
{
	...
	LOG(log_maxdebug, logtype_dsi, "dsi_stream_send(%u bytes): START",
	    length);
	...
	memcpy(block + 4, &dsi->header.dsi_code,
	       sizeof(dsi->header.dsi_code));
	memcpy(block + 8, &dsi->header.dsi_len,
	       sizeof(dsi->header.dsi_len));
	memcpy(block + 12, &dsi->header.dsi_reserved,
	       sizeof(dsi->header.dsi_reserved));

	if (!length) {		/* just write the header */
		LOG(log_maxdebug, logtype_dsi,
		    "dsi_stream_send(%u bytes): DSI header, no data",
		    sizeof(block));
		length =
	...
	while (towrite > 0) {
		if (((len = writev(dsi->socket, iov, 2)) == -1
		     && errno == EINTR) || (len == 0))
			continue;
		...
		if ((size_t) len == towrite)	/* wrote everything out */
			...
			LOG(log_error, logtype_dsi, "dsi_stream_send: %s",
			    strerror(errno));
			unblock_sig(dsi);
			return 0;
		}
	...
	LOG(log_maxdebug, logtype_dsi, "dsi_stream_send(%u bytes): END",
	    length);
...
int dsi_stream_receive(DSI * dsi)
{
	...
	/* read in the header */
	if (dsi_buffered_stream_read
	    (dsi, (u_int8_t *) block, sizeof(block)) != sizeof(block))
		return 0;
	...
	memcpy(&dsi->header.dsi_requestID, block + 2,
	       sizeof(dsi->header.dsi_requestID));
	memcpy(&dsi->header.dsi_code, block + 4,
	       sizeof(dsi->header.dsi_code));
	memcpy(&dsi->header.dsi_len, block + 8,
	       sizeof(dsi->header.dsi_len));
	...
	if (dsi_stream_read(dsi, dsi->commands, dsi->cmdlen) !=
	    dsi->cmdlen)
		return 0;

	LOG(log_debug, logtype_dsi, "dsi_stream_receive: DSI cmdlen: %zd",
	    dsi->cmdlen);
