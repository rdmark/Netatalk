			    ...
			    int tickleval)
{
	...
	if (socketpair(PF_UNIX, SOCK_STREAM, 0, ipc_fds) < 0) {
		LOG(log_error, logtype_dsi, "dsi_getsess: %s",
		    strerror(errno));
		exit(EXITERR_CLNT);
	}

	if (setnonblock(ipc_fds[0], 1) != 0
	...
	default:		/* parent */
		...
		close(ipc_fds[1]);
		if ((child =
		     server_child_add(serv_children, CHILD_DSIFORK, pid,
				      ipc_fds[0])) == NULL) {
			LOG(log_error, logtype_dsi, "dsi_getsess: %s",
			    strerror(errno));
			close(ipc_fds[0]);
	...
	case DSIFUNC_OPEN:	/* setup session */
		...
		dsi->timer.it_interval.tv_sec =
		    dsi->timer.it_value.tv_sec = tickleval;
		dsi->timer.it_interval.tv_usec =
		    dsi->timer.it_value.tv_usec = 0;
		signal(SIGPIPE, SIG_IGN);	/* we catch these ourselves */
		dsi_opensession(dsi);
		return NULL;

	default:		/* just close */
		LOG(log_info, logtype_dsi, "DSIUnknown %d",
