...
ucs2_t *strcasechr_sp(const ucs2_t * s, u_int32_t c_sp)
{
	...
	while (s[1] != 0) {
		if (tolower_sp(c_sp) ==
		    tolower_sp((u_int32_t) * s << 16 | (u_int32_t) s[1]))
			return (ucs2_t *) s;
		s++;
	}
...
ucs2_t *strcasestr_w(const ucs2_t * s, const ucs2_t * ins)
{
	...
	if ((0xD800 <= *ins) && (*ins < 0xDC00)) {
		if ((0xDC00 <= ins[1]) && (ins[1] < 0xE000)) {
			u_int32_t ins_sp =
			    (u_int32_t) * ins << 16 | (u_int32_t) ins[1];
			while ((r = strcasechr_sp(r, ins_sp))) {
				if (strncasecmp_w(r, ins, inslen) == 0)
					return r;
				r++;
			}
		} else {
...
int strcasecmp_w(const ucs2_t * a, const ucs2_t * b)
{
	...
	while (*a && *b) {
		if ((0xD800 <= *a) && (*a < 0xDC00)) {
			...
			b++;
			if (!(*a && *b))
				return (tolower_w(*a) - tolower_w(*b));	/* avoid buffer over run */
		} else {
			if ((ret = tolower_w(*a) - tolower_w(*b)))
				return ret;
		}
		a++;
		b++;
...
static ucs2_t do_precomposition(unsigned int base, unsigned int comb)
{
	...
	while (max >= min) {
		...
		that =
		    (precompositions[mid].
		     base << 16) | (precompositions[mid].comb);
		if (that < sought) {
			min = mid + 1;
		} else if (that > sought) {
			max = mid - 1;
...
/* ------------------------ */
static u_int32_t do_precomposition_sp(unsigned int base_sp,
				      unsigned int comb_sp)
{
	...
	int mid;
	u_int64_t sought_sp =
	    ((u_int64_t) base_sp << 32) | (u_int64_t) comb_sp, that_sp;

	/* binary search */
	while (max >= min) {
		mid = (min + max) / 2;
		that_sp =
		    ((u_int64_t) precompositions_sp[mid].
		     base_sp << 32) | ((u_int64_t) precompositions_sp[mid].
				       comb_sp);
...
static u_int32_t do_decomposition(ucs2_t base)
{
	...
	while (max >= min) {
		...
		if (that < sought) {
			...
			result =
			    (decompositions[mid].
			     base << 16) | (decompositions[mid].comb);
			return result;
		}
	}
	/* no match */
...
static u_int64_t do_decomposition_sp(unsigned int base_sp)
{
	...
	while (max >= min) {
		...
		if (that_sp < sought_sp) {
			...
			result_sp =
			    ((u_int64_t) decompositions_sp[mid].
			     base_sp << 32) | ((u_int64_t)
					       decompositions_sp[mid].
					       comb_sp);
			return result_sp;
		}
	}
...
size_t precompose_w(ucs2_t * name, size_t inplen, ucs2_t * comp,
		    size_t *outlen)
{
	size_t i;
	ucs2_t base, comb;
	...
	while (*outlen > 2) {
		...
		/* Binary Search for Surrogate Pair */
			...
			if ((0xDC00 <= comb) && (comb < 0xE000)
			    && (i + 6 <= inplen)) {
				base_sp =
				    ((u_int32_t) base << 16) | (u_int32_t)
				    comb;
				do {
					comb_sp =
					    ((u_int32_t) in[1] << 16) |
...
/* --------------- */
size_t decompose_w(ucs2_t * name, size_t inplen, ucs2_t * comp,
		   size_t *outlen)
{
	size_t i;
	size_t comblen;
...
size_t utf8_charlen(char *utf8)
{
	...
	if (*p < 0x80)
		return (1);
	else if (*p > 0xC1 && *p < 0xe0 && *(p + 1) > 0x7f
		 && *(p + 1) < 0xC0)
		return (2);
	else if (*p == 0xe0 && *(p + 1) > 0x9f && *(p + 1) < 0xc0
		 && *(p + 2) > 0x7f && *(p + 2) < 0xc0)
		return (3);
	else if (*p > 0xe0 && *p < 0xf0 && *(p + 1) > 0x7f
		 && *(p + 1) < 0xc0 && *(p + 2) > 0x7f && *(p + 2) < 0xc0)
		return (3);
	else if (*p == 0xf0 && *(p + 1) > 0x8f && *(p + 1) < 0xc0
		 && *(p + 2) > 0x7f && *(p + 2) < 0xc0 && *(p + 3) > 0x7f
		 && *(p + 3) < 0xc0)
		return (4);
	else if (*p > 0xf0 && *p < 0xf4 && *(p + 1) > 0x7f
		 && *(p + 1) < 0xc0 && *(p + 2) > 0x7f && *(p + 2) < 0xc0
		 && *(p + 3) > 0x7f && *(p + 3) < 0xc0)
		return (4);
	else if (*p == 0xf4 && *(p + 1) > 0x7f && *(p + 1) < 0x90
		 && *(p + 2) > 0x7f && *(p + 2) < 0xc0 && *(p + 3) > 0x7f
		 && *(p + 3) < 0xc0)
		return (4);
	else
