...
static size_t mac_hebrew_pull(void *, char **, size_t *, char **,
			      size_t *);
static size_t mac_hebrew_push(void *, char **, size_t *, char **,
			      size_t *);
...
static size_t mac_hebrew_push(void *cd _U_, char **inbuf,
			      ...
			      size_t *outbytesleft)
{
	...
	while (*inbytesleft >= 2 && *outbytesleft >= 1) {
		...
		if (inptr == 0x05b8) {
			...
			*tmpptr = c;
		} else if (inptr == 0x05f2 && *inbytesleft >= 4
			   && SVAL((*inbuf), 2) == 0x05b7) {
			(*inbuf) += 4;
			(*inbytesleft) -= 4;
			*tmpptr = 0x81;
		} else if (inptr == 0xf86a && *inbytesleft >= 6
			   && SVAL((*inbuf), 2) == 0x05dc
			   && SVAL((*inbuf), 4) == 0x05b9) {
			(*inbuf) += 6;
			(*inbytesleft) -= 6;
...
static size_t mac_hebrew_pull(void *cd _U_, char **inbuf,
			      size_t *inbytesleft, char **outbuf,
			      size_t *outbytesleft)
{
	ucs2_t temp;
