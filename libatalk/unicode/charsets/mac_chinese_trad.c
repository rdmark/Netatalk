...
static size_t mac_chinese_trad_pull(void *, char **, size_t *, char **,
				    size_t *);
static size_t mac_chinese_trad_push(void *, char **, size_t *, char **,
				    size_t *);
...
static size_t mac_chinese_trad_char_push(u_int8_t * out, const ucs2_t * in,
					 size_t *size)
{
	ucs2_t wc = in[0];
	...
	if (wc <= 0x7f) {
		...
		*size = 1;
		...
		ucs2_t comp =
		    cjk_compose(wc, in[1], mac_chinese_trad_compose,
				sizeof(mac_chinese_trad_compose) /
				sizeof(u_int32_t));
		if (comp) {
			wc = comp;
			*size = 2;
	...
	return cjk_char_push(cjk_lookup(wc, mac_chinese_trad_uni2_index,
					mac_chinese_trad_uni2_charset),
			     out);
}

static size_t mac_chinese_trad_push(void *cd, char **inbuf,
...
static size_t mac_chinese_trad_char_pull(ucs2_t * out, const u_int8_t * in,
					 size_t *size)
{
	u_int16_t c = in[0];
...
static size_t mac_chinese_trad_pull(void *cd, char **inbuf,
				    size_t *inbytesleft, char **outbuf,
				    size_t *outbytesleft)
{
