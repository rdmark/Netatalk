...
static size_t mac_chinese_simp_pull(void *, char **, size_t *, char **,
				    size_t *);
static size_t mac_chinese_simp_push(void *, char **, size_t *, char **,
				    size_t *);
...
struct charset_functions charset_mac_chinese_simp = {
	...
	CHARSET_ICONV | CHARSET_MULTIBYTE | CHARSET_PRECOMPOSED |
	    CHARSET_CLIENT,
	"EUC-CN",
	NULL, NULL
};

static size_t mac_chinese_simp_char_push(u_int8_t * out, const ucs2_t * in,
					 size_t *size)
{
	ucs2_t wc = in[0];
	...
	if (wc <= 0x7f) {
		...
		*size = 1;
		...
		ucs2_t comp =
		    cjk_compose(wc, in[1], mac_chinese_simp_compose,
				sizeof(mac_chinese_simp_compose) /
				sizeof(u_int32_t));
		if (comp) {
			wc = comp;
			*size = 2;
	...
	return cjk_char_push(cjk_lookup(wc, mac_chinese_simp_uni2_index,
					mac_chinese_simp_uni2_charset),
			     out);
}

static size_t mac_chinese_simp_push(void *cd, char **inbuf,
				    size_t *inbytesleft, char **outbuf,
				    size_t *outbytesleft)
{
	return cjk_generic_push(mac_chinese_simp_char_push,
				cd, inbuf, inbytesleft, outbuf,
				outbytesleft);
}

static size_t mac_chinese_simp_char_pull(ucs2_t * out, const u_int8_t * in,
					 size_t *size)
{
	u_int16_t c = in[0];
...
static size_t mac_chinese_simp_pull(void *cd, char **inbuf,
				    size_t *inbytesleft, char **outbuf,
				    size_t *outbytesleft)
{
