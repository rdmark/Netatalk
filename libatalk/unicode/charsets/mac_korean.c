...
static size_t mac_korean_pull(void *, char **, size_t *, char **,
			      size_t *);
static size_t mac_korean_push(void *, char **, size_t *, char **,
			      size_t *);
...
static size_t mac_korean_char_push(u_int8_t * out, const ucs2_t * in,
				   size_t *size)
{
	ucs2_t wc = in[0];
	...
	if ((wc & ~7) == 0xf860) {
		...
		*size = 1;
		...
		while ((comb & ~15) == 0xf870 ||
			...
			ucs2_t comp =
			    cjk_compose(wc, comb, mac_korean_compose,
					sizeof(mac_korean_compose) /
					sizeof(u_int32_t));
			if (!comp)
				break;
...
static size_t mac_korean_char_pull(ucs2_t * out, const u_int8_t * in,
				   size_t *size)
{
	u_int16_t c = in[0];
