			...
			char **outbuf, size_t *outbytesleft)
{
	size_t n = end - *inbuf;
	if (iconv
	    (cd, (ICONV_CONST char **) inbuf, &n, outbuf,
...
size_t
cjk_generic_push(size_t (*char_func)(u_int8_t *, const ucs2_t *, size_t *),
		 ...
		 char **outbuf, size_t *outbytesleft)
{
	...
	while (*inbytesleft >= sizeof(ucs2_t) && *outbytesleft > 0) {
		...
		if (in != *inbuf) {
			...
			*inbytesleft +=
			    cjk_iconv(cd, inbuf, in, outbuf, outbytesleft);
			if (in != *inbuf)
				return -1;
			errno = err;
		}
...
size_t
cjk_generic_pull(size_t (*char_func)(ucs2_t *, const u_int8_t *, size_t *),
		 void *cd, char **inbuf, size_t *inbytesleft,
		 char **outbuf, size_t *outbytesleft)
{
	char *in = *inbuf;
	...
	while (*inbytesleft > 0 && *outbytesleft >= sizeof(ucs2_t)) {
		...
		if (n == (size_t) -1)
			return -1;
		if (*outbytesleft < n * sizeof(ucs2_t))
			break;
...
u_int16_t cjk_lookup(u_int16_t c, const cjk_index_t * index,
		     const u_int16_t * charset)
{
	while (index->summary && c >= index->range[0]) {
		if (c <= index->range[1]) {
			const u_int16_t *summary =
			    index->summary[(c - index->range[0]) >> 4];
...
ucs2_t cjk_compose(ucs2_t base, ucs2_t comb, const u_int32_t * table,
		   size_t size)
{
...
ucs2_t cjk_compose_seq(const ucs2_t * in, size_t *len,
		       const u_int32_t * table, size_t size)
{
