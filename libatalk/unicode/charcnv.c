	...
	if (ret && strcasecmp(ret, "LOCALE") == 0) {
		...
		if (ln) {
			/* Check whether the charset name is supported
			   by iconv */
			atalk_iconv_t handle =
			    atalk_iconv_open(ln, "UCS-2");
			if (handle == (atalk_iconv_t) - 1) {
				LOG(log_debug, logtype_default,
				    "Locale charset '%s' unsupported, using ASCII instead",
				    ln);
				ln = "ASCII";
			} else {
...
charset_t add_charset(const char *name)
{
	...
	conv_handles[cur_charset_t][CH_UCS2] =
	    atalk_iconv_open(charset_name(CH_UCS2), name);
	...
	conv_handles[CH_UCS2][cur_charset_t] =
	    atalk_iconv_open(name, charset_name(CH_UCS2));
...
#ifdef DEBUG
	LOG(log_debug9, logtype_default, "Added charset %s with handle %u",
	    name, cur_charset_t);
#endif
...
void init_iconv(void)
{
	...
	for (c1 = 0; c1 < NUM_CHARSETS; c1++) {
		...
		if (c1 != CH_UCS2) {	/* avoid lost memory, make valgrind happy */
			conv_handles[CH_UCS2][c1] =
			    atalk_iconv_open(name, charset_name(CH_UCS2));
			if (conv_handles[CH_UCS2][c1] ==
			    (atalk_iconv_t) - 1) {
				LOG(log_error, logtype_default,
				    "Required conversion from %s to %s not supported",
				    charset_name(CH_UCS2), name);
 ...
 *
 **/
static size_t add_null(charset_t to, char *buf, size_t bytesleft,
		       size_t len)
{
				      ...
				      void *dest, size_t destlen)
{
	...
	if (descriptor == (atalk_iconv_t) - 1
	    || descriptor == (atalk_iconv_t) 0) {
		return (size_t) -1;
	}
	...
	if (retval == (size_t) -1) {
		...
		LOG(log_debug, logtype_default, "Conversion error: %s",
		    reason);
		return (size_t) -1;
	}
		      ...
		      void *dest, size_t destlen)
{
	...
	if ((size_t) -1 ==
	    (o_len =
	     convert_string_internal(from, CH_UCS2, src, srclen,
				     (char *) buffer, sizeof(buffer)))) {
		LOG(log_error, logtype_default,
		    "Conversion failed ( %s to CH_UCS2 )",
		    charset_name(from));
		return (size_t) -1;
	}
	...
	if (charsets[to] && (charsets[to]->flags & CHARSET_DECOMPOSED)) {
		if ((size_t) -1 ==
		    (i_len = decompose_w(buffer, o_len, u, &i_len)))
			return (size_t) -1;
	} else if (!charsets[from]
		   || (charsets[from]->flags & CHARSET_DECOMPOSED)) {
		if ((size_t) -1 ==
		    (i_len = precompose_w(buffer, o_len, u, &i_len)))
			return (size_t) -1;
	} else {
		u = buffer;
		i_len = o_len;
	...
	if ((size_t) (-1) ==
	    (o_len =
	     convert_string_internal(CH_UCS2, to, (char *) u, i_len, dest,
				     destlen))) {
		LOG(log_error, logtype_default,
		    "Conversion failed (CH_UCS2 to %s):%s",
...
static size_t convert_string_allocate_internal(charset_t from,
					       charset_t to,
					       void const *src,
					       size_t srclen, char **dest)
{
			       ...
			       void const *src, size_t srclen, char **dest)
{
	...
	*dest = NULL;
	...
	if ((size_t) (-1) ==
	    (o_len =
	     convert_string_internal(from, CH_UCS2, src, srclen, buffer,
				     sizeof(buffer)))) {
		LOG(log_error, logtype_default,
		    "Conversion failed ( %s to CH_UCS2 )",
		    charset_name(from));
	...
	if ((size_t) -1 ==
	    (o_len =
	     convert_string_allocate_internal(CH_UCS2, to, (char *) u,
					      i_len, dest)))
		LOG(log_error, logtype_default,
		    "Conversion failed (CH_UCS2 to %s):%s",
		    charset_name(to), strerror(errno));
			...
			char *dest, size_t destlen)
{
	...
	size =
	    convert_string_internal(CH_UCS2, ch, buffer, size, dest,
				    destlen);
	free(buffer);
	return size;
}
...
size_t charset_to_ucs2_allocate(charset_t ch, ucs2_t ** dest,
				const char *src)
{
	size_t src_len = strlen(src);

	*dest = NULL;
	return convert_string_allocate(ch, CH_UCS2, src, src_len,
				       (char **) dest);
}
...
/* --------------------------------- */
size_t ucs2_to_charset_allocate(charset_t ch, char **dest,
				const ucs2_t * src)
{
	size_t src_len = (strlen_w(src)) * sizeof(ucs2_t);
	*dest = NULL;
			  ...
			  size_t outlen)
{
	...
	if ((size_t) (-1) ==
	    (len =
	     convert_string_allocate_internal(ch, CH_UCS2, src, inlen,
					      &buffer)))
		return len;
	...
	if ((size_t) -1 ==
	    (ilen = precompose_w((ucs2_t *) buffer, len, u, &ilen))) {
		free(buffer);
		return (size_t) (-1);
	}
	...
	if ((size_t) (-1) ==
	    (len =
	     convert_string_internal(CH_UCS2, ch, (char *) u, ilen, dst,
				     outlen))) {
		free(buffer);
		return (size_t) (-1);
	}
			 ...
			 size_t outlen)
{
	...
	if ((size_t) -1 ==
	    (ilen = decompose_w((ucs2_t *) buffer, len, u, &ilen))) {
		free(buffer);
		return (size_t) (-1);
	}
...
static size_t pull_charset_flags(charset_t from_set, charset_t cap_set,
				 ...
				 u_int16_t * flags)
{
	...
	while (i_len > 0) {
		...
		if (i_len > 0 &&
		    atalk_iconv(descriptor, &inbuf, &i_len, &outbuf,
				&o_len) == (size_t) -1) {
			if (errno == EILSEQ || errno == EINVAL) {
				...
				if ((option & CONV__EILSEQ)) {
					...
					*((ucs2_t *) outbuf) = (ucs2_t) IGNORE_CHAR;
								/**inbuf */
					inbuf++;
					i_len--;
					outbuf += 2;
		...
		if (j) {
			...
			if (hlen) {
				const char *h_buf = h;
				if (atalk_iconv
				    (descriptor_cap, &h_buf, &hlen,
				     &outbuf, &o_len) == (size_t) -1) {
					i_len += hlen * 3;
					inbuf -= hlen * 3;
					if (errno == EILSEQ
					    && (option & CONV_IGNORE)) {
...
static size_t push_charset_flags(charset_t to_set, charset_t cap_set,
				 char *src, size_t srclen, char *dest,
				 size_t destlen, u_int16_t * flags)
{
	const u_int16_t option = (flags ? *flags : 0);
	...
	if ((option & CONV_ESCAPEDOTS) && i_len >= 2 && SVAL(inbuf, 0) == 0x002e) {	/* 0x002e = . */
		if (o_len < 3) {
			errno = E2BIG;
			goto end;
		}
...
size_t convert_charset(charset_t from_set, charset_t to_set,
		       charset_t cap_charset, const char *src,
		       size_t src_len, char *dest, size_t dest_len,
		       u_int16_t * flags)
{
	size_t i_len, o_len;
	ucs2_t *u;
	...
	if ((size_t) (-1) ==
	    (o_len =
	     pull_charset_flags(from_set, cap_charset, src, src_len,
				(char *) buffer, sizeof(buffer) - 2,
				flags))) {
		LOG(log_error, logtype_default,
		    "Conversion failed ( %s to CH_UCS2 )",
		    charset_name(from_set));
		return (size_t) -1;
	}
	...
	u = buffer2;
	if (CHECK_FLAGS(flags, CONV_DECOMPOSE)
	    || (charsets[to_set]
		&& (charsets[to_set]->flags & CHARSET_DECOMPOSED))) {
		if ((size_t) -1 ==
		    (i_len = decompose_w(buffer, o_len, u, &i_len)))
			return (size_t) (-1);
	} else if (CHECK_FLAGS(flags, CONV_PRECOMPOSE)
		   || !charsets[from_set]
	...
	if ((size_t) (-1) ==
	    (o_len =
	     push_charset_flags(to_set, cap_charset, (char *) u, i_len,
				dest, dest_len, flags))) {
		LOG(log_error, logtype_default,
		    "Conversion failed (CH_UCS2 to %s):%s",
		    charset_name(to_set), strerror(errno));
		return (size_t) -1;
	}
	/* null terminate */
	dest[o_len] = 0;
