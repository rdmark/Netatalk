...
static int ad_update(struct adouble *ad, const char *path)
{
	...
	LOG(log_maxdebug, logtype_default,
	    "ad_update: checking whether '%s' needs an upgrade.", path);
	...
	if (ad->ad_eid[ADEID_RFORK].ade_off) {
		shiftdata =
		    ADEDOFF_RFORK_V2 - ad->ad_eid[ADEID_RFORK].ade_off;
	}

	memcpy(&nentries, ad->ad_data + ADEDOFF_NENTRIES,
	       sizeof(nentries));
	...
	if (st.st_size > 0x7fffffff) {
		LOG(log_debug, logtype_default,
		    "ad_update: file '%s' too big for update.", path);
		errno = EIO;
		goto bail_lock;
	}
	...
	if (off > st.st_size) {
		LOG(log_error, logtype_default,
		    "ad_update: invalid resource fork offset. (off: %u)",
		    off);
		errno = EIO;
		goto bail_lock;
	}
	...
	if (ad->ad_eid[ADEID_RFORK].ade_len > st.st_size - off) {
		LOG(log_error, logtype_default,
		    "ad_update: invalid resource fork length. (rfork len: %u)",
	...
	if (off) {
		memmove(buf + ADEDOFF_RFORK_V2, buf + off,
			ad->ad_eid[ADEID_RFORK].ade_len);
	}
	...
	while (eid->id) {
		if (ad->ad_eid[eid->id].ade_off != 0) {
			if (eid->id > 2
			    && ad->ad_eid[eid->id].ade_len < 256)
				memcpy(databuf[eid->id],
				       ad->ad_data +
				       ad->ad_eid[eid->id].ade_off,
				       ad->ad_eid[eid->id].ade_len);
	...
	while (eid->id) {
		if (eid->id > 2 && entry_len[eid->id] > 0) {
			memcpy(ad->ad_data + eid->offset, databuf[eid->id],
			       entry_len[eid->id]);
		}
...
static int ad_convert(struct adouble *ad, const char *path)
{
	...
	/* check to see if we should convert this header. */
	toV2 = ad->ad_version == AD_VERSION1
	    && ad->ad_flags == AD_VERSION2;
	toV1 = ad->ad_version == AD_VERSION2
	    && ad->ad_flags == AD_VERSION1;
	...
	if (off > st.st_size) {
		LOG(log_error, logtype_default,
		    "ad_v1tov2: invalid resource fork offset. (off: %u)",
		    off);
		errno = EIO;
		goto bail_lock;
	}
	...
	if (ad->ad_eid[ADEID_RFORK].ade_len > st.st_size - off) {
		LOG(log_error, logtype_default,
		    "ad_v1tov2: invalid resource fork length. (rfork len: %u)",
		    ad->ad_eid[ADEID_RFORK].ade_len);
		errno = EIO;
		goto bail_lock;
	}
	...
	memmove(ad->ad_data + ADEDOFF_NAME_V2,
		ad->ad_data + ADEDOFF_NAME_V1,
...
static int ad_header_read(struct adouble *ad, struct stat *hst)
{
	...
	/* read the header */
	if ((header_len =
	     adf_pread(ad->ad_md, buf, sizeof(ad->ad_data), 0)) < 0) {
		return -1;
	}
	...
	if (!ad->ad_magic && !ad->ad_version) {
		if (!warning) {
			LOG(log_debug, logtype_default,
			    "notice: fixing up null v1 magic/version.");
			warning++;
		}
		...
		if (!warning) {
			LOG(log_debug, logtype_default,
			    "notice: fixing up byte-swapped v1 magic/version.");
			warning++;
		}
	...
	if ((ad->ad_magic != AD_MAGIC) || ((ad->ad_version != AD_VERSION1)
		...
		LOG(log_debug, logtype_default,
		    "ad_open: can't parse AppleDouble header.");
		errno = EIO;
		return -1;
	}
	...
	if (!ad_getentryoff(ad, ADEID_RFORK)
		...
		LOG(log_debug, logtype_default,
		    "ad_header_read: problem with rfork entry offset.");
		errno = EIO;
		return -1;
	}
	...
	if (ad_getentryoff(ad, ADEID_RFORK) > header_len) {
		LOG(log_debug, logtype_default,
		    "ad_header_read: can't read in entries.");
		errno = EIO;
		return -1;
	}
	...
	if (ad->ad_version == AD_VERSION1) {
		...
		if (((ad_getdate(ad, AD_DATE_MODIFY | AD_DATE_UNIX, &aint))
		     == 0) && (aint > TIMEWARP_DELTA + hst->st_mtime)) {
			ad_setdate(ad, AD_DATE_MODIFY | AD_DATE_UNIX,
				   aint - AD_DATE_DELTA);
			ad_getdate(ad, AD_DATE_CREATE | AD_DATE_UNIX,
				   &aint);
			ad_setdate(ad, AD_DATE_CREATE | AD_DATE_UNIX,
				   aint - AD_DATE_DELTA);
			ad_getdate(ad, AD_DATE_BACKUP | AD_DATE_UNIX,
				   &aint);
			ad_setdate(ad, AD_DATE_BACKUP | AD_DATE_UNIX,
				   aint - AD_DATE_DELTA);
		}
	}
...
#endif
	if ((ad->ad_magic != SFM_MAGIC)
	    || ((ad->ad_version != AD_VERSION1))) {
		errno = EIO;
		LOG(log_debug, logtype_default,
		    "ad_header_sfm_read: can't parse AppleDouble header.");
		return -1;
	}
...
char *ad_path(const char *path, int adflags)
{
	...
	if (adflags & ADFLAGS_DIR) {
		...
		strlcpy(pathbuf + l, ".AppleDouble/.Parent",
			sizeof(pathbuf) - l);
	} else {
		...
		l += strlcpy(pathbuf + l, ".AppleDouble/",
			     sizeof(pathbuf) - l);
 ...
 *
 */
char *ad_path_sfm(const char *path, int adflags)
{
	static char pathbuf[MAXPATHLEN + 1];
	char c, *slash, buf[MAXPATHLEN + 1];
	size_t l;
...
int ad_mkdir(const char *path, int mode)
{
	...
	LOG(log_debug, logtype_default,
	    "ad_mkdir(\"%s\", %04o) {cwd: \"%s\"}", path, mode,
	    getcwdpath());
...
#if AD_VERSION == AD_VERSION2
	int ret;
	if ((ret = ad_convert(ad, name)) < 0
	    || (ret = ad_update(ad, name) < 0)) {
		return ret;
	}
#endif
	return 0;
 ...
 */
int ad_open(const char *path, int adflags, int oflags, int mode,
	    struct adouble *ad)
{
	struct stat st_dir;
	struct stat st_meta;
	...
	if ((adflags & ADFLAGS_DF)) {
		if (ad_data_fileno(ad) == -1) {
			...
			ad->ad_data_fork.adf_fd =
			    open(path, hoflags | ad_get_syml_opt(ad),
				 admode);
			...
			if (ad->ad_data_fork.adf_fd == -1) {
				...
				if (ad->ad_data_fork.adf_fd == -1
				    && errno == OPEN_NOFOLLOW_ERRNO) {
					int lsz;

					ad->ad_data_fork.adf_syml =
					    malloc(MAXPATHLEN + 1);
					lsz =
					    readlink(path,
						     ad->ad_data_fork.
	...
	if (ad->ad_md->adf_fd < 0) {
		if ((errno == EACCES || errno == EROFS)
			...
			hoflags = oflags & ~(O_CREAT | O_EXCL);
			ad->ad_md->adf_fd =
			    open(ad_p, hoflags | ad_get_syml_opt(ad), 0);
		}
	}
	...
	if (ad->ad_md->adf_fd < 0) {
		if (errno == ENOENT && (oflags & O_CREAT)) {
			 ...
			 */
			LOG(log_debug, logtype_default,
			    "ad_open(\"%s\"): {cwd: \"%s\"} creating adouble file",
			    ad_p, getcwdpath());
			admode = mode;
			errno = 0;
			...
			if ((errno == ENOENT)
			    && (ad->ad_flags != AD_VERSION2_OSX)) {
				if (ad->ad_ops->ad_mkrf(ad_p) < 0) {
					return ad_error(ad, adflags);
				}
				admode = mode;
				st_invalid =
				    ad_mode_st(ad_p, &admode, &st_dir);
				if ((ad->ad_options & ADVOL_UNIXPRIV)) {
					admode = mode;
				}
				admode = ad_hf_mode(admode);
		...
		if (fstat(ad->ad_md->adf_fd, &st_meta) == 0
		    && st_meta.st_size == 0) {
	...
	if (ad->ad_resource_fork.adf_fd < 0) {
		if ((errno == EACCES || errno == EROFS)
		    && !(oflags & O_RDWR)) {
			hoflags = oflags;
			ad->ad_resource_fork.adf_fd =
			    open(ad_p, hoflags, admode);
		}
	}
...
int ad_metadata(const char *name, int flags, struct adouble *adp)
{
	...
	if ((ret = ad_open(name, ADFLAGS_HF | dir, create, 0666, adp)) < 0
	    && errno == EACCES) {
		uid = geteuid();
		if (seteuid(0)) {
			LOG(log_error, logtype_default,
			    "ad_metadata(%s): seteuid failed %s", name,
			    strerror(errno));
			errno = EACCES;
			return -1;
		}
		...
		ret =
		    ad_open(name, ADFLAGS_HF | ADFLAGS_RDONLY | dir,
			    O_RDONLY, 0, adp);
		err = errno;
		if (seteuid(uid) < 0) {
			LOG(log_error, logtype_default,
			    "ad_metadata: can't seteuid back");
			exit(EXITERR_SYS);
		}
		errno = err;
	}
	...
	if (!ret && (ADFLAGS_OPENFORKS & flags)) {
		 ...
		 */
		adp->ad_open_forks |=
		    ad_openforks(adp, adp->ad_open_forks);
	}
	return ret;
}
 ...
 */
int ad_metadataat(int dirfd, const char *name, int flags,
		  struct adouble *adp)
{
	int ret = 0;
	int cwdfd = -1;
	...
	if (dirfd != -1) {
		if (fchdir(cwdfd) != 0) {
			LOG(log_error, logtype_afpd,
			    "ad_openat: cant chdir back, exiting");
			exit(EXITERR_SYS);
		}
	}
...
int ad_openat(int dirfd,	/* dir fd openat like */
	      const char *path,
	      int adflags, int oflags, int mode, struct adouble *ad)
{
	int ret = 0;
	int cwdfd = -1;
