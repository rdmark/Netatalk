...
ssize_t adf_pread(struct ad_fd *ad_fd, void *buf, size_t count,
		  off_t offset)
{
	ssize_t cc;
 ...
 *      problems. */
ssize_t ad_read(struct adouble *ad, const u_int32_t eid, off_t off,
		char *buf, const size_t buflen)
{
	...
	if (eid == ADEID_DFORK) {
		if (ad->ad_data_fork.adf_syml != 0) {
			...
			cc = adf_pread(&ad->ad_data_fork, buf, buflen,
				       off);
		}
	} else {
		off_t r_off;
		...
		if ((cc =
		     adf_pread(&ad->ad_resource_fork, buf, buflen,
			       r_off)) < 0) {
			return (-1);
		}
		/*
