...
int balloc(bstring b, int olen)
{
	...
	if (olen >= b->mlen) {
		...
		if (7 * b->mlen < 8 * b->slen) {
			...
			x = (unsigned char *) bstr__realloc(b->data,
							    (size_t) len);
			if (x == NULL) {
...
int ballocmin(bstring b, int len)
{
	...
	if (b == NULL || b->data == NULL || (b->slen + 1) < 0
	    || b->mlen <= 0 || b->mlen < b->slen || len <= 0) {
		return BSTR_ERR;
	}
...
int bconcat(bstring b0, const_bstring b1)
{
	...
	if (b0 == NULL || b1 == NULL || b0->data == NULL
	    || b1->data == NULL)
		return BSTR_ERR;
...
int bcatblk(bstring b, const void *s, int len)
{
	...
	if (b == NULL || b->data == NULL || b->slen < 0
	    || b->mlen < b->slen || b->mlen <= 0 || s == NULL || len < 0)
		return BSTR_ERR;
...
int biseqcaseless(const_bstring b0, const_bstring b1)
{
	...
	for (i = 0, n = b0->slen; i < n; i++) {
		if (b0->data[i] != b1->data[i]) {
			unsigned char c =
			    (unsigned char) downcase(b0->data[i]);
...
{
	if (b0 == NULL || b1 == NULL || b0->data == NULL
	    || b1->data == NULL || b0->slen < 0 || b1->slen < 0)
		return BSTR_ERR;
	if (b0->slen != b1->slen)
		return BSTR_OK;
	if (b0->data == b1->data || b0->slen == 0)
...
int binsert(bstring b1, int pos, const_bstring b2, unsigned char fill)
{
	...
	if (l > d) {
		...
		bstr__memset(b1->data + b1->slen, (int) fill,
			     (size_t) (pos - b1->slen));
		b1->slen = l;
	} else {
			     ...
			     instr_fnptr instr)
{
	...
	if (delta == 0) {
		while ((pos = instr(b, pos, auxf)) >= 0) {
			bstr__memcpy(b->data + pos, auxr->data,
				     auxr->slen);
			pos += auxf->slen;
		}
		if (auxf != find)
	...
	if (delta > 0) {
		...
		while ((i = instr(b, pos, auxf)) >= 0) {
			if (acc && i > pos)
				bstr__memmove(b->data + pos - acc,
					      b->data + pos, i - pos);
			if (auxr->slen)
				bstr__memcpy(b->data + i - acc, auxr->data,
					     auxr->slen);
			acc += delta;
			pos = i + auxf->slen;
		}
	...
	while ((pos = instr(b, pos, auxf)) >= 0) {
		if (slen + 1 >= mlen) {
			...
			if (sl < mlen
			    || NULL == (t =
					(int *) bstr__realloc(d, sl))) {
				ret = BSTR_ERR;
				goto done;
			}
			if (NULL == d)
				bstr__memcpy(t, static_d,
					     sizeof(static_d));
			d = t;
		}
		d[slen] = pos;
		...
		int (*cb)(void *parm, int ofs, int len), void *parm)
{
	...
	for (i = p = pos; i <= str->slen - splitStr->slen; i++) {
		if (0 ==
		    bstr__memcmp(splitStr->data, str->data + i,
				 splitStr->slen)) {
			if ((ret = cb(parm, p, i - p)) < 0)
				return ret;
			i += splitStr->slen;
...
static int bscb(void *parm, int ofs, int len)
{
	...
	if (g->bl->qty >= g->bl->mlen) {
		...
		tbl =
		    (bstring *) bstr__realloc(g->bl->entry,
					      sizeof(bstring) * mlen);
		if (tbl == NULL)
			return BSTR_ERR;
...
struct bstrList *bsplit(const_bstring str, unsigned char splitChar)
{
	...
	g.bl->mlen = 4;
	g.bl->entry =
	    (bstring *) bstr__alloc(g.bl->mlen * sizeof(bstring));
	if (NULL == g.bl->entry) {
		bstr__free(g.bl);
		return NULL;
	}
...
int bvcformata(bstring b, int count, const char *fmt, va_list arg)
{
	...
	if (n >=
	    (l =
	     b->slen + (int) (strlen) ((const char *) b->data +
				       b->slen))) {
		b->slen = l;
		return BSTR_OK;
	}
