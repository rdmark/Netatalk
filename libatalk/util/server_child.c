...
static struct server_child_data *resolve_child(struct server_child_data
					       **table, pid_t pid)
{
	struct server_child_data *child;
...
server_child *server_child_alloc(const int connections, const int nforks)
{
	...
	children->fork =
	    (void *) calloc(nforks, sizeof(server_child_fork));
 ...
 */
afp_child_t *server_child_add(server_child * children, int forkid,
			      pid_t pid, int ipc_fd)
{
	...
	if (kill(pid, 0) < 0) {
		LOG(log_error, logtype_default,
		    "server_child_add: no such process pid [%d]", pid);
		goto exit;
	}
...
static int kill_child(struct server_child_data *child)
{
	if (!child->killed) {
		...
		LOG(log_info, logtype_default,
		    "Unresponsive child[%d], sending SIGKILL", child->pid);
...
int server_child_transfer_session(server_child * children,
				  ...
				  pid_t pid,
				  uid_t uid,
				  int afp_socket, uint16_t DSI_requestID)
{
	EC_INIT;
	server_child_fork *fork;
	struct server_child_data *child;
	...
	if ((child = resolve_child(fork->table, pid)) == NULL) {
		LOG(log_note, logtype_default, "Reconnect: no child[%u]",
		    pid);
		if (kill(pid, 0) == 0) {
			LOG(log_note, logtype_default,
			    "Reconnect: terminating old session[%u]", pid);
			kill(pid, SIGTERM);
			sleep(2);
			if (kill(pid, 0) == 0) {
				LOG(log_error, logtype_default,
				    "Reconnect: killing old session[%u]",
				    pid);
				kill(pid, SIGKILL);
				sleep(2);
			}
	...
	if (!child->valid) {
		...
		LOG(log_error, logtype_default,
		    "Reconnect: invalidated child[%u]", pid);
		return 0;
	} else if (child->uid != uid) {
		LOG(log_error, logtype_default,
		    "Reconnect: child[%u] not the same user", pid);
		return 0;
	}

	LOG(log_note, logtype_default,
	    "Reconnect: transfering session to child[%u]", pid);
...
void server_child_kill_one_by_id(server_child * children, int forkid,
				 pid_t pid, uid_t uid, uint32_t idlen,
				 char *id, uint32_t boottime)
{
	server_child_fork *fork;
	struct server_child_data *child, *tmp;
	int i;
	...
	for (i = 0; i < CHILD_HASHSIZE; i++) {
		...
		while (child) {
			tmp = child->next;
			if (child->pid != pid) {
				if (child->idlen == idlen
				    && memcmp(child->clientid, id,
					      idlen) == 0) {
					if (child->time != boottime) {
						...
						LOG(log_debug,
						    logtype_default,
						    "Found another session[%u] for client[%u]",
						    child->pid, pid);
					}
				}
			} else {
