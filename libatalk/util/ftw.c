...
static int add_object(struct ftw_data *data, struct STAT *st)
{
	...
	newp->dev = st->st_dev;
	newp->ino = st->st_ino;
	return __tsearch(newp, &data->known_objects,
			 object_compare) ? 0 : -1;
}
...
open_dir_stream(int *dfdp, struct ftw_data *data, struct dir_data *dirp)
{
	...
	if (data->dirstreams[data->actdir] != NULL) {
		...
		if (buf == NULL)
			...
			while ((d = __readdir64(st)) != NULL) {
				...
				*((char *)
				  __mempcpy(buf + actsize, d->d_name,
					    this_len))
				    = '\0';
				actsize += this_len + 1;
			}
			...
			if (data->dirstreams[data->actdir]->content ==
				...
				__closedir(st);
				data->dirstreams[data->actdir]->stream =
				    NULL;
				data->dirstreams[data->actdir]->streamfd =
				    -1;
	...
	if (result == 0) {
		...
		if (dfdp != NULL && *dfdp != -1) {
			int fd =
			    openat(*dfdp, data->dirbuf + data->ftw.base,
				   O_RDONLY);
			dirp->stream = NULL;
			if (fd != -1
			    && (dirp->stream = fdopendir(fd)) == NULL)
				close(fd);
		} else {
			const char *name;
...
static int
process_entry(struct ftw_data *data, struct dir_data *dir,
	...
	*((char *) __mempcpy(data->dirbuf + data->ftw.base, name, namlen))
	...
	if (dir->streamfd != -1)
		statres = FXSTATAT(_STAT_VER, dir->streamfd, name, &st,
				   (data->
				    flags & FTW_PHYS) ? AT_SYMLINK_NOFOLLOW
				   : 0);
	else {
	...
	if (statres < 0) {
		if (errno != EACCES && errno != ENOENT)
			...
			if (dir->streamfd != -1)
				statres =
				    FXSTATAT(_STAT_VER, dir->streamfd,
					     name, &st,
					     AT_SYMLINK_NOFOLLOW);
			else
...
ftw_dir(struct ftw_data *data, struct STAT *st, struct dir_data *old_dir)
{
	...
	result =
	    open_dir_stream(old_dir == NULL ? NULL : &old_dir->streamfd,
			    data, &dir);
	if (result != 0) {
		if (errno == EACCES)
	...
	if (!(data->flags & FTW_DEPTH)) {
		result =
		    (*data->func) (data->dirbuf, st, FTW_D, &data->ftw);
		if (result != 0) {
			int save_err;
		      fail:
			save_err = errno;
	...
	if (dir.stream != NULL) {
		...
		while (result == 0 && *runp != '\0') {
			...
			result =
			    process_entry(data, &dir, runp, endp - runp);
	...
	if (result == 0 && (data->flags & FTW_DEPTH))
		result =
		    (*data->func) (data->dirbuf, st, FTW_DP, &data->ftw);
...
int NFTW_NAME(const char *path,
	      NFTW_FUNC_T func,
	      dir_notification_func_t up, int descriptors, int flags)
{
