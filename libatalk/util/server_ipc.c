...
static int ipc_get_session(struct ipc_header *ipc, server_child * children)
{
	...
	LOG(log_debug, logtype_afpd,
	    "ipc_get_session(pid: %u, uid: %u, time: 0x%08x)",
	    ipc->child_pid, ipc->uid, boottime);

	server_child_kill_one_by_id(children,
				    CHILD_DSIFORK,
				    ipc->child_pid,
				    ipc->uid, idlen, clientid, boottime);
...
int ipc_server_uds(const char *name)
{
	...
	address_length =
	    sizeof(address.sun_family) + sprintf(address.sun_path, "%s",
						 name);
	EC_ZERO_LOG(bind
		    (fd, (struct sockaddr *) &address, address_length));
...
int ipc_client_uds(const char *name)
{
	...
	LOG(log_debug, logtype_afpd,
	    "ipc_client_uds: connected to master");

	EC_ZERO_LOG(setnonblock(fd, 1));

	if (writet(fd, &pid, sizeof(pid_t), 0, 1) != sizeof(pid_t)) {
		LOG(log_error, logtype_afpd, "ipc_client_uds: writet: %s",
		    strerror(errno));
		EC_FAIL;
	}
	...
	while (retrycount++ < 10) {
		if ((obj->ipc_fd = ipc_client_uds(_PATH_AFP_IPC)) == -1) {
			LOG(log_error, logtype_afpd,
			    "reconnect_ipc: cant reconnect to master");
			sleep(1);
			continue;
		}
		LOG(log_debug, logtype_afpd,
		    "reconnect_ipc: succesfull IPC reconnect");
		return 0;
	}
	return -1;
}
...
int ipc_server_read(server_child * children, int fd)
{
	...
	if (ipc.len > (IPC_MAXMSGSIZE - IPC_HEADERLEN)) {
		LOG(log_info, logtype_afpd,
		    "IPC message exceeds allowed size (%u)", ipc.len);
		return -1;
	}
	...
	case IPC_DISCOLDSESSION:
		if (readt(fd, &ipc.DSI_requestID, 2, 0, 2) != 2) {
			LOG(log_error, logtype_afpd,
			    "ipc_read(%s:child[%u]): couldnt read DSI id: %s",
			    ipc_cmd_str[ipc.command], ipc.child_pid,
			    strerror(errno));
			return -1;
		}
	...
	default:
		LOG(log_info, logtype_afpd,
		    "ipc_read: unknown command: %d", ipc.command);
		return -1;
	}
...
int ipc_child_write(int fd, uint16_t command, int len, void *msg)
{
	...
	LOG(log_debug, logtype_afpd, "ipc_child_write(%s)",
	    ipc_cmd_str[command]);

	if ((ret =
	     writet(fd, block, len + IPC_HEADERLEN, 0,
		    1)) != len + IPC_HEADERLEN) {
		return -1;
	}
