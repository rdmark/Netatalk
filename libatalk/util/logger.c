...
static const char arr_loglevel_chars[] =
    { '-', 'S', 'E', 'W', 'N', 'I', 'D' };
static const unsigned int num_loglevel_chars =
COUNT_ARRAY(arr_loglevel_chars);
			      ...
			      const char *filename)
{
	...
	if (loglevel == NULL) {
		...
		for (levelnum = 1; levelnum < num_loglevel_strings;
		     levelnum++) {
			if (strcasecmp
			    (loglevel,
			     arr_loglevel_strings[levelnum]) == 0)
				break;
		}
				     ...
				     enum logtypes logtype)
{
	...
	*ptr = 0;
	...
	pid = getpid();
	templen =
	    snprintf(ptr, len, "%s[%d]", log_config.processname, pid);
	if (templen == -1 || templen >= len)
		return;
	len -= templen;
	ptr += templen;
	...
	if (!(display_options & logoption_nsrcinfo)) {
		...
		if (basename)
			templen =
			    snprintf(ptr, len, " {%s:%d}", basename + 1,
				     log_src_linenumber);
		else
			templen =
			    snprintf(ptr, len, " {%s:%d}",
				     log_src_filename, log_src_linenumber);
		if (templen == -1 || templen >= len)
			return;
		len -= templen;
		ptr += templen;
	...
	if (logtype < num_logtype_strings) {
		templen =
		    snprintf(ptr, len, "%s", arr_logtype_strings[logtype]);
		if (templen == -1 || templen >= len)
			return;
		len -= templen;
		ptr += templen;
	       ...
	       enum logtypes logtype)
{
	...
	LOG(log_debug, logtype_logger,
	    "Setup file logging: type: %s, level: %s, file: %s",
	    arr_logtype_strings[logtype], arr_loglevel_strings[loglevel],
	    filename);
}
		    ...
		    const char *file, int line, char *message, ...)
{
	...
	if (type_configs[logtype].syslog) {
		if (type_configs[logtype].level >= loglevel) {
			...
			vsnprintf(temp_buffer, MAXLOGSIZE - 1, message,
				  args);
			va_end(args);
	...
	for (int i = log_flood_entries - 1; i >= 0; i--) {
		if (log_flood_array[i].hash == hash) {
			...
			if (log_flood_array[i].count >=
			    LOG_FLOODING_MAXCOUNT) {
				/* yes, log it and remove from array */

				/* reusing log_details_buffer */
				sprintf(log_details_buffer,
					"message repeated %i times\n",
					LOG_FLOODING_MAXCOUNT - 1);
				write(fd, log_details_buffer,
				      strlen(log_details_buffer));
	...
	if (log_flood_entries == LOG_FLOODING_ARRAY_SIZE) {
		...
		if (log_flood_array[0].count >= LOG_FLOODING_MINCOUNT) {
			...
			sprintf(log_details_buffer,
				"message repeated %i times\n",
				log_flood_array[0].count -
				LOG_FLOODING_MINCOUNT + 1);
			write(fd, log_details_buffer,
			      strlen(log_details_buffer));
		}
	...
	if (!log_config.console) {
		generate_message_details(log_details_buffer,
					 sizeof(log_details_buffer),
					 type_configs[logtype].
					 set ? type_configs[logtype].
					 display_options :
					 type_configs[logtype_default].
					 display_options, loglevel,
					 logtype);
