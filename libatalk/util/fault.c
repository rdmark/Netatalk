...
void netatalk_panic(const char *why)
{
	...
	backtrace_strings =
	    backtrace_symbols(backtrace_stack, backtrace_size);

	LOG(log_severe, logtype_default, "BACKTRACE: %d stack frames:",
	    backtrace_size);
	...
	if (backtrace_strings) {
		...
		for (i = 0; i < backtrace_size; i++)
			LOG(log_severe, logtype_default, " #%u %s", i,
			    backtrace_strings[i]);
...
static void fault_report(int sig)
{
	...
	LOG(log_severe, logtype_default,
	    "===============================================================");
	LOG(log_severe, logtype_default,
	    "INTERNAL ERROR: Signal %d in pid %d (%s)", sig,
	    (int) getpid(), VERSION);
	LOG(log_severe, logtype_default,
	    "===============================================================");
