...
{
	if ((charset_t) - 1 ==
	    (vol->v_maccharset = add_charset(vol->v_maccodepage))) {
		fprintf(stderr,
	...
	if ((charset_t) - 1 ==
	    (vol->v_volcharset = add_charset(vol->v_volcodepage))) {
		fprintf(stderr,
...
static int parse_options(char *buf, int *flags,
			 const vol_opt_name_t * options)
{
	char *p, *q;
	...
	*value = '\0';
	...
	case ADOUBLE_VER:
		if (strcasecmp(value, "v1") == 0) {
			...
			fprintf(stderr, "unknown adouble version: %s, %s",
				buf, value);
			return -1;
		}
		break;
	case VOLUME_OPTS:
		...
		break;
	case VOLCASEFOLD:
		parse_options(value, &vol->v_casefold,
			      &vol_opt_casefold[0]);
		break;
	case EXTATTRTYPE:
		...
		break;
	default:
		fprintf(stderr, "unknown volume information: %s, %s", buf,
			value);
		return (-1);
		break;
	}
...
int loadvolinfo(char *path, struct volinfo *vol)
{
	...
	if (NULL == (fp = fopen(volinfofile, "r"))) {
		fprintf(stderr, "error opening volinfo (%s): %s",
			volinfofile, strerror(errno));
		return (-1);
	}
	fd = fileno(fp);
...
int closevolinfo(struct volinfo *volinfo)
{
	...
	if (volinfo->retaincount == 0) {
		...
		free(volinfo->v_path);
		volinfo->v_path = NULL;
		free(volinfo->v_cnidscheme);
		volinfo->v_cnidscheme = NULL;
		free(volinfo->v_dbpath);
		volinfo->v_dbpath = NULL;
		free(volinfo->v_volcodepage);
		volinfo->v_volcodepage = NULL;
		free(volinfo->v_maccodepage);
		volinfo->v_maccodepage = NULL;
		free(volinfo->v_dbd_host);
		volinfo->v_dbd_host = NULL;
		free(volinfo->v_dbd_port);
		volinfo->v_dbd_port = NULL;
		if (volinfo->malloced) {
			volinfo->malloced = 0;
			free(volinfo);
		}
 ...
 */
int savevolinfo(const struct vol *vol, const char *Cnid_srv,
		const char *Cnid_port)
{
	...
	if ((fd = open(item, O_RDWR | O_CREAT, 0666)) < 0) {
		LOG(log_debug, logtype_default, "Error opening %s: %s",
		    item, strerror(errno));
		if (process_uid) {
	...
	if (fcntl(fd, F_SETLK, &lock) < 0) {
		...
		if (errno == EACCES || errno == EAGAIN) {
			...
			LOG(log_error, logtype_default,
			    "savevoloptions: cannot get lock: %s",
			    strerror(errno));
			return (-1);
		}
	}
	...
	if (strlen(buf) >= sizeof(buf) - 1)
		LOG(log_debug, logtype_default,
		    "Error writing .volinfo file: buffer too small, %s",
		    buf);
	if (write(fd, buf, strlen(buf)) < 0
	    || ftruncate(fd, strlen(buf)) < 0) {
		LOG(log_debug, logtype_default,
		    "Error writing .volinfo file: %s", strerror(errno));
	}
