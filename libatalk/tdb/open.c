...
static void null_log_fn(struct tdb_context *tdb _U_, enum tdb_debug_level level _U_, const char *fmt _U_, ...)
{
}
                                         ...
                                         tdb_hash_func hash_fn)
{
    ...
    errno = 0;
    if (read(tdb->fd, &header, sizeof(header)) != sizeof(header) || strcmp(header.magic_food, TDB_MAGIC_FOOD) != 0) {
    ...
    } else if (header.version != TDB_VERSION && !(rev = (header.version == TDB_BYTEREV(TDB_VERSION)))) {
        /* wrong version */
        errno = EIO;
        goto fail;
    }
