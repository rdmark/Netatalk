...
static int fcntl_unlock(struct tdb_context *tdb, int rw _U_, off_t off, off_t len)
{
    struct flock fl;
...
static int tdb_allrecord_check(struct tdb_context *tdb, int ltype,
                               enum tdb_lock_flags flags _U_, bool upgradable)
{
...
int tdb_allrecord_unlock(struct tdb_context *tdb, int ltype, bool mark_lock)
{
    ...
    if (tdb->allrecord_lock.ltype != (uint32_t)ltype && (!tdb->allrecord_lock.off || ltype != F_RDLCK)) {
        tdb->ecode = TDB_ERR_LOCK;
        return -1;
    }
