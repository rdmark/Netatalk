...
_PUBLIC_ char *tdb_summary(struct tdb_context *tdb)
{
    ...
    for (off = TDB_DATA_START(tdb->hash_size);
        ...
        switch (rec.magic) {
            case TDB_MAGIC:
                ...
                tally_add(&extra, rec.rec_len - (rec.key_len + rec.data_len));
                if (unc > 1)
    ...
    len = asprintf(&ret, SUMMARY_FORMAT,
                   ...
                   (keys.num + freet.num + dead.num) * (sizeof(struct tdb_record) + sizeof(uint32_t)) * 100.0 / file_size,
                   tdb->hash_size * sizeof(tdb_off_t) * 100.0 / file_size);
    if (len == -1) {
        goto unlock;
    }
