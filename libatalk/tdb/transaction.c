                                 ...
                                 tdb_len_t *recovery_max_size)
{
    ...
    if (recovery_head == 0 || recovery_head + sizeof(rec) + rec.rec_len != tdb->map_size) {
    ...
    *recovery_max_size = tdb_expand_adjust(tdb->map_size,
    ...
    if (methods->tdb_expand_file(tdb, tdb->transaction->old_map_size,
                                 new_end - tdb->transaction->old_map_size) == -1) {
...
static int _tdb_transaction_prepare_commit(struct tdb_context *tdb)
{
    ...
    if (tdb_allrecord_upgrade(tdb) == -1) {
        ...
        } else if (tdb->ecode == TDB_ERR_RDONLY && tdb->traverse_read) {
            TDB_LOG((tdb, TDB_DEBUG_ERROR,
                     "tdb_transaction_prepare_commit: "
                     "failed to upgrade hash locks: "
