...
*/
static int transaction_oob(struct tdb_context *tdb, tdb_off_t len, int probe _U_)
{
	if (len <= tdb->map_size) {
		return 0;
	}
...
*/
static int transaction_brlock(struct tdb_context *tdb _U_, tdb_off_t offset _U_,
			      int rw_type _U_, int lck_type _U_, int probe _U_, size_t len _U_)
{
	return 0;
}
