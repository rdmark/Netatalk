...
size_t tdb_mutex_size(struct tdb_context *tdb _U_)
{
    return 0;
}

bool tdb_have_mutexes(struct tdb_context *tdb _U_)
{
    return false;
}

int tdb_mutex_allrecord_lock(struct tdb_context *tdb, int ltype _U_,
                             enum tdb_lock_flags flags _U_)
{
    tdb->ecode = TDB_ERR_LOCK;
    return -1;
}

int tdb_mutex_allrecord_unlock(struct tdb_context *tdb _U_)
{
    return -1;
}
...
void tdb_mutex_allrecord_downgrade(struct tdb_context *tdb _U_)
{
    return;
}

int tdb_mutex_mmap(struct tdb_context *tdb _U_)
{
    errno = ENOSYS;
    return -1;
}

int tdb_mutex_munmap(struct tdb_context *tdb _U_)
{
    errno = ENOSYS;
    return -1;
}

int tdb_mutex_init(struct tdb_context *tdb _U_)
{
    errno = ENOSYS;
    return -1;
}
