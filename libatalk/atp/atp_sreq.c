 ...
 */
int atp_sreq(ATP ah, struct atp_block *atpb, int respcount, u_int8_t flags)
{
	...
	atp_build_req_packet(req_buf, ah->atph_tid++, flags | ATP_TREQ,
			     atpb);
	...
	if ((random() % 3) != 2) {
#endif				/* DROPPACKETS */
		if (netddp_sendto
		    (ah->atph_socket, req_buf->atpbuf_info.atpbuf_data,
		     req_buf->atpbuf_dlen, 0,
		...
		printf("<%d> atp_sreq: dropped request\n", getpid());
#endif				/* DROPPACKETS */
