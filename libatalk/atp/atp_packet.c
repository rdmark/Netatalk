...
void atp_build_req_packet(struct atpbuf *pktbuf,
			  u_int16_t tid,
			  u_int8_t ctrl, struct atp_block *atpb)
{
	struct atphdr hdr;
...
void atp_build_resp_packet(struct atpbuf *pktbuf,
			   u_int16_t tid,
			   u_int8_t ctrl,
			   struct atp_block *atpb, u_int8_t seqnum)
{
	struct atphdr hdr;
	...
	*(pktbuf->atpbuf_info.atpbuf_data) = DDPTYPE_ATP;
	 ...
	 */
	pktbuf->atpbuf_dlen =
	    ATP_HDRSIZE + (size_t) atpb->atp_sresiov[seqnum].iov_len;
}
...
atp_recv_atp(ATP ah,
	     ...
	     u_int8_t * func, u_int16_t tid, char *rbuf, int wait)
{
/* 
...
*/
	...
	if (cq != NULL) {
		...
		*func = rfunc;
		memcpy(fromaddr, &cq->atpbuf_addr,
	...
	do {
		...
		if (recvlen >= ATP_HDRSIZE && *rbuf == DDPTYPE_ATP) {
			...
			if (rfunc == ATP_TREL) {
				...
				for (pq = NULL, cq = ah->atph_sent;
				     cq != NULL;
				     pq = cq, cq = cq->atpbuf_next) {
					if (at_addr_eq
					    (&faddr, &cq->atpbuf_addr)
					    && cq->atpbuf_info.atpbuf_xo.
					    atpxo_tid == ntohs(rtid))
						break;
				}
				if (cq != NULL) {
#ifdef EBUG
			...
			} else if (((tid & rtid) == rtid) && ((*func & rfunc) == rfunc) && at_addr_eq(fromaddr, &faddr)) {	/* got what we wanted */
				*func = rfunc;
				dlen = recvlen;
				memcpy(fromaddr, &faddr,
				       sizeof(struct sockaddr_at));
				...
				memcpy(inbuf->atpbuf_info.atpbuf_data,
				       rbuf, recvlen);
			}
		}
		if (!wait && dlen < 0) {
...
int at_addr_eq(struct sockaddr_at *paddr,	/* primary address */
...
{				/* secondary address */
	 ...
	 */ return ((paddr->sat_port == ATADDR_ANYPORT
			...
			|| paddr->sat_addr.s_node ==
			saddr->sat_addr.s_node));
}
