...
static int resend_request(ATP ah)
{
	...
	if (netddp_sendto(ah->atph_socket,
			  ...
			  (struct sockaddr *) &ah->atph_reqpkt->
			  atpbuf_addr,
...
int atp_rsel(ATP ah,		/* open atp handle */
	     struct sockaddr_at *faddr,	/* address to receive from */
	     int func)
{				/* which function(s) to wait for;
	...
	for (;;) {
		...
		if (requesting) {
			...
			tv.tv_usec = 0;
			if ((c =
			     select(ah->atph_socket + 1, &fds, NULL, NULL,
				    &tv)) < 0) {
				atp_free_buf(abuf);
				return (-1);
			}
	...
	if (rfunc == ATP_TREQ) {
		...
		for (pb = NULL, cb = ah->atph_sent; cb != NULL;
			...
			if (tv.tv_sec -
				...
				if (cb->atpbuf_info.atpbuf_xo.atpxo_tid ==
				    tid) {
					break;
				}
			}
...
#ifdef EBUG
			printf
			    ("<%d> duplicate request -- re-sending XO resp\n",
			     getpid());
#endif				/* EBUG */
	...
	if (requesting && ah->atph_rbitmap & (1 << resp_hdr.atphd_bitmap)
		...
		if (ah->atph_resppkt[resp_hdr.atphd_bitmap] != NULL) {
			atp_free_buf(ah->
				     atph_resppkt[resp_hdr.atphd_bitmap]);
		}
...
#ifdef EBUG
			printf
			    ("<%d> EOM -- seq num %d  current bitmap %d\n",
			     getpid(), resp_hdr.atphd_bitmap,
			     ah->atph_rbitmap);
#endif				/* EBUG */
...
#endif				/* EBUG */
			req_hdr.atphd_bitmap = ah->atph_rbitmap;
			memcpy(ah->atph_reqpkt->atpbuf_info.atpbuf_data +
			       1, &req_hdr, sizeof(struct atphdr));
			if (netddp_sendto
			    (ah->atph_socket,
			     ah->atph_reqpkt->atpbuf_info.atpbuf_data,
			     ah->atph_reqpkt->atpbuf_dlen, 0,
...
#ifdef EBUG
		printf
		    ("atp_rsel: ignoring resp bm=%x tid=%d (expected %x/%d)\n",
	...
	if (!ah->atph_rbitmap && (req_hdr.atphd_ctrlinfo & ATP_XO)) {
		...
		memset(ah->atph_reqpkt->atpbuf_info.atpbuf_data +
		       ATP_HDRSIZE, 0, 4);
		ah->atph_reqpkt->atpbuf_dlen =
		    sizeof(struct atphdr) + ATP_HDRSIZE;
#ifdef EBUG
...
#endif				/* DROP_ATPTREL */
			netddp_sendto(ah->atph_socket,
				      ah->atph_reqpkt->atpbuf_info.
				      atpbuf_data,
				      ah->atph_reqpkt->atpbuf_dlen, 0,
				      (struct sockaddr *) &ah->
				      atph_reqpkt->atpbuf_addr,
				      sizeof(struct sockaddr_at));
#ifdef DROP_ATPTREL
		}
#endif				/* DROP_ATPTREL */
