...
*/
int atp_sresp(ATP ah,		/* open atp handle */
...
{				/* parameter block */
	...
	if (ah->atph_rxo) {
		...
		for (i = 0; i < 8;
		     save_buf->atpbuf_info.atpbuf_xo.atpxo_packet[i++] =
		     NULL);
	}
	for (i = 0; i < atpb->atp_sresiovcnt; ++i) {
		...
		atp_build_resp_packet(resp_buf, ah->atph_rtid, ctrlinfo,
				      atpb, i);
		...
		if ((random() % 3) != 2) {
...
#ifdef EBUG
			printf
			    ("<%d> sending packet tid=%hu serial no.=%d\n",
			     getpid(), ah->atph_rtid, i);
			bprint(resp_buf->atpbuf_info.atpbuf_data,
			       resp_buf->atpbuf_dlen);
#endif				/* EBUG */
			if (netddp_sendto
			    (ah->atph_socket,
			     resp_buf->atpbuf_info.atpbuf_data,
			     resp_buf->atpbuf_dlen, 0,
			     (struct sockaddr *) atpb->atp_saddr,
			     sizeof(struct sockaddr_at)) !=
			    resp_buf->atpbuf_dlen) {
				if (ah->atph_rxo) {
					for (; i >= 0; --i) {
						atp_free_buf
						    (save_buf->atpbuf_info.
						     atpbuf_xo.
						     atpxo_packet[i]);
					}
			...
			printf("<%d> atp_sresp: dropped serial no. %d\n",
			       getpid(), i);
#endif				/* DROPPACKETS */
	...
	if (ah->atph_rxo) {
		...
		save_buf->atpbuf_info.atpbuf_xo.atpxo_reltime =
		    ah->atph_rreltime;
