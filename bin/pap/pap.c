...
int main(int ac, char **av)
{
	...
	if (printer == NULL && ((printer = paprc()) == NULL)) {
		fprintf(stderr,
			"No printer specified and ./.paprc not found.\n");
		exit(2);
	}
...
static int send_file(int fd, ATP atp, int lastfile)
{
	...
	for (;;) {
		...
		if (!fiovcnt && FD_ISSET(fd, &fds)) {
			...
			fiovcnt =
			    cc / PAP_MAXDATA + (cc % PAP_MAXDATA > 0);
			for (i = 0; cc > 0; i++) {
				rfiov[i].iov_len =
				    (cc > PAP_MAXDATA) ? PAP_MAXDATA : cc;
				cc -=
				    (cc > PAP_MAXDATA) ? PAP_MAXDATA : cc;
			}
		}
		...
		if (FD_ISSET(atp_fileno(atp), &fds)) {
			...
			case ATP_TREQ:
				...
				case PAP_CLOSE:
					 ...
					 */
					sniov[0].iov_len = 4;
					((char *) sniov[0].iov_base)[0] =
					    connid;
					((char *) sniov[0].iov_base)[1] =
					    PAP_CLOSEREPLY;
		 ...
		 */
		if (!waitforprinter && !senteof && data
		    && (fiovcnt || eof)) {
			ssat.sat_port = port;
			atpb.atp_saddr = &ssat;
			if (fiovcnt) {
				for (i = 0; i < fiovcnt; i++) {
					sniov[i].iov_len =
					    rfiov[i].iov_len + 4;
					((char *) sniov[i].iov_base)[0] =
					    connid;
					((char *) sniov[i].iov_base)[1] =
					    PAP_DATA;
					senteof =
					    ((char *) sniov[i].
					     iov_base)[2] = eof;
					((char *) sniov[i].iov_base)[3] =
					    0;
				}
			} else {
				sniov[0].iov_len = 4;
				...
				senteof = ((char *) sniov[0].iov_base)[2] =
				    eof;
			...
			if (waitforprinter) {
				...
				memcpy(st_buf,
				       (char *) rniov[0].iov_base + 9,
				       ((char *) rniov[0].iov_base)[8]);
				st_buf[(int) ((char *) rniov[0].iov_base)
				       [8]] = '\0';
