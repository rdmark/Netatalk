...
static int do_rename(char *src, char *dst, struct stat *st)
{
	...
	if (!stat(dst, &tmp_st)) {
		fprintf(stderr,
			"error: cannot rename %s to %s, destination exists\n",
			src, dst);
		return -1;
	}
	...
	if (rename(adsrc, ad_path(dst, 0)) < 0) {
		...
		if (errno == ENOENT) {
			...
			fprintf(stderr,
				"failed to rename resource fork, error: %s\n",
				strerror(errno));
			return -1;
		}
...
static char *convert_name(char *name, struct stat *st, cnid_t cur_did)
{
	...
	outlen =
	    convert_charset(ch_from, ch_to, ch_mac, (char *) p,
			    strlen((char *) p), (char *) q,
			    sizeof(buffer) - 2, &flags);
	if ((size_t) -1 == outlen) {
		if (ch_to == CH_UTF8) {
			...
			p = (unsigned char *) name;
			outlen =
			    convert_charset(ch_to, ch_to, ch_mac,
					    (char *) p, strlen((char *) p),
					    (char *) q, sizeof(buffer) - 2,
					    &flags);
			if ((size_t) -1 == outlen) {
				/* it's not UTF8... */
				fprintf(stderr,
					"ERROR: conversion from '%s' to '%s' for '%s' in DID %u failed!!!\n",
					from_charset, to_charset, name,
					ntohl(cur_did));
				return name;
			}
		...
		fprintf(stderr,
			"ERROR: conversion from '%s' to '%s' for '%s' in DID %u failed. Please check this!\n",
	...
	if (strcmp(name, buffer)) {
		if (dry_run) {
			fprintf(stdout,
				"dry_run: would rename %s to %s.\n", name,
				buffer);
		} else if (!do_rename(name, buffer, st)) {
			if (CNID_INVALID !=
			    (id =
			     cnid_add(cdb, st, cur_did, buffer,
				      strlen(buffer), 0)))
				fprintf(stdout,
					"converted '%s' to '%s' (ID %u, DID %u).\n",
					name, buffer, ntohl(id),
					ntohl(cur_did));
		}
	} else if (verbose > 1)
		fprintf(stdout, "no conversion required\n");
...
static int check_adouble(DIR * curdir, char *path _U_)
{
	...
	while (NULL != (ad_entry = readdir(adouble))) {
		...
		if (!found) {
			fprintf(stderr, "found orphaned resource file %s",
				ad_entry->d_name);
		}
	}
...
static cnid_t add_dir_db(char *name, cnid_t cur_did)
{
	...
	if (CNID_INVALID !=
	    (id = cnid_get(cdb, cur_did, name, strlen(name))))
		return id;
...
static int checkdir(DIR * curdir, char *path, cnid_t cur_did)
{
	...
	while (n--) {
		...
		if (ret == 1) {
			...
			if (cdir == NULL) {
				fprintf(stderr,
					"ERROR: cannot open DIR '%s' with ID %u\n",
...
static int init(char *path)
{
	...
	if (NULL ==
	    (cdb =
	     cnid_open(path, 0, cnid_type, 0, "localhost", "4700"))) {
		fprintf(stderr,
			"ERROR: cannot open CNID database in '%s'\n",
			path);
		fprintf(stderr,
			"ERROR: check the logs for reasons, aborting\n");
		return -1;
	}
...
{
	fprintf(stderr,
		"usage:\t%s [-ndv] -c cnid -f fromcode -t tocode [-m maccode] path\n",
		name);
...
static void help(void)
{
	...
	fprintf(stdout,
		"     uniconv -c dbd -f ASCII -t UTF8 -m MAC_ROMAN /path/to/share\n");
	fprintf(stdout,
		"     uniconv -c cdb -f ISO-8859-1 -t UTF8 -m MAC_ROMAN /path/to/share\n");
	fprintf(stdout,
		"     uniconv -c cdb -f ISO-8859-ADAPTED -t ASCII -m MAC_ROMAN /path/to/share\n");
	fprintf(stdout,
		"     uniconv -f UTF8 -t ASCII -m MAC_ROMAN /path/to/share\n\n");
	...
	fprintf(stdout,
		"\t\tIf not specified, the default cnid backend `%s' is used\n",
		DEFAULT_CNID_SCHEME);
	fprintf(stdout,
	...
	fprintf(stdout,
		"     Setting the wrong options might render your data unusable!!!\n");
	fprintf(stdout,
		"     Make sure you know what you are doing. Always backup your data first.\n\n");
	fprintf(stdout,
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "f:m:t:c:dnvVh")) != -1)
		...
		case 'n':
			fprintf(stderr,
				"doing dry run, volume will *not* be changed\n");
			dry_run = 1;
			break;
		case 'c':
	...
	if ((charset_t) - 1 == (ch_from = add_charset(from_charset))) {
		fprintf(stderr,
			"Setting codepage %s as source codepage failed",
			from_charset);
		exit(-1);
	}
	...
	if ((charset_t) - 1 == (ch_to = add_charset(to_charset))) {
		fprintf(stderr,
			"Setting codepage %s as destination codepage failed",
			to_charset);
		exit(-1);
	}
