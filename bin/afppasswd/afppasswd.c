...
static void convert_passwd(char *buf, char *newpwd, const int keyfd)
{
	...
	if (keyfd > -1) {
		...
		if (newpwd) {
			DES_ecb_encrypt((DES_cblock *) newpwd,
					(DES_cblock *) newpwd, &schedule,
					DES_ENCRYPT);
		} else {
			/* decrypt the password */
			DES_ecb_encrypt((DES_cblock *) buf,
					(DES_cblock *) buf, &schedule,
					DES_DECRYPT);
		}
...
int main(int argc, char **argv)
{
	...
	if (flags & OPT_CREATE) {
		if ((flags & OPT_ISROOT) == 0) {
			fprintf(stderr,
				"afppasswd: only root can create the password file.\n");
			return -1;
		}

		if (!i && ((flags & OPT_FORCE) == 0)) {
			fprintf(stderr,
				"afppasswd: password file already exists.\n");
			return -1;
		}
		...
		pwd =
		    (flags & OPT_ISROOT) ? getpwnam(argv[optind]) :
		    getpwuid(uid);
		if (pwd)
