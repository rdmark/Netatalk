...
u_int32_t ddp_recvspace = 10 * (587 + sizeof(struct sockaddr_at));
 ...
 /*ARGSUSED*/ ddp_usrreq(so, req, m, addr, rights)
struct socket *so;
int req;
...
struct mbuf *addr;
{
	...
	if (ro->ro_rt) {
		...
		if (aa == NULL || (satosat(&ro->ro_dst)->sat_addr.s_net !=
				   (hintnet ? hintnet : sat->sat_addr.
				    s_net)
				   || satosat(&ro->ro_dst)->sat_addr.
				   s_node != sat->sat_addr.s_node)) {
			RTFREE(ro->ro_rt);
...
struct ddpcb *ddp;
{
	...
	if (ddp->ddp_lsat.sat_port != ATADDR_ANYPORT &&
		...
		if (ddp->ddp_pprev != NULL) {
			...
			ddp_ports[ddp->ddp_lsat.sat_port - 1] =
			    ddp->ddp_pnext;
		}
 ...
 * sockets (pcbs).
 */
struct ddpcb *ddp_search(from, to, aa)
