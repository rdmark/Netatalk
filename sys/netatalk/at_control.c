...
int atalk_netmatch(struct sockaddr_at *sat1, struct sockaddr_at *sat2)
{
	...
	if (aa) {
		return (ntohs(aa->aa_firstnet) <=
			ntohs(sat2->sat_addr.s_net)
			&& ntohs(aa->aa_lastnet) >=
			ntohs(sat2->sat_addr.s_net));
	}
...
struct ifnet *ifp;
{
	...
	case SIOCSIFADDR:
		return (at_ifinit
			(ifp, aa, (struct sockaddr_at *) &ifr->ifr_addr));
...
struct at_ifaddr *aa;
{
	...
	if (aa->aa_flags & AFA_ROUTE) {
		...
		if ((error = rtinit(&(aa->aa_ifa), RTM_DELETE,
				    (ifp->
				     if_flags & IFF_LOOPBACK) ? RTF_HOST :
				    0)) != 0) {
			return (error);
		}
		...
		if (ifp->if_flags & IFF_LOOPBACK) {
			rtinit(&aa->aa_addr, &aa->aa_addr, SIOCDELRT,
			       RTF_HOST);
		} else {
			...
			if (aa->aa_firstnet == htons(0x0000) &&
				...
				rtinit((struct sockaddr *) &netsat,
				       &aa->aa_addr, (int) SIOCDELRT, 0);
			} else {
...
struct sockaddr_at *sat;
{
	...
	if (ifp->if_flags & IFF_LOOPBACK) {
		...
		for (i = nnets, netinc = 1; i > 0;
		     net =
		     ntohs(nr.nr_firstnet) +
		     ((net - ntohs(nr.nr_firstnet) + netinc) % nnets),
		     i--) {
	...
	if (ifp->if_flags & IFF_LOOPBACK) {
		 ...
		 */
		((struct sockaddr_at *) &aa->aa_broadaddr)->sat_addr.
		    s_net = sat->sat_addr.s_net;
		((struct sockaddr_at *) &aa->aa_broadaddr)->sat_addr.
		    s_node = ATADDR_BCAST;
		...
		if ((aa->aa_flags & AFA_PHASE2) == 0) {
			...
			if (aa->aa_firstnet == htons(0x0000) &&
				...
				rtinit((struct sockaddr *) &netaddr,
				       &aa->aa_addr, (int) SIOCADDRT,
				       RTF_UP);
			} else {
				for (net = ntohs(aa->aa_firstnet);
				     net <= ntohs(aa->aa_lastnet); net++) {
