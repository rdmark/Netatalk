...
struct ifaddr *at_ifawithnet(sat, ifa)
...
struct sockaddr_at *sat;
{
	 ...
	 */
	if ((aa =
	     (struct at_ifaddr *) at_ifawithnet(sat,
						ac->ac_if.if_addrlist))
	    == NULL) {
		m_freem(m);
		return;
	}
	...
	if (aa->aa_flags & AFA_PHASE2) {
		...
		eh->ether_type =
		    sizeof(struct llc) + sizeof(struct ether_aarp);
#ifdef __NetBSD__
...
struct mbuf *m;
{
	...
	if (aat == 0 && tpa.s_net == ma.s_net && tpa.s_node == ma.s_node
	    && op != AARPOP_PROBE) {
		if (aat = aarptnew(&spa)) {
			bcopy((caddr_t) ea->aarp_sha,
			      (caddr_t) aat->aat_enaddr,
			      sizeof(ea->aarp_sha));
			aat->aat_flags |= ATF_COM;
		}
	}
